...
{
    EXPECT_EQ(thresholds::getInterface(thresholds::Level::WARNING), "xyz.openbmc_project.Sensor.Threshold.Warning");
...
TEST(Threshold, parseThresholdsFromAttr)
{
    ...
    std::string path = "temp1_input"; // type == "temp", nr == 1, item == "input"
    ...
    std::ofstream f0("temp1_lcrit"); f0 << "0\n"; f0.close();
    std::ofstream f1("temp1_min");   f1 << "1\n"; f1.close();
    std::ofstream f2("temp1_max");   f2 << "2\n"; f2.close();
    std::ofstream f3("temp1_crit");  f3 << "3\n"; f3.close();
