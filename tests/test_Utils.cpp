...
TEST_F(TestUtils, getI2cBusAddr_path_invalid)
{
    size_t bus, addr;
    std::string devName;
...
TEST_F(TestUtils, getI2cBusAddr_path_valid_bus_invalid)
{
    size_t bus, addr;
...
TEST_F(TestUtils, getI2cBusAddr_path_valid_addr_invalid)
{
    size_t bus, addr;
    std::string devName = "12-fanController";

    auto ret = getI2cBusAddr(devName, &bus, &addr);
    EXPECT_FALSE(ret);
}

TEST_F(TestUtils, getI2cBusAddr_success)
{
    size_t bus, addr;
