...
static constexpr const char* cableIfc =
...
using DBusVariant =
    std::variant<int, std::string, std::vector<std::string>, bool>;
...
class GPIOPresence
{
  ...
  private:
    ...
    void addInputLine(std::string_view line_label);

    int readLine(std::string_view line_label);
