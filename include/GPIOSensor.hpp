...
class GPIOSensorConfigListener
{
  ...
  private:
    ...
    // @param[in] properties: dbus properties
    Config getConfig(DBusProperties& properties)
    {
      auto name = getDbusProperty<std::string>(properties, Properties::propertyName);
      auto gpioLine = getDbusProperty<std::string>(properties, Properties::propertyGpioLine);
      auto polarity = getDbusProperty<std::string>(properties, Properties::propertyPolarity);
      auto faultLedGroup = getDbusProperty<std::vector<std::string>>(
          properties, Properties::propertyFaultLedGroup);
      return {name, gpioLine, polarity=="active_low", /*functional*/ false, faultLedGroup};
    }
