...
class IpmbSDRDevice : public std::enable_shared_from_this<IpmbSDRDevice>
{
  public:
    ...
    void checkSDRData(std::vector<uint8_t>& data, uint8_t dataLength) const;

    void checkSDRThreshold(std::vector<uint8_t>& sdrDataBytes, int busIndex,
                           std::string tempName) const;
