...
ADCSensor::ADCSensor(const std::string& path,
                     ...
                     const double scaleFactor, const double offsetFactor,
                     unsigned int interval, PowerState readState,
    ...
    scaleFactor(scaleFactor), offsetFactor(offsetFactor),
    sensorPollMs(interval), bridgeGpio(std::move(bridgeGpio)),
...
void ADCSensor::handleResponse(const boost::system::error_code& err)
{
    ...
    if (!err)
    {
        ...
        try
        {
            ...
            double nvalue =
                (rawValue / scaleFactor + offsetFactor) / sensorScaleFactor;
