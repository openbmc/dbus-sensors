...
ADCSensor::ADCSensor(const std::string& path,
                     ...
                     const double scaleFactor, const double offset,
                     float PollRate, PowerState readState,
    ...
    scaleFactor(scaleFactor), offset(offset), sensorPollMs(PollRate * 1000),
...
void ADCSensor::handleResponse(const boost::system::error_code& err)
{
    ...
    if (!err)
    {
        ...
        try
        {
            ...
            double nvalue =
                (rawValue / sensorScaleFactor) / scaleFactor + offset;
