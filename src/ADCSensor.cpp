...
ADCSensor::ADCSensor(const std::string& path,
    ...
    objServer(objectServer), scaleFactor(scaleFactor), bridgeGpio(bridgeGpio),
...
void ADCSensor::setupRead(void)
{
    if (bridgeGpio != 0)
    {
        ...
        // In case a channel has a bridge circuit,we have to turn the bridge on prior
        // to reading a value at least for one scan cycle to get a valid value.
        // Guarantee that the HW signal can be stable, the HW signal could be instability.
        waitTimer.expires_from_now(boost::posix_time::milliseconds(20));
