...
static constexpr const char* dbusObjPathStr =
...
ChassisIntrusionSensor::ChassisIntrusionSensor(
    bool autoRearm, boost::asio::io_context& io, const std::string& confPath,
    sdbusplus::asio::object_server& objServer) :
    mValue(normalValStr), mAutoRearm(autoRearm), mPollTimer(io),
    mObjServer(objServer)
{
    mIface = mObjServer.add_interface(dbusObjPathStr,
                                      "xyz.openbmc_project.Chassis.Intrusion");
    ...
    mIface->initialize();
    mAssocIface =
        mObjServer.add_interface(dbusObjPathStr, association::interface);
    if (mAssocIface)
    {
...
ChassisIntrusionPchSensor::ChassisIntrusionPchSensor(
    ...
    ChassisIntrusionSensor(autoRearm, io, confPath, objServer)
{
    ...
    mGpioInverted(gpioInverted), mGpioFd(io)
{
    ...
    // Initial sensor value update for GPIO type
    // which uses interrupt for polling
    auto value = mGpioLine.get_value();
    if (value >= 0)
    {
        updateValue(value);
