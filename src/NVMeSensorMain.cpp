...
std::vector<std::pair<NVMeSensor, std::shared_ptr<struct NVMeContext>>>
    nvmeDeviceList;
                   ...
                   std::shared_ptr<sdbusplus::asio::connection>& dbusConnection)
{
    ...
    for (auto& path : paths)
    {
        ...
        NVMeSensor sensor =
            NVMeSensor(objectServer, io, dbusConnection, sensorName,
                       std::move(sensorThresholds), *interfacePath);

        nvmeDeviceList.emplace_back(
            std::make_pair<NVMeSensor, std::shared_ptr<struct NVMeContext>>(
                std::move(sensor),
