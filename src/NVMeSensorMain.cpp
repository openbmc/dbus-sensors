...
void printNvmeDeviceList()
{
    for (auto it = nvmeDeviceList.begin(); it != nvmeDeviceList.end(); it++)
    {
        std::cout << "-------------------------------"
                  ...
                  << "\nIn_fd: " << mctp_smbus_get_in_fd(it->second->smbus)
                  << "\n";
    }
}
                     ...
                     boost::asio::deadline_timer& mtcpResponseTimer)
{
    ...
    scanTimer.async_wait([&io, &scanTimer, &mtcpResponseTimer](
        ...
        pollNVMeDevices(io, scanTimer, mtcpResponseTimer);
    });
}
...
int main(int argc, char** argv)
{
    ...
    boost::asio::deadline_timer mtcpResponseTimer(io);
    ...
    io.post([&]() { pollNVMeDevices(io, scanTimer, mtcpResponseTimer); });
