...
boost::container::flat_map<int, int> rootBusFileDescriptors; // bus , fd

std::pair<int, int> lastQueriedDevice; // device was on this rootbus, bus

void createInboundFD(boost::asio::ip::tcp::socket& nvmeSlaveSocket)
{

    for (auto bus : rootBuses)
    {
        int out_fd{0};
        int r = mctp_smbus_open_root_bus(&out_fd, bus);
        rootBusFileDescriptors[bus] = out_fd;
        nvmeSlaveSocket.assign(boost::asio::ip::tcp::v4(), out_fd);
    }

    for (auto& bus : rootBusFileDescriptors)
    {
        std::cerr << "rootbus:" << bus.first << " in_fd=" << bus.second << "\n";
    }
}
    ...
    for (auto& path : paths)
    {
        ...
        nvmeDeviceList[bus] = std::make_pair<SensorInfo, MctpDeviceData>(
            ...
            std::make_shared<struct nvmeContext>(
                io, bus, rootBusFileDescriptors.find(2)->second));
    }
                     ...
                     boost::asio::deadline_timer& tcpResponseTimer)
{
    ...
    scanTimer.async_wait([&io, &scanTimer, &nvmeSlaveSocket, &tcpResponseTimer](
        ...
        pollNVMeDevices(io, scanTimer, nvmeSlaveSocket, tcpResponseTimer);
    });
}
...
int main(int argc, char** argv)
{
    ...
    boost::asio::ip::tcp::socket nvmeSlaveSocket(io);
    boost::asio::deadline_timer tcpResponseTimer(io);

    io.post([&]() {
        createInboundFD(nvmeSlaveSocket);
