...
// a map with key value of {path, NVMeSubsys}
using NVMEMap = std::map<std::string, std::shared_ptr<NVMeSubsys>>;
    ...
    const ManagedObjectType& nvmeConfigurations)
{
    ...
    for (const auto& [interfacePath, configData] : nvmeConfigurations)
    {
        ...
        auto sensorBase = configData.find(NVMeSubsys::configType);
        if (sensorBase != configData.end())
        {
            ...
            if (*nvmeProtocol == "mi_basic")
            {
                ...
                try
                {
                    ...
                    nvmeSubsysMap.emplace(
                        interfacePath,
                        new NVMeSubsys(io, objectServer, dbusConnection,
                                       interfacePath, *sensorName, configData,
                                       nvmeBasic));
                }
    ...
    std::shared_ptr<sdbusplus::asio::connection>& dbusConnection)
{
    ...
    getter->getConfiguration(std::vector<std::string>{NVMeSubsys::configType});
}
                             ...
                             NVMEMap& subsystems)
{
    ...
    auto interface =
        std::find(interfaces.begin(), interfaces.end(), NVMeSubsys::configType);
...
int main()
{
    ...
    sdbusplus::bus::match::match configMatch(
        ...
        "type='signal',member='PropertiesChanged',path_namespace='" +
            ...
            std::string(NVMeSubsys::configType) + "'",
        eventHandler);
