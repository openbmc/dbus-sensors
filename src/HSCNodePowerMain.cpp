...
static constexpr const char* hscType = "xyz.openbmc_project.Configuration.MP5920";
    ...
    HSCType* pType, int* pBusId, int* pSlaveAddr)
{
    ...
    if (!getSensorConfiguration(hscType, dbusConnection, sensorConfigurations, useCache))
    {
        return false;
    }
         ...
         sensorConfigurations)
    {
        ...
        try
        {
            *pBusId = std::get<uint64_t>(
                findBus->second);
            *pSlaveAddr = std::get<uint64_t>(
                findAddress->second);
        }
