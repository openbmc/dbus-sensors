...
// The ExternalSensor is a trivial sensor,
// merely a placeholder, not corresponding to any hwmon file,
// or anything else on the local filesystem.
// It is intended to be settable by something external to the BMC,
// so that the host (or something else external) can write to it,
// perhaps by using an IPMI connection.

// Do not validate configuration against hwmon or any driver,
// instead, blindly accept all configuration from the JSON parameters:
// MinValue, MaxValue, Prefix, Name, PowerState

// The purpose of "Prefix" is to provide additional, optional,
// path components, as required by phosphor-pid-control usage,
// for example: "/xyz/openbmc_project/extsensors/margin/fleeting0"
// In this case, set Prefix="margin" and Name="fleeting0" in JSON.
        ...
        [&io, &objectServer, &sensors, &dbusConnection,
         sensorsChanged](const ManagedObjectType& sensorConfigurations) {
                     ...
                     sensor : sensorConfigurations)
            {
                ...
                auto findPrefix = baseConfigMap->find("Prefix");
                std::string sensorPrefix;
                if (findPrefix != baseConfigMap->end())
                {
                    sensorPrefix = std::get<std::string>(findPrefix->second);
                }
                ...
                sensorEntry = std::make_shared<ExternalSensor>(
                    ...
                    sensorPrefix, std::move(sensorThresholds), *interfacePath,
