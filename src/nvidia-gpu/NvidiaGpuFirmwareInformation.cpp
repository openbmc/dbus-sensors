...
constexpr const char* softwareInventoryPath = "/xyz/openbmc_project/software/";
    ...
    eid(eid), mctpRequester(mctpRequester), objectServer(objectServer)
{
    ...
    {
        lg2::error(
            "Failed to encode Firmware Information request for eid {EID}, rc={RC}",
            "EID", eid, "RC", rc);
    }

    const std::string dbusPath =
        std::format("{}{}_Firmware", softwareInventoryPath, escapeName(name));
