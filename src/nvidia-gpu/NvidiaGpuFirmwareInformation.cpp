    ...
    eid(eid), conn(conn), mctpRequester(mctpRequester)
{
    ...
    {
        lg2::error(
            "Failed to initialize Version interface for Firmware Information for eid {EID}",
            "EID", eid);
    }

    const std::string inventoryPath =
        std::string(inventoryPrefix) + escapeName(name);

    std::vector<Association> associations;
    associations.emplace_back("running", "ran_on", inventoryPath);
    ...
    {
        lg2::error(
            "Failed to initialize Association interface for Firmware Information for eid {EID}",
            "EID", eid);
    }
}
