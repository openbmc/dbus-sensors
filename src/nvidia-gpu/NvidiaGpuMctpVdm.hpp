...
// ECC Error Counts structures and functions
...
{
    uint64_t sram_corrected;
    uint64_t sram_uncorrected_secded;
    uint64_t sram_uncorrected_parity;
    uint64_t dram_corrected;
    uint64_t dram_uncorrected;
} __attribute__((packed));

struct GetEccErrorCountsResponse
{
    ocp::accelerator_management::CommonResponse hdr;
    NsmEccErrorCounts counts;
} __attribute__((packed));
