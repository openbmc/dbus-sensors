...
struct DeviceCapabilities
{
    ...
    // GetSupportedCommandCodes. Codes are stored as raw uint8_t because each
    // message type uses a different command-code enum (see MessageType); the
    // typed supports() overloads below restore type safety, and has() is
    // private so a command cannot be checked against the wrong message type.
    //
    ...
    bool queried{false};
    std::map<MessageType, std::set<uint8_t>> commands;
  ...
  private:
    bool has(MessageType type, uint8_t command) const
    {
        ...
        return it != commands.end() && it->second.contains(command);
    }
};
