...
boost::asio::awaitable<int>
    getGPUEIDs(std::shared_ptr<sdbusplus::asio::connection> dbusConnection,
               mctp::MctpRequester& mctpRequester, std::vector<GpuInfo>& eids)
{
    ...
    for (const auto ocpamiEndpoint : ocpamiEndpoints)
    {
        ...
        rc = ocp_ami_oem_nvidia_decode_query_device_identification_resp(
            reinterpret_cast<const ocp_ami_msg*>(respMsg.data()),
            respMsg.size(), &cc, &reasonCode, &deviceIdentification,
            &instanceId);
        if (rc != 0)
        {
            lg2::error(
