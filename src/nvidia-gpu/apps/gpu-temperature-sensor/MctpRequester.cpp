                               ...
                               const std::vector<uint8_t>& reqMsg)
{
    ...
    auto rc =
        mctp_transport_send_msg(transport, eid, reqMsg.data(), reqMsg.size());
    ...
    try
    {
        ...
        const auto result = co_await (
            socket.async_receive(boost::asio::buffer(respMsg), flag,
    ...
    void* recvMsg = nullptr;
    size_t recvMsgLen = 0;

    rc = mctp_transport_recv_msg(transport, &respEid, &recvMsg, &recvMsgLen);
    ...
    {
        ocp_ami_binding_pci_vid reqHdr{};
        std::memcpy(&reqHdr, reqMsg.data(), sizeof(ocp_ami_binding_pci_vid));

        ocp_ami_binding_pci_vid respHdr{};
        std::memcpy(&respHdr, recvMsg, sizeof(ocp_ami_binding_pci_vid));
    ...
    uint8_t* recvMsgData = static_cast<uint8_t*>(recvMsg);
    respMsg.insert(respMsg.begin(), recvMsgData, recvMsgData + recvMsgLen);
