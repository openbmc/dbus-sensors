    ...
    std::optional<std::pair<uint8_t, uint8_t>> bridgePool)
{
    ...
    // If this endpoint is a bridge, the devices behind it are described by the
    // config's BridgedEndpoints records, one per EID in the bridge's pool.
    std::vector<BridgedEndpoint> bridged;
    if (bridgePool && bridgePool->first != 0 && bridgePool->second != 0)
    {
        uint8_t expectedCount = bridgePool->second - bridgePool->first + 1;

        if (bridgedEndpoints.empty())
        {
    ...
    const ConfigPathHandler& done)
{
    ...
    conn->async_method_call(
        [this, boardName, fallbackPath, eid,
         done](const boost::system::error_code& ec, const GetSubTreeType& ret) {
            processBoardInventoryResult(boardName, fallbackPath, eid, ec, ret,
                                        done);
        },
        "xyz.openbmc_project.ObjectMapper",
        "/xyz/openbmc_project/object_mapper",
...
void DeviceManager::processBoardInventoryResult(
    ...
    const ConfigPathHandler& done)
{
    std::string inventoryPath = fallbackPath;
