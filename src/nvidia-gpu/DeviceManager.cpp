    ...
    switch (static_cast<gpu::DeviceIdentification>(responseDeviceType))
    {
        case gpu::DeviceIdentification::DEVICE_GPU:
        {
            ...
            if (gpu == nullptr)
            {
                ...
                registerEndpoint(
                    endpointPath, eid,
                    std::static_pointer_cast<DeviceInterface>(gpu));
            }
            else
            {
                lg2::info(
        ...
        case gpu::DeviceIdentification::DEVICE_PCIE:
        {
            ...
            if (pcie == nullptr)
            {
                ...
                registerEndpoint(
                    endpointPath, eid,
                    std::static_pointer_cast<DeviceInterface>(pcie));
            }
            else
            {
                lg2::info(
    ...
    const std::shared_ptr<DeviceInterface>& device)
{
    endpoints[endpointPath] =
        EndpointRecord{device, eid, EndpointState::Online};
}
                               ...
                               EndpointEvent event)
{
    ...
    switch (action)
    {
        case EndpointAction::GoOffline:
            lg2::info(
                "MCTP endpoint {PATH} (eid {EID}) degraded; taking device offline",
        ...
        case EndpointAction::GoOnline:
            lg2::info(
                "MCTP endpoint {PATH} (eid {EID}) available; bringing device online",
                "PATH", endpointPath, "EID", rec.eid);
            if (device)
            {
                device->setOnline();
