...
void DeviceManager::start()
{
    ...
    std::function<void(sdbusplus::message_t&)> eventHandler =
        [this](sdbusplus::message_t&) { scheduleRescan(); };
    ...
    mctpConnectivityMatch = std::make_unique<sdbusplus::bus::match_t>(
        bus,
        rules::type::signal() +
        ...
        [this](sdbusplus::message_t& msg) { onConnectivityChanged(msg); });

    // Watch mctpd for endpoint objects being removed (recovery failed within
    // Treclaim / device gone) and re-added (device came back).
    mctpEndpointRemovedMatch = std::make_unique<sdbusplus::bus::match_t>(
        bus,
        rules::interfacesRemoved() +
            rules::argNpath(0, "/au/com/codeconstruct/mctp1/"),
        [this](sdbusplus::message_t& msg) { onEndpointRemoved(msg); });

    mctpEndpointAddedMatch = std::make_unique<sdbusplus::bus::match_t>(
        bus,
        rules::interfacesAdded() +
            rules::argNpath(0, "/au/com/codeconstruct/mctp1/"),
        [this](sdbusplus::message_t& msg) { onEndpointAdded(msg); });
}
