    ...
    std::string_view deviceName, uint8_t portNumber, uint32_t thresholdMask)
{
    ...
    const bool hasUnnamedBits = (thresholdMask & ~knownThresholdMask) != 0;

    // Keep the raw mask so a bit this build cannot name is still visible to
    // the operator, and so a mask that names nothing does not render as an
    // empty "[]". Mask 0x84 gives "symbol_ber; 0x00000084".
    if (hasUnnamedBits || thresholds.empty())
    {
        if (!thresholds.empty())
        {
            thresholds += "; ";
