/*
 * SPDX-FileCopyrightText: Copyright OpenBMC Authors
 * SPDX-License-Identifier: Apache-2.0
 */

#include "DeviceManager.hpp"
#include "MctpRequester.hpp"
#include "NvidiaSensorConfig.hpp"
#include "Utils.hpp"

#include <boost/asio/io_context.hpp>
#include <boost/asio/post.hpp>
#include <phosphor-logging/lg2.hpp>
#include <sdbusplus/asio/connection.hpp>
#include <sdbusplus/asio/object_server.hpp>
#include <sdbusplus/bus.hpp>
#include <sdbusplus/bus/match.hpp>
#include <sdbusplus/message.hpp>

#include <array>
#include <cstdlib>
#include <exception>
#include <functional>
#include <memory>
#include <string>
#include <string_view>

int main()
{
    boost::asio::io_context io;
    auto systemBus = std::make_shared<sdbusplus::asio::connection>(io);
    sdbusplus::asio::object_server objectServer(systemBus, true);
    objectServer.add_manager("/xyz/openbmc_project/sensors");
    objectServer.add_manager("/xyz/openbmc_project/control");
    objectServer.add_manager("/xyz/openbmc_project/inventory");
    objectServer.add_manager("/xyz/openbmc_project/software");
    objectServer.add_manager("/xyz/openbmc_project/metric");
    objectServer.add_manager("/xyz/openbmc_project/network");
    systemBus->request_name("xyz.openbmc_project.GpuSensor");

    mctp::MctpRequester mctpRequester(io);

    DeviceManager deviceManager(io, objectServer, systemBus, mctpRequester);

    boost::asio::post(io,
                      [&deviceManager]() { deviceManager.createSensors(); });

    std::function<void(sdbusplus::message_t&)> eventHandler =
        [&deviceManager](sdbusplus::message_t&) {
            deviceManager.scheduleRescan();
        };

    // arg0namespace matches on dot boundaries, so one match per type is
    // needed: NvidiaMctpVdmSma is not caught by watching NvidiaMctpVdm.
    std::array<std::string_view, 3> sensorTypes(
        {sensorTypeGpu, sensorTypeSma, sensorTypeCx});
    auto configMatches =
        setupPropertiesChangedMatches(*systemBus, sensorTypes, eventHandler);

    // Watch for entity-manager to remove configuration interfaces so the
    // corresponding sensors can be removed.
    auto configIfaceRemovedMatch = std::make_unique<sdbusplus::match>(
        static_cast<sdbusplus::bus_t&>(*systemBus),
        sdbusplus::match_rules::interfacesRemovedAtPath(
            std::string(inventoryPath)),
        [&deviceManager](sdbusplus::message_t& msg) {
            deviceManager.onConfigInterfaceRemoved(msg);
        });

    // Watch mctpd for endpoint Connectivity changes so devices can react.
    auto mctpConnectivityMatch = std::make_unique<sdbusplus::match>(
        static_cast<sdbusplus::bus_t&>(*systemBus),
        sdbusplus::match_rules::type::signal() +
            sdbusplus::match_rules::interface(
                "org.freedesktop.DBus.Properties") +
            sdbusplus::match_rules::member("PropertiesChanged") +
            sdbusplus::match_rules::argN(
                0, "au.com.codeconstruct.MCTP.Endpoint1") +
            sdbusplus::match_rules::path_namespace(
                "/au/com/codeconstruct/mctp1"),
        [&deviceManager](sdbusplus::message_t& msg) {
            deviceManager.onConnectivityChanged(msg);
        });

    // Watch mctpd for endpoint objects being removed (device reset / power
    // cycle) and re-added, so devices can go offline and recover in place.
    auto mctpEndpointRemovedMatch = std::make_unique<sdbusplus::match>(
        static_cast<sdbusplus::bus_t&>(*systemBus),
        sdbusplus::match_rules::interfacesRemoved() +
            sdbusplus::match_rules::argNpath(0, "/au/com/codeconstruct/mctp1/"),
        [&deviceManager](sdbusplus::message_t& msg) {
            deviceManager.onEndpointRemoved(msg);
        });

    auto mctpEndpointAddedMatch = std::make_unique<sdbusplus::match>(
        static_cast<sdbusplus::bus_t&>(*systemBus),
        sdbusplus::match_rules::interfacesAdded() +
            sdbusplus::match_rules::argNpath(0, "/au/com/codeconstruct/mctp1/"),
        [&deviceManager](sdbusplus::message_t& msg) {
            deviceManager.onEndpointAdded(msg);
        });

    try
    {
        io.run();
    }
    catch (const std::exception& e)
    {
        lg2::error("fatal error caught during processing: {MSG}", "MSG",
                   e.what());
        return EXIT_FAILURE;
    }

    return 0;
}
