...
NvidiaSwitchResetControl::NvidiaSwitchResetControl(
    ...
    const sdbusplus::object_path& inventoryPath, uint8_t eid) :
    ...
    name(escapeName(deviceName)), pendingResetValue(noneEnumValue)
{
    ...
    const sdbusplus::object_path resetPath =
        sdbusplus::object_path("/xyz/openbmc_project/control/reset") / name;

    resetInterface = objectServer.add_interface(
        resetPath, "xyz.openbmc_project.Control.Reset");
    ...
    const std::error_code& ec, std::span<const uint8_t> response)
{
    ...
    else
    {
        ...
        {
            lg2::error(
                "Reset rejected: rc={RC}, cc={CC}, reasonCode={RESC}, EID={EID}",
