...
void GpuPowerSensor::processResponse(int sendRecvMsgResult,
                                     std::optional<std::vector<uint8_t>> resp)
{
    ...
    auto rc =
        gpu::decodeGetCurrentPowerDrawResponse(respMsg, cc, reasonCode, power);
...
void GpuPowerSensor::update()
{
    ...
    mctpRequester.sendRecvMsg(
        eid, reqMsg, std::bind_front(&GpuPowerSensor::processResponse, this));
}
