...
//
// Only a device that holds an LLDP mode has an agent to configure, so the
// device is asked before this is constructed and the mode it answered with is
// handed over. Constructing this is therefore what says the device supports
// LLDP, and the object goes on the bus at once.
    ...
    public std::enable_shared_from_this<NvidiaLldpConfiguration>
{
  public:
    NvidiaLldpConfiguration(
        ...
        boost::asio::io_context& io, uint8_t modeData);
