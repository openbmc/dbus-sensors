    ...
    public std::enable_shared_from_this<NvidiaNVLinkPortMetrics>
{
  ...
  private:
    ...
    void createMetrics(uint32_t supportedCounters, size_t numCounters);
    ...
    std::string metricsDbusPathPrefix;

    sdbusplus::object_path portDbusPath;

    // One entry per counter the device actually supports; created lazily on the
    // first telemetry response, once supportedCounters is known.
    struct CreatedMetric
    {
        uint8_t counterIndex;
    ...
    std::vector<CreatedMetric> createdMetrics;
