...
class Memory : public std::enable_shared_from_this<Memory>
{
  ...
  private:
    ...
    std::shared_ptr<std::array<uint8_t, sizeof(gpu::GetCurrentClockFrequencyRequest)>> requestBuffer;
    std::shared_ptr<std::array<uint8_t, sizeof(gpu::GetCurrentClockFrequencyResponse)>> responseBuffer;
