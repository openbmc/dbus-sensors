    ...
    const std::error_code& ec, std::span<const uint8_t> buffer)
{
    ...
    const int rc = gpu::decodeGetPowerLimitsResponse(
        buffer, cc, reasonCode, oneshotPowerLimit, enforcedLimit);
...
void NvidiaGpuPowerControl::applyPendingPowerLimit()
{
    ...
    if (enable)
    {
        const uint32_t capToEnforce =
            (requestedPowerCapWatts != unsetPowerLimit)
                ...
                : powerCapValue;
        if (capToEnforce == unsetPowerLimit)
        {
            // No cap has ever been set and the device reports none, so there
            // is nothing to enforce. Skip rather than send the unset sentinel
    ...
    // else: disabled and only the cap changed; nothing to enforce now, the
    // value is remembered for a later enable.
}
