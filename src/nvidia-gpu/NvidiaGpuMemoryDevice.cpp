    ...
    objectServer(objectServer), dramName(gpuName + "_DRAM_0")
{
    ...
    // === Association: GPU -> DRAM (all_memory) ===
    ...
    // === Association: DRAM -> GPU (parent_processor) ===
    std::vector<Association> dramAssociations;
    dramAssociations.emplace_back("parent_processor", "all_memory", gpuPath);

    dramAssociationInterface =
        objectServer.add_interface(dramPath, association::interface);
    dramAssociationInterface->register_property("Associations",
                                                dramAssociations);
    ...
    lg2::info("Created GPU-DRAM associations: {GPU} <-> {DRAM}", "GPU", gpuPath,
              "DRAM", dramPath);
}
