    ...
    metricsInfo(metricsInfo)
{
    ...
    metricsDbusPathPrefix =
        metricPath + std::format("port_{}_{}", gpuName, portName);

    portDbusPath = inventoryPrefix / gpuName / portName;
}

NvidiaNVLinkPortMetrics::~NvidiaNVLinkPortMetrics()
{
    for (const auto& metric : createdMetrics)
    {
        objectServer.remove_interface(metric.valueInterface);
        objectServer.remove_interface(metric.associationInterface);
    }
}

void NvidiaNVLinkPortMetrics::createMetrics(uint32_t supportedCounters,
                                            size_t numCounters)
{
    createdMetrics.reserve(metricsInfo.size());

    for (const auto& metric : metricsInfo)
    {
        const uint8_t index = metric.counterIndex;

        // Only publish counters the device reports as supported and present.
        ...
        createdMetrics.emplace_back(CreatedMetric{
            index, std::move(valueInterface), std::move(associationInterface)});
    }
}
    ...
    const std::error_code& sendRecvMsgResult, std::span<const uint8_t> response)
{
    ...
    if (!metricsCreated)
    {
        createMetrics(supportedCounters, numCounters);
        metricsCreated = true;
    }

    for (const auto& metric : createdMetrics)
    {
        metric.valueInterface->set_property(
            "Value", static_cast<double>(counters[metric.counterIndex]));
    }
}
