...
static std::vector<std::shared_ptr<sdbusplus::match>> associationMatches;
    ...
    std::span<const uint8_t> queryDeviceIdentificationResponse)
{
    ...
    switch (static_cast<gpu::DeviceIdentification>(responseDeviceType))
    {
        case gpu::DeviceIdentification::DEVICE_GPU:
        {
            ...
            std::shared_ptr<GpuDevice>& gpu = gpuDevices[deviceName];

            if (gpu == nullptr)
            {
                gpu = std::make_shared<GpuDevice>(configs, deviceName,
                                                  configPath, conn, eid, io,
        ...
        case gpu::DeviceIdentification::DEVICE_SMA:
        {
            ...
            std::shared_ptr<SmaDevice>& sma = smaDevices[deviceName];

            if (sma == nullptr)
            {
                sma = std::make_shared<SmaDevice>(configs, deviceName,
                                                  configPath, conn, eid, io,
                                                  mctpRequester, objectServer);
        ...
        case gpu::DeviceIdentification::DEVICE_PCIE:
        {
            ...
            std::shared_ptr<PcieDevice>& pcie = pcieDevices[deviceName];

            if (pcie == nullptr)
            {
                pcie = std::make_shared<PcieDevice>(
                    configs, deviceName, configPath, conn, eid, io,
                    mctpRequester, objectServer);
...
{
    auto match = std::make_shared<sdbusplus::match>(
        static_cast<sdbusplus::bus_t&>(*conn),
        sdbusplus::match_rules::interfacesAdded() +
            sdbusplus::match_rules::argNpath(0, associationPath),
        [&io, &objectServer, &gpuDevices, &smaDevices, &pcieDevices, conn,
