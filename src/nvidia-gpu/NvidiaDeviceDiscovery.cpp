...
DeviceDiscoveryManager::DeviceDiscoveryManager(
    ...
    objectServer(objectServer), configs(configs), path(path)
{
    discoverDevices();
}

void DeviceDiscoveryManager::processQueryDeviceIdResponse(uint8_t eid,
                                                          int sendRecvMsgResult)
{
        ...
        static_cast<uint8_t>(gpu::DeviceIdentification::DEVICE_GPU))
    {
        ...
        gpuDevice.push_back(
            std::make_shared<GpuDevice>(configs, gpuName, path, conn, eid, io,
    ...
    mctp::MctpRequester& mctpRequester, const ManagedObjectType& resp)
{
    for (const auto& [path, interfaces] : resp)
    {
        for (const auto& [intf, cfg] : interfaces)
        {
            ...
            gpuDevice[configs.name] = std::make_shared<DeviceDiscoveryManager>(
                configs, path, dbusConnection, io, mctpRequester, objectServer);
