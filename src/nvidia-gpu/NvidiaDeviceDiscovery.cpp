...
DeviceDiscoveryManager::DeviceDiscoveryManager(
    ...
    io(io), mctpRequester(mctpRequester), conn(conn),
    objectServer(objectServer), name(escapeName(name)), path(path)
{
    discoverDevices();
}

void DeviceDiscoveryManager::processQueryDeviceIdResponse(
    uint8_t eid, int sendRecvMsgResult,
    std::optional<std::vector<uint8_t>> resp)
{
    if (sendRecvMsgResult != 0)
    {
        lg2::error(
            "Error processing MCTP endpoint: sending message over MCTP failed, rc={RC}",
    ...
    if (!resp.has_value())
    {
        lg2::error("Error processing MCTP endpoint: empty response");
        return;
    }
    ...
    auto rc = gpu::decodeQueryDeviceIdentificationResponse(
        respMsg, cc, reasonCode, responseDeviceType, responseInstanceId);

    if (rc != 0 || cc != ocp::accelerator_management::CompletionCode::SUCCESS)
    {
        lg2::error(
            "Error processing MCTP endpoint: decode failed, rc={RC}, cc={CC}, reasonCode={RESC}",
        ...
        static_cast<uint8_t>(gpu::DeviceIdentification::DEVICE_GPU))
    {
        ...
        auto gpuName = name + '_' + std::to_string(responseInstanceId);

        gpuDevice[gpuName] = std::make_shared<GpuDevice>(
            gpuName, path, conn, eid, io, mctpRequester, objectServer);
    }
}

void DeviceDiscoveryManager::queryDeviceIdentification(uint8_t eid)
{
    ...
    if (rc != 0)
    {
        lg2::error("Error processing MCTP endpoint: encode failed, rc={RC}",
                   "RC", rc);
        return;
    }
    ...
    mctpRequester.sendRecvMsg(
        eid, reqMsg,
        std::bind_front(&DeviceDiscoveryManager::processQueryDeviceIdResponse,
                        this, eid));
}

void DeviceDiscoveryManager::queryEndpoints(const boost::system::error_code& ec,
...
void DeviceDiscoveryManager::processEndpoint(
    const boost::system::error_code& ec, const SensorBaseConfigMap& endpoint)
{
                  ...
                  ocp::accelerator_management::messageType) != mctpTypes.end())
    {
        ...
        this->queryDeviceIdentification(eid);
    }
}

void DeviceDiscoveryManager::discoverDevices()
{
    ...
    mctp::MctpRequester& mctpRequester, const ManagedObjectType& resp)
{
    for (const auto& [path, interfaces] : resp)
    {
        for (const auto& [intf, cfg] : interfaces)
        {
            ...
            gpuDevice[name] = std::make_shared<DeviceDiscoveryManager>(
