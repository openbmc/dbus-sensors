    ...
    std::vector<LeakSensorData>& parsedSensors)
{
    ...
    int rc = decodeResponseCommonHeader(
        buffer, MessageType::PLATFORM_ENVIRONMENTAL,
        static_cast<uint8_t>(PlatformEnvironmentalCommands::GET_LEAK_DETECTION_INFO),
        cc, reasonCode);
    ...
    for (uint8_t i = 0; i < numSensors; ++i)
    {
        ...
        {
            uint16_t thresholdMv = 0;
            rc = buffer.unpack(thresholdMv);
            if (rc != 0) return rc;

            sensor.thresholds.push_back(thresholdMv);
        }

        rc = buffer.unpack(sensor.adcReadingMv);
        if (rc != 0) return rc;
