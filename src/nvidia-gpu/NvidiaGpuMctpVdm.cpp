...
int encodeQueryPortsAvailableRequest(uint8_t instanceId, std::span<uint8_t> buf)
{
    PackBuffer buffer(buf);

    const int rc = encodeRequestCommonHeader(
        buffer, MessageType::NETWORK_PORT,
        static_cast<uint8_t>(NetworkPortCommands::QueryPortsAvailable),
        instanceId);
    ...
    uint8_t& numberNvPorts)
{
    ...
    int rc = decodeResponseCommonHeader(
        buffer, MessageType::NETWORK_PORT,
        static_cast<uint8_t>(NetworkPortCommands::QueryPortsAvailable), cc,
        reasonCode);
    ...
    return buffer.unpack(numberNvPorts);
}
    ...
    uint8_t instanceId, uint8_t portNumber, std::span<uint8_t> buf)
{
    ...
    const int rc = encodeRequestCommonHeader(
        buffer, MessageType::NETWORK_PORT,
        static_cast<uint8_t>(NetworkPortCommands::QueryPortCharacteristics),
        instanceId);
    ...
    uint32_t& nvportDataRateKbps, uint32_t& statusLaneInfo)
{
    ...
    int rc = decodeResponseCommonHeader(
        buffer, MessageType::NETWORK_PORT,
        static_cast<uint8_t>(NetworkPortCommands::QueryPortCharacteristics), cc,
        reasonCode);
    ...
    constexpr uint16_t expectedSize = sizeof(uint32_t) * 4;
    if (dataSize != expectedSize)
    {
        return EINVAL;
    }

    buffer.unpack(status);
    buffer.unpack(nvportLineRateMbps);
    buffer.unpack(nvportDataRateKbps);
    buffer.unpack(statusLaneInfo);
                                 ...
                                 std::span<uint8_t> buf)
{
    ...
    const int rc = encodeRequestCommonHeader(
        buffer, MessageType::NETWORK_PORT,
        static_cast<uint8_t>(NetworkPortCommands::QueryPortStatus), instanceId);
    ...
    uint8_t& portState, uint8_t& portStatus)
{
    ...
    int rc = decodeResponseCommonHeader(
        buffer, MessageType::NETWORK_PORT,
        static_cast<uint8_t>(NetworkPortCommands::QueryPortStatus), cc,
        reasonCode);
    ...
    constexpr uint16_t expectedSize = sizeof(uint8_t) * 2;
    if (dataSize != expectedSize)
    {
        return EINVAL;
    }

    buffer.unpack(portState);
...
// The device sends a 32-bit supportedCounters bitmap followed by a dense array
// of up to maxPortTelemetryCounters 64-bit counters (data_size bytes), in the
// fixed order of the port counter struct; bit i of supportedCounters marks
// counter i valid. numCounters returns how many counters were on the wire.
    ...
    std::array<uint64_t, maxPortTelemetryCounters>& counters)
{
    ...
    // The counters are a dense array of 64-bit values, so the bytes after the
    // bitmap must be a whole multiple of a counter; reject a partial trailing
    // counter instead of silently dropping it.
    if ((dataSize - sizeof(supportedCounters)) % sizeof(uint64_t) != 0)
    {
        return EINVAL;
    }
    ...
    // Counter i is carried at index i, so every counter the bitmap claims has
    // to be present. A bit set beyond the counters received means the response
    // is not laid out the way it is decoded here, and reading it would
    // attribute values to the wrong counters; reject it instead.
    if (numCounters < maxPortTelemetryCounters &&
        (supportedCounters >> numCounters) != 0)
    {
        return EINVAL;
    }
