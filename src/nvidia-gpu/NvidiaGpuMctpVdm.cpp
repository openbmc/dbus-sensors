    ...
    GetEccErrorCountsResponse& response)
{
    ...
    response.flags = le16toh(rawResponse->flags);
    response.sram_corrected = le32toh(rawResponse->sram_corrected);
    response.sram_uncorrected_secded =
        le32toh(rawResponse->sram_uncorrected_secded);
    response.sram_uncorrected_parity =
        le32toh(rawResponse->sram_uncorrected_parity);
    response.dram_corrected = le32toh(rawResponse->dram_corrected);
    response.dram_uncorrected = le32toh(rawResponse->dram_uncorrected);
