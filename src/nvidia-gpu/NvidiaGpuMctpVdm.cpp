...
int encodeQueryScalarGroupTelemetryV2Request(
    uint8_t instanceId, uint8_t portNumber, PciePortType portType,
    uint8_t deviceId, uint8_t groupId, std::span<uint8_t> buf)
{
    ...
    msg->hdr.data_size = 3;
    msg->port = (static_cast<uint8_t>(portType) << 7) | (portNumber & 0x7F);
    ...
    size_t& numTelemetryValues, std::vector<uint32_t>& telemetryValues)
{
    ...
    const size_t telemetryDataSize =
        buf.size() - sizeof(ocp::accelerator_management::CommonResponse);

    if (dataSize != telemetryDataSize)
    {
        return EINVAL;
    }

    numTelemetryValues = telemetryDataSize / sizeof(uint32_t);
