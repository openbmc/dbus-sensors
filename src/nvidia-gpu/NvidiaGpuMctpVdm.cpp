...
int encodeGetInventoryInformationRequest(
    const uint8_t instanceId, const uint8_t propertyId, std::span<uint8_t> buf)
{
...
int decodeGetInventoryInformationResponse(
    std::span<const uint8_t> buf,
    ocp::accelerator_management::CompletionCode& cc, uint16_t& reasonCode,
    const InventoryPropertyId propertyId, InventoryInfo& info)
{
    auto rc =
        ocp::accelerator_management::decodeReasonCodeAndCC(buf, cc, reasonCode);
    if (rc != 0 || cc != ocp::accelerator_management::CompletionCode::SUCCESS)
    ...
    switch (propertyId)
    {
        ...
        case InventoryPropertyId::DEVICE_PART_NUMBER:
            info = std::string(
                reinterpret_cast<const char*>(response->data.data()), dataSize);
            break;
        case InventoryPropertyId::DEVICE_GUID:
            info = std::vector<uint8_t>(response->data.data(),
