...
// ============================================================================
// ECC Error Counts Implementation
// ============================================================================
    ...
    NsmEccErrorCounts& errorCounts)
{
    ...
    // Check response size
    if (buf.size() < (sizeof(ocp::accelerator_management::CommonResponse) +
                      sizeof(NsmEccErrorCounts)))
    {
        return EINVAL;
    }

    // Extract error counts structure from response
    const auto* response =
        std::bit_cast<const GetEccErrorCountsResponse*>(buf.data());

    // Copy error counts and convert from little-endian
    errorCounts.flags = le16toh(response->errorCounts.flags);
    errorCounts.sram_corrected = le32toh(response->errorCounts.sram_corrected);
    errorCounts.sram_uncorrected_secded =
        le32toh(response->errorCounts.sram_uncorrected_secded);
    errorCounts.sram_uncorrected_parity =
        le32toh(response->errorCounts.sram_uncorrected_parity);
    errorCounts.dram_corrected = le32toh(response->errorCounts.dram_corrected);
    errorCounts.dram_uncorrected =
        le32toh(response->errorCounts.dram_uncorrected);
