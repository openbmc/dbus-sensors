    ...
    uint8_t& deviceIdentification, uint8_t& deviceInstance)
{
    ...
    const int rc = decodeResponseCommonHeader(
        buffer, MessageType::DEVICE_CAPABILITY_DISCOVERY,
        static_cast<uint8_t>(
            DeviceCapabilityDiscoveryCommands::QUERY_DEVICE_IDENTIFICATION),
        cc, reasonCode);
                                       ...
                                       std::span<uint8_t> buf)
{
    ...
    buffer.pack(static_cast<uint8_t>(sizeof(sensorId)));
...
int decodeGetEccErrorCountsResponse(
    std::span<const uint8_t> buf,
    ocp::accelerator_management::CompletionCode& cc, uint16_t& reasonCode,
    GetEccErrorCountsResponse& response)
{
    auto rc =
        ocp::accelerator_management::decodeReasonCodeAndCC(buf, cc, reasonCode);
    if (rc != 0 || cc != ocp::accelerator_management::CompletionCode::SUCCESS)
    ...
    response.flags = le16toh(raw->flags);
    response.sram_corrected = le32toh(raw->sram_corrected);
    response.sram_uncorrected_secded = le32toh(raw->sram_uncorrected_secded);
    response.sram_uncorrected_parity = le32toh(raw->sram_uncorrected_parity);
    response.dram_corrected = le32toh(raw->dram_corrected);
    response.dram_uncorrected = le32toh(raw->dram_uncorrected);
