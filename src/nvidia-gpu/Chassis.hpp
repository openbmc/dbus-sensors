...
// Inventory.Item.Accelerator interfaces are published.
...
class Chassis
{
  public:
    ...
    // Inventory.Item.Accelerator interfaces on the matched chassis Board
    // path and seed them with any cached UUID / Min / Max values.
    ...
    // Cache MinPowerWatts (nameplate, watts); write through if attached.
    void onMinPower(uint32_t value);

    // Cache MaxPowerWatts (nameplate, watts); write through if attached.
