...
NvidiaEthPortMetrics::NvidiaEthPortMetrics(
    ...
    const std::string& path, uint8_t eid, uint8_t portNumber,
    ...
    mctpRequester(mctpRequester)
{
    ...
    constexpr std::array<std::pair<uint8_t, const char*>, maxTelemetryValues>
        telemetryMetrics = {{
            {0, "RXBytes"},
            {1, "TXBytes"},
    ...
    for (const auto& [tag, metricName] : telemetryMetrics)
    {
        metricValueInterface[tag] = objectServer.add_interface(
            metricsDbusPath + metricName, "xyz.openbmc_project.Metic.Value");
