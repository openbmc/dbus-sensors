    ...
    mctpRequester(mctpRequester)
{
    ...
    portInterface = objectServer.add_interface(
        portDbusPath, "xyz.openbmc_project.Inventory.Item.Port");
    ...
    const std::error_code& sendRecvMsgResult, std::span<const uint8_t> response)
{
    ...
    lg2::debug(
        "Received Ethernet Port Telemetry Counters: EID={EID}, PortNumber={PN}, TelemetryValuesCount={TVC}",
        "EID", eid, "PN", portNumber, "TVC", telemetryValues.size());
    ...
    for (const auto& [tag, value] : telemetryValues)
    {
        ...
        {
            lg2::debug(
                "Updating Ethernet Port Metric: EID={EID}, PortNumber={PN}, Tag={TAG}, Value={VAL}",
                "EID", eid, "PN", portNumber, "TAG", static_cast<uint8_t>(tag),
...
void NvidiaEthPortMetrics::update()
{
    ...
    mctpRequester.sendRecvMsg(
        eid, request,
        std::bind_front(&NvidiaEthPortMetrics::processResponse, this));
}
