        ...
        NvidiaGpuLongRunningCommand::Config{
            ...
            .encodeRequest =
                [](std::span<uint8_t> buf) {
                    return gpu::encodeGetViolationDurationRequest(0, buf);
                },
    ...
    uint8_t eid, UnpackBuffer& buf)
{
    ...
    int rc = buf.unpack(hwViolationDuration);
    if (rc == 0)
    {
        rc = buf.unpack(globalSwViolationDuration);
    }
    if (rc == 0)
    {
        rc = buf.unpack(powerViolationDuration);
    }
    if (rc == 0)
    {
        rc = buf.unpack(thermalViolationDuration);
    }

    if (rc != 0)
    {
        lg2::error("Error updating GPU Violation Duration: failed to unpack "
