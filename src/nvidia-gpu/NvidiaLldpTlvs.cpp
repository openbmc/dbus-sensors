...
std::string toSubtypeString(LldpIdSubtype subtype)
{
...
std::string toCapabilityString(LldpSystemCapability capability)
{
    const std::string prefix{capabilityPrefix};
    switch (capability)
    {
        case LldpSystemCapability::Bridge:
            return prefix + "Bridge";
        case LldpSystemCapability::DOCSISCableDevice:
            return prefix + "DOCSISCableDevice";
        case LldpSystemCapability::Other:
            return prefix + "Other";
        case LldpSystemCapability::Repeater:
            return prefix + "Repeater";
        case LldpSystemCapability::Router:
            return prefix + "Router";
        case LldpSystemCapability::Station:
            return prefix + "Station";
        case LldpSystemCapability::Telephone:
            return prefix + "Telephone";
        case LldpSystemCapability::WLANAccessPoint:
            return prefix + "WLANAccessPoint";
        case LldpSystemCapability::None:
        default:
            return prefix + "None";
    }
}
...
std::vector<std::string> toCapabilityStrings(
    const std::vector<LldpSystemCapability>& capabilities)
{
    std::vector<std::string> names;
    names.reserve(capabilities.size());
    for (const LldpSystemCapability capability : capabilities)
    {
                                     ...
                                     std::span<const uint8_t> buffer)
{
    ...
    if (cc == ocp::accelerator_management::CompletionCode::SUCCESS &&
        !packet.empty())
    {
        if (parseLldpFrame(unitOf(packet), frame) != 0)
        {
            frame = LldpFrame{};
        }
    }
...
void NvidiaLldpTlvs::publish(const LldpFrame& frame)
{
