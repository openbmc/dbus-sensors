                                           ...
                                           std::span<const uint8_t> response)
{
    ...
    for (auto& parsedSensor : parsedSensors)
    {
        ...
        std::string sensorName =
            name + "_LEAKDETECTOR_" + std::to_string(sensorId);

        // Use dynamic thresholds from hardware if exactly 3 are provided
        ...
        leakSensors.emplace_back(newSensor);
    }
    lg2::info("Added SMA Leak Sensors with chassis path: {PATH}.", "PATH",
              path.str);
}
