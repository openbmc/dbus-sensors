...
void SmaDevice::makeSensors()
{
    ...
    auto rc = gpu::encodeGetTemperatureReadingRequest(0, smaTempSensorId,
                                                      *tempReq);
    if (rc != 0)
    {
    ...
    mctpRequester.sendRecvMsg(
        eid, *tempReq,
        [weak{weak_from_this()},
         tempReq](const std::error_code& ec,
            ...
            if (rc2 != 0)
            {
                lg2::error("Error encoding GetLeakDetectionInfoRequest: rc={RC}",
                           "RC", rc2);
                self->read();
                return;
            }
