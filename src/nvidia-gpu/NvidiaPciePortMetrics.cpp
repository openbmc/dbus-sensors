    ...
    conn(conn), mctpRequester(mctpRequester)
{
    ...
    for (const auto& [id, name] : metricsInfo)
    {
        metricValueInterfaces[id] =
            objectServer.add_interface(metricsDbusPath + name, metricInterface);
...
void NvidiaPciePortMetrics::update()
{
    ...
    mctpRequester.sendRecvMsg(
        ...
        [weak{weak_from_this()}](const std::error_code& ec,
            ...
            {
                lg2::error(
                    "Invalid reference to NvidiaPciePortErrors, EID={EID}, PortType={PT}, PortNumber={PN}, ScalarGroup={SG}",
                    "EID", self->eid, "PT",
                    static_cast<uint8_t>(self->portType), "PN",
                    self->portNumber, "SG", self->scalarGroupId);
                return;
            }
