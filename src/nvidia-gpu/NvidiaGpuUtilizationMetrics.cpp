        ...
        longRunningResponseHandler)
{
    ...
    const std::string memoryInventoryPath =
        processorInventoryPath.str + "_DRAM_0";
    ...
    uint8_t eid, UnpackBuffer& buf)
{
    ...
    if (rc != 0)
    {
        lg2::error("Error updating GPU Current Utilization: "
    ...
    buf.unpack(utilization);
    buf.unpack(memoryUtilization);

    if (buf.getError() != 0)
    {
        lg2::error("Error updating GPU Current Utilization: "
                   "payload unpack failed, rc={RC}, EID={EID}",
                   "RC", buf.getError(), "EID", eid);
        return;
    }
    ...
    uint8_t eid, UnpackBuffer& buf)
{
    ...
    if (buf.getError() != 0)
    {
        lg2::error("Error updating GPU Current Utilization: failed to unpack "
