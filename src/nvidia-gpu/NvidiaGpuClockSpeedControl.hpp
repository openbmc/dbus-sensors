    ...
    public std::enable_shared_from_this<NvidiaGpuClockSpeedControl>
{
  ...
  private:
    ...
    // RequestedSpeedLimit*Hz properties. pendingMaxHz / pendingMinHz hold the
    // values requested in the current burst, if any; they are reset once
    // dispatched. PresentSpeedLimit*Hz are read-only and stored directly on the
    // D-Bus interface (updated via set_property), so they need no member here.
