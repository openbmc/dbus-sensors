...
NvidiaPcieInterface::NvidiaPcieInterface(
    ...
    const std::string& path, const std::string& fabricPath, uint8_t eid,
    ...
    eid(eid), path(path), conn(conn), mctpRequester(mctpRequester)
{
    ...
    switchInterface = objectServer.add_interface(
        dbusPath, "xyz.openbmc_project.Inventory.Item.Switch");

    fabricAssociationInterface = objectServer.add_interface(
        dbusPath, "xyz.openbmc_project.Association.Definitions");

    std::vector<Association> associations;
    associations.emplace_back("fabric", "all_switches", fabricPath);

    fabricAssociationInterface->register_property("Associations", associations);
    ...
    if (!fabricAssociationInterface->initialize())
    {
        lg2::error(
            "Error initializing Fabric Association Interface for EID={EID}",
            "EID", eid);
    }
}
...
void NvidiaPcieInterface::update()
{
    ...
    mctpRequester.sendRecvMsg(eid, request,
                              [this](const std::error_code& sendRecvMsgResult,
                                     std::span<const uint8_t> result) {
                                  processResponse(sendRecvMsgResult, result);
                              });
}
