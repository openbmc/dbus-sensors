    ...
    public std::enable_shared_from_this<NvidiaGpuLongRunningCommand>
{
  public:
    struct Config
    {
        ...
        // Invoked when the device replies SUCCESS synchronously. `buf` is an
        // UnpackBuffer that has already been advanced past the common response
        // header (command/cc/reasonCode). The caller continues unpacking from
        // dataSize onward.
        std::move_only_function<void(UnpackBuffer& buf)> onImmediateSuccess;
