    ...
    mctpRequester(mctpRequester), objectServer(objectServer)
{
    ...
    statisticsInterface = objectServer.add_interface(
        dbusPath, "xyz.openbmc_project.Common.Statistics");

    statisticsInterface->register_property(
        "Statistic",
...
void NvidiaGpuPowerPeakReading::processResponse(int sendRecvMsgResult)
{
    ...
    const int rc = gpu::decodeGetCurrentPowerDrawResponse(
        response, cc, reasonCode, peakPower);
    ...
    // is watts.
    statisticsInterface->set_property(
        "Statistic",
        std::unordered_map<std::string, std::tuple<double, uint64_t>>{
...
{
    const int rc = gpu::encodeGetMaxObservedPowerRequest(
