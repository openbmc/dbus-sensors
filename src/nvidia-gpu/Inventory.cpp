    ...
    std::shared_ptr<InventoryResponseBuffer> responseBuffer)
{
    ...
    mctpRequester.sendRecvMsg(
        ...
        [self = weak_from_this(), propertyId, requestBuffer, responseBuffer](int sendRecvMsgResult) {
            if (auto inventory = self.lock())
            {
                inventory->handleInventoryPropertyResponse(propertyId, sendRecvMsgResult,
    ...
    std::shared_ptr<InventoryResponseBuffer> responseBuffer)
{
    ...
    {
        attempts++;
        lg2::error("Failed to update property ID {PROP_ID} (attempt {ATTEMPT}/{MAX})",
                   "PROP_ID", static_cast<uint8_t>(propertyId), "ATTEMPT", attempts, "MAX", maxRetryAttempts);
    ...
    std::shared_ptr<InventoryResponseBuffer> responseBuffer)
{
    ...
    retryTimer.async_wait(
        [self = weak_from_this(), propertyId, requestBuffer, responseBuffer]
        (const boost::system::error_code& ec) {
            ...
            if (auto inventory = self.lock())
            {
                inventory->requestInventoryProperty(propertyId, requestBuffer, responseBuffer);
            }
        });
}
