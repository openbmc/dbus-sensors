...
void Inventory::update()
{
    ...
    requestInventoryProperty(gpu::InventoryPropertyId::DEVICE_PART_NUMBER,
                             "PartNumber", partNumberAttempts);
    requestInventoryProperty(gpu::InventoryPropertyId::SERIAL_NUMBER,
                             "SerialNumber", serialNumberAttempts);
}
                                         ...
                                         int& attempts)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failed to encode {PROP} request: rc={RC}", "PROP",
                   dbusProperty, "RC", rc);
        return;
    }
    mctpRequester.sendRecvMsg(
        eid, requestBuffer, responseBuffer,
        [this, propertyId, dbusProperty,
         &attempts](int sendRecvMsgResult) mutable {
            handleInventoryPropertyResponse(propertyId, dbusProperty, attempts,
                                            sendRecvMsgResult);
        });
}

void Inventory::handleInventoryPropertyResponse(
    gpu::InventoryPropertyId propertyId, const std::string& dbusProperty,
    int& attempts, int sendRecvMsgResult)
{
    ...
    {
        attempts++;
        lg2::error("Failed to update {PROP} (attempt {ATTEMPT}/{MAX})", "PROP",
                   dbusProperty, "ATTEMPT", attempts, "MAX", maxAttempts);
