    ...
    portNumber(portNumber), path(path), conn(conn), mctpRequester(mctpRequester)
{
    ...
    pciePortInterface->register_property("Speed", static_cast<double>(0));

    pciePortInterface->register_property("Lanes",
    ...
    const std::error_code& sendRecvMsgResult, std::span<const uint8_t> response)
{
    ...
    pciePortInterface->set_property(
        "Speed", mapPcieGenToLinkSpeedGbps(telemetryValues[0]));

    pciePortInterface->set_property(
        "Lanes", NvidiaPcieInterface::decodeLinkWidth(
