    ...
    eid(eid), retryHandler(io)
{
    ...
    dimmInterface->register_property("MemoryType", std::string("HBM"));
...
void Memory::requestMemorySpeed()
{
    ...
    mctpRequester.sendRecvMsg(
        eid, *requestBuffer, *responseBuffer, [this](int sendRecvMsgResult) {
            this->handleMemorySpeedResponse(sendRecvMsgResult);
        });
}
...
void Memory::handleMemorySpeedResponse(int sendRecvMsgResult)
{
    if (sendRecvMsgResult != 0)
    {
        lg2::error("Failed to get memory speed: rc={RC}", "RC",
                   sendRecvMsgResult);
        retryHandler.handleRetry(false, "Memory Speed", [this]() {
            this->requestMemorySpeed();
        });
        return;
    }
