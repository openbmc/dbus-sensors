...
static constexpr const char* dimmIfaceName = "xyz.openbmc_project.Inventory.Item.Dimm";
static constexpr const char* associationIfaceName = "xyz.openbmc_project.Association.Definitions";
...
Memory::Memory(const std::shared_ptr<sdbusplus::asio::connection>& /*conn*/,
    ...
    path(std::string(inventoryPrefix) + name),
    mctpRequester(mctpRequester), eid(eid), retryTimer(io)
{
    requestBuffer = std::make_shared<std::array<uint8_t, sizeof(gpu::GetCurrentClockFrequencyRequest)>>();
    responseBuffer = std::make_shared<std::array<uint8_t, sizeof(gpu::GetCurrentClockFrequencyResponse)>>();
...
void Memory::handleMemorySpeedResponse(int sendRecvMsgResult)
{
    ...
    {
        lg2::error("Failed to get memory speed: rc={RC}", "RC", sendRecvMsgResult);
        retryCount++;
