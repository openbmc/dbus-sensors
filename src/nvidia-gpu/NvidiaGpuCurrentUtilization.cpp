    ...
    longRunningResponseHandler(std::move(longRunningResponseHandler))
{
    ...
    {
        lg2::error(
            "Failed to initialize Current Utilization metric interface for GPU {NAME}, eid={EID}",
    ...
    {
        lg2::error(
            "Failed to initialize Current Utilization metric association interface for GPU {NAME}, eid={EID}",
            "NAME", deviceName, "EID", eid);
    }
}
