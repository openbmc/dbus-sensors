    ...
    mctpRequester(mctpRequester), eid(eid)
{
    ...
    clearPowerCapInterface = objectServer.add_interface(
        powerControlPath, "com.nvidia.Common.ClearPowerCap");

    clearPowerCapInterface->register_property("DefaultPowerCap",
                                              defaultPowerCapValue);

    clearPowerCapInterface->initialize();
...
NvidiaGpuControl::~NvidiaGpuControl()
{
    ...
    objectServer.remove_interface(clearPowerCapInterface);
    ...
    const std::error_code& ec, std::span<const uint8_t> buffer)
{
    ...
    if (std::holds_alternative<uint32_t>(value))
    {
        ...
        clearPowerCapInterface->set_property("DefaultPowerCap",
                                             defaultPowerCapValue);
    }
