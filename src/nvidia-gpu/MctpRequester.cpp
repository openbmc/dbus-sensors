    ...
    std::span<uint8_t> respMsg, const std::function<void(int)>& callback)
{
    ...
    // auto respMsg = std::make_shared<std::array<uint8_t, maxMessageSize>>();
    auto ep =
    ...
    mctpSocket.async_receive_from(
        ...
        [eid, reqMsg, callback, timer, ep,
         respMsg](const boost::system::error_code& ec, size_t length) {
            ...
            processRecvMsg(eid, reqMsg, respMsg, *ep, callback);
        });
}
    ...
    std::span<uint8_t> respMsg, const std::function<void(int)>& callback)
{
    ...
    mctpSocket.async_send_to(
        ...
        [this, eid, reqMsg, respMsg,
         callback](const boost::system::error_code& ec, size_t) {
            if (ec)
            {
                lg2::error(
            ...
            handleSendMsgCompletion(eid, reqMsg, respMsg, callback);
        });
}
} // namespace mctp
