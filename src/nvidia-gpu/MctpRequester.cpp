...
static std::optional<const ocp::accelerator_management::BindingPciVid*>
    getHeaderFromBuffer(std::span<const uint8_t> buffer)
{
...
{
    return getHeaderFromBuffer(buffer).and_then(
        [](const auto* header) -> std::optional<uint8_t> {
...
{
    return getHeaderFromBuffer(buffer).and_then(
        [](const auto* header) -> std::optional<bool> {
            return (header->instance_id &
                    ocp::accelerator_management::requestBitMask) != 0;
        });
}
                                   ...
                                   const size_t length)
{
    ...
    // we always keep receive hot, no matter what happens
    bindReceive();
    ...
    std::optional<bool> is_rq = getRequestBit(responseBuffer);
    if (!optionalIid || !is_rq)
    {
        ...
        callback)
{
    ...
    if (inserted)
    {
        lg2::debug("Inserted new queue for eid {EID}", "EID", eid);
    }
