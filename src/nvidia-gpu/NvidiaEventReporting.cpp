...
void NvidiaEventReportingConfig::init(
    const gpu::DeviceSupportedCommandCodes& supportedCommands)
{
    ...
    eventSourcesSupported = supportedCommands.supports(
                                     ...
                                     std::span<const uint8_t> buffer)
{
    ...
    const int rc = ocp::accelerator_management::decodeEvent(
        buffer, gpu::nvidiaPciVendorId, messageType, eventInfo.ackRequired,
        eventInfo.version, eventId, eventInfo.eventClass, eventInfo.eventState,
        eventSize, eventData);
