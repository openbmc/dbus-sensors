...
void NvidiaSmaLeakSensor::updateState(uint8_t value)
{
    ...
    if (newState != lastLeakState)
    {
        ...
        {
            using LeakDetectedCritical = sdbusplus::error::xyz::openbmc_project::state::leak::Detector::LeakDetectedCritical;
            lg2::commit(LeakDetectedCritical("DETECTOR_NAME", detectorPath));
        }
        else
        {
            using LeakDetectedNormal = sdbusplus::event::xyz::openbmc_project::state::leak::Detector::LeakDetectedNormal;
    ...
    if (newFault != lastLeakFault)
    {
        ...
        std::string faultPath =
            "/xyz/openbmc_project/state/leak/detector/" + escapeName(name) + "_Fault";
