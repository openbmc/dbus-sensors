...
void readThermalParameter(uint8_t eid, uint8_t id,
                          ...
                          const std::function<void(uint8_t, int32_t)>& callback)
{
    std::array<uint8_t, sizeof(gpu::ReadThermalParametersRequest)> reqMsg{};

    static std::array<uint8_t, sizeof(gpu::ReadThermalParametersResponse)>
        respMsg{};
    ...
    mctpRequester.sendRecvMsg(
        eid, reqMsg, respMsg,
        std::bind_front(processReadThermalParameterResponse, callback,
                        respMsg));
}
