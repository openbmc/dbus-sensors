...
void processlerReadThermalParameterResponse(
    const std::function<void(uint8_t, int32_t)>& callback,
    int sendRecvMsgResult, std::optional<std::vector<uint8_t>> resp)
{
                          ...
                          const std::function<void(uint8_t, int32_t)>& callback)
{
    ...
    mctpRequester.sendRecvMsg(
        eid, reqMsg,
        std::bind_front(processlerReadThermalParameterResponse, callback));
}
