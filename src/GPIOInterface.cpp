...
GPIOInterface::GPIOInterface(sdbusplus::async::context& ctx,
                             ...
                             UpdateStateCallback_t updateGPIOStateCallback) :
    ctx(ctx), pinName(pinName), updateGPIOStateCallback(updateGPIOStateCallback)
{
...
sdbusplus::async::task<> GPIOInterface::start()
{
    ...
    co_return co_await readGPIOAsync();
}
...
sdbusplus::async::task<> GPIOInterface::readGPIOAsync()
{
    ...
    if (updateGPIOStateCallback)
    {
        co_await updateGPIOStateCallback(
