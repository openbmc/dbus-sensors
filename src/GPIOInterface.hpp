...
class GPIOInterface
{
  public:
    ...
    GPIOInterface(sdbusplus::async::context& ctx,
                  const std::string consumerName, std::string pinName,
                  bool activeLow) :
    ...
    auto configureGPIO() -> void
    {
        ...
        {
            throw std::runtime_error(
                "Failed to request and read GPIO line for " + pinName +
                " with error " + e.what());
        }
  ...
  private:
    ...
    /** @brief GPIO consumer name */
    std::string consumerName;
    /** @brief GPIO pin name */
    std::string pinName;
    /* @brief Flag to indicate if the pin polarity is active low */
