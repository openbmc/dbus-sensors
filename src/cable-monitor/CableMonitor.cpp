...
void Monitor::inventoryAddedHandler(
    const sdbusplus::message::object_path& objectPath,
    const std::string& /*unused*/)
{
    debug("Received cable added for {NAME}", "NAME", objectPath.filename());
    ctx.spawn(processCableAddedAsync(objectPath));
}

void Monitor::inventoryRemovedHandler(
    const sdbusplus::message::object_path& objectPath,
    const std::string& /*unused*/)
{
    debug("Received cable removed for {NAME}", "NAME", objectPath.filename());
    ctx.spawn(processCableRemovedAsync(objectPath));
}

sdbusplus::async::task<> Monitor::configUpdateHandler(
...
void Monitor::reconcileCableData()
{
