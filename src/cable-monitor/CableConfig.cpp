...
auto Config::getExpectedCableConfig() -> sdbusplus::async::task<Cables>
{
    ...
    while (offset < bytes)
    {
        ...
        std::memcpy(&mask,
                    ...
                    sizeof(mask));
        std::memcpy(&len, buffer.data() + offset + offsetof(inotify_event, len),
                    sizeof(len));
        ...
        {
            if (strcmp(name.data(), configFileName) == 0)
            {
                auto configFile =
                    std::string(configFileDir) + "/" + configFileName;
                co_return co_await Config::processConfig(configFile);
            }
        }
