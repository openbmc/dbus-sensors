    ...
    const double& offset)
{
    ...
    const std::map<std::string, decltype(averageTypeThresholds)> map = {
        {"average", std::move(averageTypeThresholds)},
        {"input", std::move(inputTypeThresholds)},
    };
