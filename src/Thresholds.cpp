                           ...
                           thresholds::Direction direction)
{
    ...
    for (auto& threshold : sensor->thresholds)
    {
        if (threshold.level == level && threshold.direction == direction)
        {
            ...
            {
                auto logEntry = std::string(*threshold.assertedLog);
                const auto* logService = "xyz.openbmc_project.Logging";
                const auto* logEntryIntf = "xyz.openbmc_project.Logging.Entry";
                sdbusplus::asio::setProperty(
                    *sensor->dbusConnection, logService, logEntry, logEntryIntf,
                    "Resolved", true,
                    [&logEntry](const boost::system::error_code& ec) {
                        if (ec)
                        {
                            std::cerr << "Error trying to resolve: " << logEntry
                                      << " With RC: " << ec << std::endl;
                        }
                    });
                         ...
                         thresholds::Direction direction)
{
    ...
    using Unit = sdbusplus::common::xyz::openbmc_project::sensor::Value::Unit;
    auto getThreshold =
        [&sensor](thresholds::Level level,
                  thresholds::Direction direction) -> thresholds::Threshold* {
        for (auto& threshold : sensor->thresholds)
        {
            if (threshold.level == level && threshold.direction == direction)
            {
    ...
    {
        lg2::commit(Events::InvalidSensorReading("SENSOR_NAME", objPath));
        return;
    }
    switch (level)
    {
        case Level::WARNING:
            ...
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingAboveUpperWarningThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            else
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingBelowLowerWarningThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            break;
        case Level::PERFORMANCELOSS:
            if (direction == Direction::HIGH)
            {
                threshold->assertedLog = lg2::commit(
                    Events::ReadingAboveUpperPerformanceLossThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            else
            {
                threshold->assertedLog = lg2::commit(
                    Events::ReadingBelowLowerPerformanceLossThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            break;
        case Level::CRITICAL:
            ...
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingAboveUpperCriticalThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            else
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingBelowLowerCriticalThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            break;
        case Level::SOFTSHUTDOWN:
            ...
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingAboveUpperSoftShutdownThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            else
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingBelowLowerSoftShutdownThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            break;
        case Level::HARDSHUTDOWN:
            ...
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingAboveUpperHardShutdownThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            else
            {
                threshold->assertedLog =
                    lg2::commit(Events::ReadingBelowLowerHardShutdownThreshold(
                        "SENSOR_NAME", objPath, "READING_VALUE", assertValue,
                        "UNITS", *unit, "THRESHOLD_VALUE", threshold->value));
            }
            break;
