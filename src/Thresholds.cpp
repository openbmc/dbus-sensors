...
void updateThresholds(Sensor* sensor)
{
    ...
    for (const auto& threshold : sensor->thresholds)
    {
        ...
        if (sensor->findThresIfaceSize(threshold.level))
        {
            continue;
        }

        std::shared_ptr<sdbusplus::asio::dbus_interface>& interface =
            sensor->thresholdInterfaces[static_cast<size_t>(threshold.level)];
        if (!interface)
        {
            continue;
        }
                      ...
                      bool assert)
{
    ...
    if (sensor->findThresIfaceSize(level))
    {
        return;
    }

    std::shared_ptr<sdbusplus::asio::dbus_interface>& interface =
        sensor->thresholdInterfaces[static_cast<size_t>(level)];
    if (!interface)
    {
