...
ThresholdEmit thresholdEmit(const Sensor* sensor)
{
    return sensor->hadValidValue ? ThresholdEmit::onChange
                                 : ThresholdEmit::forced;
}

bool shouldEmitThresholdSignal(bool propertyChanged, ThresholdEmit emit)
{
    return propertyChanged || emit == ThresholdEmit::forced;
}
                                ...
                                double assertValue, ThresholdEmit emit)
{
    ...
    // Keep the emission mode from the evaluation that created this one-shot
    // timer. hadValidValue may change before the delayed callback runs.
    pair->second.async_wait([weakSensor, pair, threshold, assert, assertValue,
                             emit](boost::system::error_code ec) {
...
bool checkThresholds(Sensor* sensor)
{
    ...
    // updateValue() sets hadValidValue after this threshold evaluation, so
    // only the first valid evaluation after reconstruction is forced.
    ThresholdEmit emit = thresholdEmit(sensor);
                               ...
                               ThresholdTimer& thresholdTimer)
{
    ...
    std::vector<ChangeParam> changes = checkThresholds(sensor, sensor->value);
    // Sample the mode once. Delayed events retain this evaluation's mode;
    // subsequent sensor updates use the normal change-based mode.
    ThresholdEmit emit = thresholdEmit(sensor);
    for (const auto& change : changes)
    {
                      ...
                      bool assert, ThresholdEmit emit)
{
    ...
    if (!shouldEmitThresholdSignal(propertyChanged, emit))
    {
        return;
    }
