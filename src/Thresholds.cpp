    ...
    const std::string* sensorPathStr, const SensorBaseConfigMap& cfg)
{
    ...
    auto attrPath = boost::replace_all_copy(
        *sensorPathStr, item, hwmonFileSuffix->second);
    return readFile(attrPath,
                    (1.0 / std::visit(VariantToDoubleVisitor(),
    ...
    const std::string* sensorPathStr, bool allowLabelProbe)
{
    for (const auto& [intf, cfg] : sensorData)
    {
        ...
        auto labelValue = allowLabelProbe ? parseThresholdFromLabel(
                                                 sensorPathStr, cfg) :
                                             std::nullopt;
        ...
        double val = labelValue.has_value() ? labelValue.value() :
...
bool parseThresholdsFromConfig(
    ...
    const std::string* matchLabel, const int* sensorIndex)
{
    return parseThresholdsFromConfigImpl(sensorData, thresholdVector, matchLabel,
                                         sensorIndex, nullptr, false);
}
...
{
    return parseThresholdsFromConfigImpl(sensorData, thresholdVector, matchLabel,
                                         sensorIndex, sensorPathStr, true);
}
