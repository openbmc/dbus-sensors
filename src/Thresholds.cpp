                           ...
                           thresholds::Direction direction)
{
    ...
    for (auto& threshold : sensor->thresholds)
    {
        ...
        {
            if (threshold.level == level && threshold.direction == direction)
            {
    ...
    if (numOutstanding == 0)
    {
        ...
        auto unit =
            sdbusplus::message::convert_from_string<Unit>(sensor->units);
        if (unit)
        {
...
thresholds::Threshold* findThreshold(Sensor* sensor, thresholds::Level level,
                                     thresholds::Direction direction)
{
    for (auto& threshold : sensor->thresholds)
    {
...
auto logWarningAssertThresholds(Direction direction, std::string objPath,
                                double assertValue, Unit unit,
    ...
    -> sdbusplus::message::object_path
{
    ...
    {
        return lg2::commit(Errors::ReadingAboveUpperWarningThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
    else
    {
        return lg2::commit(Errors::ReadingBelowLowerWarningThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
}

auto logPerformanceLossAssertThresholds(Direction direction,
                                        std::string objPath, double assertValue,
    ...
    -> sdbusplus::message::object_path
{
    ...
    if (direction == Direction::HIGH)
    {
        return lg2::commit(Errors::ReadingAboveUpperPerformanceLossThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
    else
    {
        return lg2::commit(Errors::ReadingBelowLowerPerformanceLossThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
}

auto logCriticalAssertThresholds(Direction direction, std::string objPath,
                                 double assertValue, Unit unit,
                                 double thresholdValue)
    -> sdbusplus::message::object_path
{
    ...
    if (direction == Direction::HIGH)
    {
        return lg2::commit(Errors::ReadingAboveUpperCriticalThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
    else
    {
        return lg2::commit(Errors::ReadingBelowLowerCriticalThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
}

auto logSoftShutdownAssertThresholds(Direction direction, std::string objPath,
                                     double assertValue, Unit unit,
                                     double thresholdValue)
    -> sdbusplus::message::object_path
{
    ...
    if (direction == Direction::HIGH)
    {
        return lg2::commit(Errors::ReadingAboveUpperSoftShutdownThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
    else
    {
        return lg2::commit(Errors::ReadingBelowLowerSoftShutdownThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
}

auto logHardShutdownAssertThresholds(Direction direction, std::string objPath,
                                     double assertValue, Unit unit,
                                     double thresholdValue)
    -> sdbusplus::message::object_path
{
    ...
    if (direction == Direction::HIGH)
    {
        return lg2::commit(Errors::ReadingAboveUpperHardShutdownThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
    else
    {
        return lg2::commit(Errors::ReadingBelowLowerHardShutdownThreshold(
            "SENSOR_NAME", objPath, "READING_VALUE", assertValue, "UNITS", unit,
            "THRESHOLD_VALUE", thresholdValue));
    }
}
                         ...
                         thresholds::Direction direction)
{
    namespace Errors =
        sdbusplus::error::xyz::openbmc_project::sensor::Threshold;
    auto objPath = sensor->sensorInterface->get_object_path();
    auto unit = sdbusplus::message::convert_from_string<Unit>(sensor->units);
    auto* threshold = findThreshold(sensor, level, direction);
    if ((threshold == nullptr) || threshold->assertedLog)
    {
        std::cerr << "Unable to find threshold" << std::endl;
        return;
    }
    if (direction == Direction::ERROR || level == Level::ERROR || !unit)
    {
        lg2::commit(Errors::InvalidSensorReading("SENSOR_NAME", objPath));
        return;
    }
    ...
    {
        case Level::WARNING:
            threshold->assertedLog = logWarningAssertThresholds(
                direction, objPath, assertValue, *unit, threshold->value);
            break;
        case Level::PERFORMANCELOSS:
            threshold->assertedLog = logPerformanceLossAssertThresholds(
                direction, objPath, assertValue, *unit, threshold->value);
            break;
        case Level::CRITICAL:
            threshold->assertedLog = logCriticalAssertThresholds(
                direction, objPath, assertValue, *unit, threshold->value);
            break;
        case Level::SOFTSHUTDOWN:
            threshold->assertedLog = logSoftShutdownAssertThresholds(
                direction, objPath, assertValue, *unit, threshold->value);
            break;
        case Level::HARDSHUTDOWN:
            threshold->assertedLog = logHardShutdownAssertThresholds(
                direction, objPath, assertValue, *unit, threshold->value);
            break;
        default:
            break;
