        ...
        if (findClass != baseConfiguration->second.end())
        {
            ...
            if (classString == "Gpio")
            {
                ...
                try
                {
                    ...
                    {
                        lg2::info(
                            "find chassis intrusion sensor polarity inverted flag is {GPIO}",
                            "GPIO", gpioInverted);
                    }
                    return;
                }
            ...
            for (const auto& [path, cfgData] : sensorConfigurations)
            {
                    ...
                    findName != baseConfiguration->second.end())
                {
                    ...
                    if (pEthIndex != nullptr && pName != nullptr)
                    {
                        ...
                        if (debugLanLeash)
                        {
                            lg2::info("find name of eth{INDEX} is {NAME}",
                                      "INDEX", *pEthIndex, "NAME", *pName);
                        }
                    }
                }
            }
...
static void processLanStatusChange(sdbusplus::message_t& message)
{
    ...
    {
        lg2::error("unexpected eth{INDEX} is lanStatusMap", "INDEX", ethNum);
        return;
    }
    ...
    if (!lanInfoMap.empty())
    {
        ...
        {
            lg2::error("unexpected eth{INDEX} is lanInfoMap", "INDEX", ethNum);
        }
        else
        {
    ...
    if (debugLanLeash)
    {
        lg2::info("ethNum = {NUM}, state = {STATE}, oldLanConnected = {OLD}, "
                  "newLanConnected = {NEW}",
                  "NUM", ethNum, "STATE", *pState, "OLD",
                  (oldLanConnected ? "true" : "false"), "NEW",
    ...
    const std::shared_ptr<sdbusplus::asio::connection>& conn)
{
    ...
    for (const std::filesystem::path& fileName : files)
    {
        ...
        if (r.ec != std::errc())
        {
            lg2::error("invalid ethNum string: {NUM}", "NUM", ethNumStr);
            continue;
        }
        ...
        if (debugLanLeash)
        {
            lg2::info("ethNum = {NUM}, ifindex = {LINE}, pathSuffix = {PATH}",
                      "NUM", ethNum, "LINE", line, "PATH", pathSuffix);
        }
        ...
        conn->async_method_call(
            [ethNum](boost::system::error_code ec,
                ...
                if (ec)
                {
                    lg2::error("Error reading init status of eth{NUM}", "NUM",
                               ethNum);
                    return;
                }
                ...
                if (debugLanLeash)
                {
                    lg2::info("ethNum = {NUM}, init LAN status = {STATUS}",
                              "NUM", ethNum, "STATUS",
