...
void ChassisIntrusionSensor::updateValue(const size_t& value)
{
    ...
    if constexpr (debug)
    {
        lg2::info("Update value from {VALUE} to {NEWVALUE}", "VALUE", mValue,
                  "NEWVALUE", newValue);
    }
...
int ChassisIntrusionHwmonSensor::readSensor()
{
    ...
    {
        lg2::error("Error reading status at {PATH}: {ERR}", "PATH", mHwmonPath,
                   "ERR", e);
        return -1;
    }
    ...
    mHwmonName(std::move(hwmonName)), mPollTimer(io)
{
    ...
    {
        lg::info(
            "Found {PATH_SIZE} paths for intrusion status. The first path is: {PATH}",
            "PATH_SIZE", paths.size(), "PATH", mHwmonPath);
    }
}
