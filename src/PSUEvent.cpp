    ...
    paths(paths), eventName(eventName), name(psuName)
{
    ...
    {
        subEvents[index] =
            std::make_unique<PSUSubEvent>(eventInterface, path, objectServer,
                                          conn, io, eventName, ledName, assert);
        index++;
    }
}
...
int PSUSubEvent::triggerLED(const bool& value)
{
    sdbusplus::message::message setLEDObjects = dbusConnection->new_method_call(
        ledGroupManager, ledPath.c_str(), dbusProperties, "Set");
    setLEDObjects.append(ledInterface, "Asserted", std::variant<bool>(value));
    try
    {
        dbusConnection->call(setLEDObjects);
    }
    catch (const sdbusplus::exception::exception&)
    {
        std::cerr << "Error communicating to led group manager\n";
        return -1;
    }
    return 0;
}
...
void PSUSubEvent::updateValue(const int& newValue)
{
    if (newValue == 0)
    {
        ...
        if (*assert == 0)
        {
            ...
            if (triggerLED(true))
            {
                std::cerr << "Failed to clear LED status by " << eventName
                          << "\n";
            }
        }
        if (*assert < 0)
    ...
    if (*assert == 1)
    {
        ...
        if (triggerLED(false))
        {
            std::cerr << "Failed to trigger LED status by " << eventName
                      << "\n";
        }
    }
    value = newValue;
