...
static constexpr double MAX_SIGNED_DUAL_WORD = INT_MAX; // 2147483647
static constexpr double MIN_SIGNED_DUAL_WORD = INT_MIN; // -2147483648
static constexpr double MAX_SIGNED_WORD = SHRT_MAX;     // 32767
static constexpr double MIN_SIGNED_WORD = SHRT_MIN;     // -32768

// Constants for unsigned readings
static constexpr double MAX_UNSIGNED_DUAL_WORD = UINT_MAX; // 4294967295
static constexpr double MAX_UNSIGNED_WORD = USHRT_MAX;     // 65535
static constexpr double MIN_UNSIGNED = 0;

static bool isConnected(int errnum)
{
    switch (errnum)
    {
    ...
    config(std::move(config)), objectServer(objectServer), waitTimer(io)
{
    ...
    switch (config.requestFunctionCode)
    {
        ...
        case (READ_INPUT_REGISTERS):
            ...
            setInitialProperties(sensor_paths::getUnitPath(config.sensorUnit));
            break;
    }
}
...
void ModbusSensor::setSensorValue()
{
    switch (config.requestFunctionCode)
    {
        ...
        case (READ_INPUT_REGISTERS):
            ...
            if (!responseWord.empty())
            {
                if (config.readTwoWords)
                {
                    updateValue(
                        (static_cast<double>(
                            ((static_cast<uint32_t>(responseWord[1])) << 16) |
    ...
    switch (config.requestFunctionCode)
    {
        ...
        case (READ_DISCRETE_INPUTS):
                                       ...
                                       responseHalfword.data()) == -1)
            {
                std::cerr << name << ": Failed to read discrete inputs. "
                          << modbus_strerror(errno) << ", errno: " << errno
                          << std::endl;
                std::fill(responseHalfword.begin(), responseHalfword.end(), 0);

                connected = isConnected(errno);
            }
            break;
        ...
        case (READ_HOLDING_REGISTER):
                ...
                -1)
            {
                ...
                std::fill(responseWord.begin(), responseWord.end(), 0);
...
void ModbusSensor::routine()
{
    ...
    waitTimer.async_wait([this](const boost::system::error_code& ec) {
        ...
        if (connected)
        {
            ...
            std::shared_ptr<ModbusSensor> sharedThis = shared_from_this();
            requestQueue.addRequest(sharedThis);
        }
...
int ModbusSensor::setupModbusConnection()
{
    ...
    {
        std::cerr << name << ": Connection failed. " << modbus_strerror(errno)
...
int ModbusSensor::newModbusConnection()
{
    if (config.protocol == PROTOCOL_RTU)
    {
    ...
    {
        std::cerr << name << ": Failed to set device ID. "
                  << modbus_strerror(errno) << ", errno: " << errno
                  << std::endl;
        return -1;
    }
    ...
    {
        std::cerr << name << ": Failed to set timeout. "
                  << modbus_strerror(errno) << ", errno: " << errno
                  << std::endl;
        return -1;
    }
        ...
        [&io, &objectServer, &dbusConnection, &sensors](
            ...
            for (const auto& [path, interfaces] : resp)
            {
                for (const auto& [intf, cfg] : interfaces)
                {
                    ...
                    config.sensorUnit = loadVariantWithDefault<std::string>(
                        cfg, "Unit", "Unclassified");
