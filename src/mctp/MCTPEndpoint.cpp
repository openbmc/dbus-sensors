    ...
    const SensorBaseConfigMap& iface)
{
    ...
    if (mName == iface.end() || mBus == iface.end() || mPort == iface.end() ||
    ...
    auto bus = std::visit(VariantToStringVisitor(), mBus->second);
    ...
    {
        warning(
            "Failed to create MCTPUSBDevice at [ bus: {USB_BUS}, port: {USB_PORT} ]: {EXCEPTION}",
            "USB_BUS", bus, "USB_PORT", port, "EXCEPTION", ex);
        return {};
    }
}
