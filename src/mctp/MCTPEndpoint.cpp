    ...
    const SensorBaseConfigMap& iface)
{
    ...
    // Map RootHubPosition to actual path. Only the host controllers are
    // counted, since the companion serves their ports at USB 1.1 speeds
    // rather than being a root hub of its own.
    auto rootHubPosition =
        std::visit(VariantToStringVisitor(), mRootHubPosition->second);
    std::string rootHubPath;
    if (rootHubPosition == "0")
    {
        rootHubPath = "/sys/devices/platform/ahb/1e6a1000.usb";
    }
    ...
    try
    {
        ...
        // The root hub position is mapped to a controller address below, so
        // this covers the SOCs those addresses belong to.
        const bool socSupported =
            socFamily.find("AST2600") != std::string::npos ||
