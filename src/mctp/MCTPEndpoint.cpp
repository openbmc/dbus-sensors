                       ...
                       const std::shared_ptr<MCTPEndpoint>& ep)>&& added)
{
    ...
    // Check if the EID already exists on the interface first, Local EID persists across reboots sometimes
    std::string checkCmd =
        std::format("mctp addr show {} | grep -q 'eid {}'", interface, localEID);
    if (std::system(checkCmd.c_str()) == 0)  // NOLINT(cert-env33-c)
    {
        info("Local EID {LOCAL_EID} already exists on interface '{INTERFACE}', skipping add",
    ...
    else
    {
        ...
        {
            error("Failed to add local EID {LOCAL_EID} to interface '{INTERFACE}'",
                  "LOCAL_EID", localEID, "INTERFACE", interface);
            added(std::make_error_code(std::errc::io_error), {});
            return;
        }
    ...
    const SensorBaseConfigMap& iface)
{
    ...
    if (mAddress == iface.end() || mBus == iface.end() || mName == iface.end() || mLocalEID == iface.end())
    {
        throw std::invalid_argument(
    ...
    if (ec != std::errc{})
    {
        throw std::invalid_argument("Bad local EID address");
    }

    try
    {
        return std::make_shared<I2CMCTPDDevice>(connection, bus, address, localEID);
    }
    ...
    const SensorBaseConfigMap& iface)
{
    ...
    try
    {
        return std::make_shared<I3CMCTPDDevice>(connection, bus, address, localEID);
    }
    catch (const MCTPException& ex)
    {
        warning(
