    ...
    sdbusplus::message_t& msg)
{
    ...
    auto entry = std::find(removedIfaces.begin(), removedIfaces.end(),
                       ...
                       const std::shared_ptr<MctpEndpoint>& ep)>&& added)
{
    ...
    catch (const sdbusplus::exception::SdBusError& err)
    {
        if constexpr (debug)
        {
            std::cerr << "Caught exception while configuring endpoint: "
...
SmbusMctpdDevice::SmbusMctpdDevice(
    ...
    uint8_t smdev, std::optional<std::uint8_t> eid) :
...
{
    return std::string("transport: smbus, bus: ")
    ...
    try
    {
        ...
        connection->async_method_call(
            ...
            if (ec)
            {
                ...
                {
                    std::cerr
                        << "Failed to get current connectivity state: " << ec
                        << std::endl;
                }
                return;
            }
    ...
    try
    {
        connection->async_method_call(
            ...
            if (ec)
            {
                if constexpr (debug)
                {
                    std::cerr << "Failed to remove endpoint @ ["
                              << self->describe() << "]: " << ec.message()
                              << std::endl;
                }
                return;
            }
