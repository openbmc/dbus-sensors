...
CPUSensor::CPUSensor(
    ...
    Sensor(boost::replace_all_copy(sensorName, " ", "_"), path,
           ...
           minReading),
    objServer(objectServer), cpuSensors(cpuSensors), cpuId(cpuId),
    inputDev(io, open(path.c_str(), O_RDONLY)), waitTimer(io), show(show),
...
void CPUSensor::handleResponse(const boost::system::error_code& err)
{
    ...
    if (!err)
    {
        ...
        try
        {
            ...
            double gTcontrol = getTcontrol(cpuSensors, cpuId);
