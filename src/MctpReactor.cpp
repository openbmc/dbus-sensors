...
void MctpReactor::trackEndpoint(const std::shared_ptr<MctpEndpoint>& ep)
{
    ...
    // GetAssociatedSubTree ooias /xyz/openbmc_project/mctp/1/9/chassis / 0 1
    // xyz.openbmc_project.Configuration.MCTPDevice a{sa{sas}} 1
    // "/xyz/openbmc_project/inventory/system/nvme/NVMe_1/NVMe_1_Temp" 1
    // "xyz.openbmc_project.EntityManager" 1
    // "xyz.openbmc_project.Configuration.MCTPDevice"
    // ```
                                   ...
                                   const SensorBaseConfigMap& iface)
{
    ...
    std::vector<std::uint8_t> translated = encodeDeviceAddress(transport,
                                                               address);
    if (translated.empty())
    {
        error(
    ...
    auto device = std::make_shared<MctpdDevice>(connection, interface,
                                                translated, eid);
