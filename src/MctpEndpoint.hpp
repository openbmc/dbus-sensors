...
class MctpEndpoint
{
  public:
     ...
     */
    virtual void subscribe(std::function<void(const std::shared_ptr<MctpEndpoint>& ep)>&& degraded,
                           std::function<void(const std::shared_ptr<MctpEndpoint>& ep)>&& available,
                           std::function<void(const std::shared_ptr<MctpEndpoint>& ep)>&& removed) = 0;
     ...
     *         endpoint.
     */
    virtual std::shared_ptr<MctpDevice> device() = 0;
};
    ...
    public std::enable_shared_from_this<MctpdEndpoint>
{
  public:
    ...
    void subscribe(std::function<void(const std::shared_ptr<MctpEndpoint>& ep)>&& degraded,
                   std::function<void(const std::shared_ptr<MctpEndpoint>& ep)>&& available,
                   std::function<void(const std::shared_ptr<MctpEndpoint>& ep)>&& removed) override;
    void setMtu(
        uint32_t mtu,
    ...
    std::shared_ptr<MctpDevice> device() override;
  ...
  private:
    ...
    std::function<void(const std::shared_ptr<MctpEndpoint>& ep)> notifyAvailable;
