...
Config getConfig(const DBusProperties& properties)
{
    ...
    try
    {
        faultLedGroup = getDbusProperty<std::vector<std::string>>(
            properties, Properties::propertyFaultLedGroup);
        pollRate =
            getDbusProperty<int>(properties, Properties::propertyPollRate);
    }
    catch (std::exception&)
    {
        // It is ok for poll rate or faultLedGroup not defined.
    }
    return {name,
            className,
            gpioLine,
