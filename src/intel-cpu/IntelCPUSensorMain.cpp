                   ...
                   ManagedObjectType& sensorConfigs)
{
    ...
    for (const std::filesystem::path& hwmonNamePath : hwmonNamePaths)
    {
        ...
        if (debug)
        {
            lg2::info("Checking: {PATH}: {NAME}", "PATH", hwmonNamePath, "NAME",
                      hwmonName);
        }
        ...
        for (const auto& inputPath : inputPaths)
        {
            ...
            if (findSensor != gCpuSensors.end())
            {
                if (debug)
                {
                    lg2::info("Skipped: {PATH}: {NAME} is already created",
                              "PATH", labelPath, "NAME", sensorName);
                }
                continue;
            }
            ...
            if (debug)
            {
                lg2::info("Mapped: {PATH} to {NAME}", "PATH", inputPath, "NAME",
                          sensorName);
            }
        }
    }
...
bool exportDevice(const CPUConfig& config)
{
    ...
    lg2::info("{PARAMETERS} on bus {BUS} is exported", "PARAMETERS", parameters,
              "BUS", busStr);
               ...
               ManagedObjectType& sensorConfigs)
{
    ...
    for (CPUConfig& config : cpuConfigs)
    {
        ...
        if ((peci_Lock(&peciFd, PECI_NO_WAIT) != PECI_CC_SUCCESS) ||
            (peciFd < 0))
        {
            lg2::error("unable to open {PATH}, {ERRNO}", "PATH", peciDevPath,
        ...
        if (debug)
        {
            lg2::info("{NAME}, state: {STATE}", "NAME", config.name, "STATE",
                      config.state);
        }
        peci_Unlock(peciFd);
    }
    ...
    for (const char* type : sensorTypes)
    {
        for (const auto& [path, cfgData] : sensorConfigs)
        {
            for (const auto& [intf, cfg] : cfgData)
            {
                ...
                {
                    lg2::error("Can't find 'Address' setting in {NAME}", "NAME",
                               name);
                    continue;
                }
...
int main()
{
    ...
    std::function<void(sdbusplus::message_t&)> eventHandler =
        [&](sdbusplus::message_t& message) {
            ...
            if (debug)
            {
                lg2::info("{PATH} os changed", "PATH", message.get_path());
            }
