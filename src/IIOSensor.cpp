...
// is in Pascals.
static constexpr unsigned int pressureScaleFactor = 1000;
static constexpr unsigned int temperatureScaleFactor = 1000;

// the DPS310 can read up to 120000 Pascals
static constexpr double maxReading = 120000;
// Zero Kelvins is -273.15 DegreesC
static constexpr double minReading = -273.15;
...
void IIOSensor::handleResponse(const boost::system::error_code& err)
{
    ...
    if (!err)
    {
        ...
        try
        {
            ...
            double nvalue;
            std::string my_pressure = "in_pressure";
            std::string my_temperature = "in_temp";
            if (path.find(my_pressure) != std::string::npos)
            {
                nvalue = rawValue * pressureScaleFactor;
            }
            else if (path.find(my_temperature) != std::string::npos)
            {
