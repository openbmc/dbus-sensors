...
void IIOSensor::handleResponse(const boost::system::error_code& err)
{
    ...
    if (!err)
    {
        ...
        try
        {
            ...
            std::string myTemperatureRaw = "in_temp_raw";
            std::string myTemperatureInput = "in_temp_input";
            if (path.find(myPressure) != std::string::npos)
            {
                rawValue = std::stod(response);
                nvalue = rawValue * pressureScaleFactor;
            }
            else if (path.find(myTemperatureInput) != std::string::npos)
            {
                rawValue = std::stod(response);
                nvalue = rawValue / temperatureScaleFactor;
            }
            else if (path.find(myTemperatureRaw) != std::string::npos)
            {
                rawValue = std::stod(response);
                // the driver takes the "raw value"
                // and shifts if right by 2
                nvalue = (175.72 * (rawValue * 4.0)) / 65536.0 - 46.85;
            }
