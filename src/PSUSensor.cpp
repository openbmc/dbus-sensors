...
PSUDisposition PSUSensor::prepareInput()
{
    ...
    // This should never be true, because prepareInput()
    // should never be called for sensors in the trash can.
    if (deleteRequested)
    {
        std::cerr << "Sensor anomaly: Trashed sensor prepared input\n";
