                               ...
                               size_t bytesRead)
{
    ...
    {
        log<level::ERR>(std::format("Bad file descriptor for {}", path));
        return;
    }
    if (err || bytesRead == 0)
    {
        if (readingStateGood())
        {
            log<level::ERR>(std::format("{} read failed", name));
        }
        restartRead();
        return;
    }
    ...
    {
        log<level::ERR>(std::format("Could not parse input from {}", path));
        incrementError();
    }
