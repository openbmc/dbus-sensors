...
PSUSensor::PSUSensor(const std::string& path, const std::string& objectType,
                     ...
                     unsigned int interval, const std::string& label,
                     size_t tSize) :
    ...
    sensorFactor(factor), minMaxReadCounter(0), sensorGain(gain),
    sensorOffset(offset), sensorPollMs(interval)
{
    if constexpr (DEBUG)
    {
...
void PSUSensor::handleResponse(const boost::system::error_code& err)
{
    ...
    if (!err)
    {
        ...
        try
        {
            ...
            double nvalue =
                (rawValue * sensorGain + sensorOffset) / sensorFactor;
            updateValue(nvalue);
