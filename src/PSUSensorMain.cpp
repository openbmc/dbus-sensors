    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            sensors[sensorName] = std::make_shared<PSUSensor>(
                ...
                psuProperty->minReading, labelHead, thresholdConfSize, pollRate,
                readThread);
        ...
        combineEvents[*psuName + "OperationalStatus"] =
            std::make_unique<PSUCombineEvent>(
                ...
                groupEventPathList, "OperationalStatus", pollRate, readThread);
    }
    ...
    boost::asio::static_thread_pool& readThread)
{
    auto getter = std::make_shared<GetSensorConfiguration>(
        dbusConnection, [&io, &objectServer, &dbusConnection, sensorsChanged,
                         &readThread](const ManagedObjectType& sensorConfigs) {
...
int main()
{
    ...
    io.post([&]() {
        createSensors(io, objectServer, systemBus, nullptr, readThread);
    });
