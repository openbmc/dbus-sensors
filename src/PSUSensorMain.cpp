...
struct SensorConfigKey {
    ...
    bool operator<(const SensorConfigKey& other) const
    {
        ...
        if (bus == other.bus && addr < other.addr)
        {
            return true;
        }
        return false;
...
static SensorConfigMap buildSensorConfigMap(const ManagedObjectType& sensorConfigs)
{
    SensorConfigMap configSensors;
         ...
         sensorConfigs)
    {
             ...
             sensor.second)
        {
            ...
            SensorConfig val = {sensor.first.str, sensor.second, cfgmap.first,
                                cfg};
            auto [it, inserted] = configSensors.emplace(key, val);
            if (!inserted)
            {
        ...
        sensorsChanged)
{
    ...
    SensorConfigMap configSensors = buildSensorConfigMap(sensorConfigs);
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        auto findSensorCfg = configSensors.find({bus, addr});
        if (findSensorCfg == configSensors.end())
        {
