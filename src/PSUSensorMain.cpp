  ...
  for (const auto &pmbusPath : pmbusPaths) {
    ...
    for (const std::pair<sdbusplus::message::object_path, SensorData> &sensor :
      ...
      if ((*confBus != bus) || (*confAddr != addr)) {
        std::cerr << "Configuration skipping " << *confBus << "-" << *confAddr
                  << " because not " << bus << "-" << addr << "\n";
        continue;
      }
    ...
    if (findPSUName == baseConfig->second.end()) {
      std::cerr << "could not determine configuration name for " << deviceName
                << "\n";
      continue;
    }
    ...
    for (const auto &sensorPath : sensorPaths) {
      ...
      if
        constexpr(DEBUG) {
          std::cerr << "Sensor type=\"" << sensorNameSubStr << "\" label=\""
                    << labelHead << "\"\n";
        }
      ...
      if (!customizedName) {
        ...
        if (psuNames.size() <= nameIndex) {
          std::cerr << "Could not pair " << labelHead << " with a Name field\n";
          continue;
        }
        ...
        if
          constexpr(DEBUG) {
            std::cerr << "Sensor label head " << labelHead << " paired with "
                      << psuNameFromIndex << " at index " << nameIndex << "\n";
          }
      }
