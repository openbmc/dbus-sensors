...
static void finishMasterTimer()
{
    ...
    // If timer lag greater than sensorPollMs, big problem, we are not
    ...
    if (msCollect > sensorPollMs)
    {
        std::cerr << "Sensor anomaly: taking too long to read, " << msCollect
                  << " ms taken beyond " << sensorPollMs << " ms allowed\n";
...
int main()
{
    ...
    masterTimer->expires_at(std::chrono::steady_clock::now());
    rescheduleMasterTimer();
