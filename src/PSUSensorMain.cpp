...
bool getDeviveParameters(const fs::path& directory)
{
    fs::path device = "";
    ...
    if (dirType == "/sys/class/hwmon")
    {
        ...
        devParam.matchRegEx = R"(\w\d+_input$)";
        devParam.nameRegEx = "([A-Za-z]+)[0-9]*_";
    }
    else if (dirType == "/sys/bus/iio/devices")
    {
        ...
        devParam.matchRegEx = R"(\w+_(raw|input)$)";
        devParam.nameRegEx = "^(in|out)_([A-Za-z]+)[0-9]*_";
    }
    else
    {
        return false;
        ...
        sensorsChanged)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        if (!getDeviveParameters(directory))
        {
            continue;
        }
