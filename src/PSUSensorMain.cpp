        ...
        groupEventPathList)
{
    for (const auto& match : groupEventMatch)
    {
        ...
        {
            const std::string& eventName = match.first + "\\d+_";
            const std::vector<std::string>& eventAttrs = match.second;
            for (const auto& eventAttr : eventAttrs)
            {
                std::vector<fs::path> eventPaths;
                if (!findFiles(fs::path(directory), eventName + eventAttr,
                               eventPaths))
                {
                    continue;
                }
                ...
                for (const auto& eventPath : eventPaths)
                {
                    ...
                    pathList[attrName.substr(0, attrName.find('_'))].push_back(
                        eventPath);
                }
            }
        }
                   ...
                   const std::string& psuName)
{
    ...
    std::string labelHeadPrefix = matches[1];
    std::string labelHeadIndex = matches[2];

    for (const auto& [pwmLabel, pwmName] : pwmTable)
    {
        if (pwmLabel != labelHeadPrefix)
        {
            continue;
        }
        ...
        name += psuName;
        name += "_";
        name += pwmName + labelHeadIndex;

        std::string objPath = interfacePath;
        objPath += "_";
        objPath += pwmName + labelHeadIndex;
...
void propertyInitialize(void)
{
    ...
    groupEventMatch = {{"FanFault", {{"fan", {"alarm", "fault"}}}}};
