...
static const std::string hwmonInputRegex = R"(\w\d+_input$)";
static const std::string iioInputRegex = R"(\w+_(raw|input)$)";
static const std::regex hwmonNameRegex("([A-Za-z]+)[0-9]*_");
static const std::regex iioNameRegex("^(in|out)_([A-Za-z]+)[0-9]*_");
static const std::regex initRegex("");

struct DevParams devParam = {.devType = DevTypes::Unknown,
                             .matchIndex = 0,
                             .deviceName = "",
                             .matchRegEx = "",
                             .nameRegEx = initRegex};

bool getDeviceParameters(const fs::path& directory)
{
    fs::path device = "";
    ...
    if (dirType == "/sys/class/hwmon")
    {
        ...
        devParam.devType = DevTypes::PSU;
        devParam.matchIndex = 1;
        devParam.deviceName = fs::canonical(device).stem();
        devParam.matchRegEx = hwmonInputRegex;
        devParam.nameRegEx = hwmonNameRegex;
        return true;
    }
    if (dirType == "/sys/bus/iio/devices")
    {
        device = fs::canonical(directory);
        devParam.devType = DevTypes::IIO;
        devParam.matchIndex = 2;
        devParam.deviceName = device.parent_path().stem();
        devParam.matchRegEx = iioInputRegex;
        devParam.nameRegEx = iioNameRegex;
        return true;
    }
    return false;
}
        ...
        sensorsChanged)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        if (!getDeviceParameters(directory))
        {
            continue;
        }
        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            if (std::regex_search(sensorNameStr, matches, devParam.nameRegEx))
            {
