...
static boost::container::flat_map<DevTypes, std::pair<std::string, std::string>>
    regexMatchs;

struct DevParams devParam = {
    .devType = DevTypes::Unknown, .matchIndex = 0, .deviceName = ""};
...
bool getDeviceParameters(const fs::path& directory)
{
    ...
    if (dirType == "/sys/class/hwmon")
    {
        ...
        devParam.devType = DevTypes::HWMON;
        ...
        sensorsChanged)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        auto regexAttrs = regexMatchs.find(devParam.devType);
        auto regexAttr = regexAttrs->second;
        if (!findFiles(directory, regexAttr.first, sensorPaths, 0))
        {
        ...
        std::regex sensorNameRegEx(regexAttr.second);
        std::smatch matches;
        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            if (devParam.devType == DevTypes::HWMON)
            {
...
void propertyInitialize(void)
{
    ...
    regexMatchs = {{DevTypes::HWMON, {R"(\w\d+_input$)", "([A-Za-z]+)[0-9]*_"}},
                   {DevTypes::IIO,
                    {R"(\w+_(raw|input)$)", "^(in|out)_([A-Za-z]+)[0-9]*_"}}};
}
