        ...
        sensorsChanged)
{
    ...
    using SensorConfigKey = std::pair<uint64_t, uint64_t>;
    using SensorConfig = std::tuple<const std::string*, const SensorData*,
                                    const std::string*, const SensorBaseConfigMap*>;
    // map (bus, addr) to (interface, SensorData, type, config map)
    boost::container::flat_map<SensorConfigKey, SensorConfig> configSensors;
    for (const std::pair<sdbusplus::message::object_path, SensorData>& sensor : sensorConfigs)
    {
        for (const std::pair<std::string, SensorBaseConfigMap>& cfgmap : sensor.second)
        {
            ...
            const uint64_t* bus;
            const uint64_t* addr;
            if (!(bus = std::get_if<uint64_t>(&busCfg->second))
                || !(addr = std::get_if<uint64_t>(&addrCfg->second)))
            {
            ...
            {
                configSensors[key] = {&sensor.first.str, &sensor.second, &cfgmap.first, &cfg};
            }
        }
    }
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        auto [interfacePath, sensorData, sensorType, baseConfig] = configSensors[key];
