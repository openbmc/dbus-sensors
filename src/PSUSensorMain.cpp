...
static boost::container::flat_map<PSULabelKey, PSUProperty> labelMatch;
        ...
        sensorsChanged)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            auto findProperty = labelMatch.find({.label = labelHead, .driver = pmbusName});
            if (findProperty == labelMatch.end())
            {
                findProperty = labelMatch.find({.label = labelHead, .driver = ""});
            }
            if (findProperty == labelMatch.end())
            {
                if constexpr (debug)
