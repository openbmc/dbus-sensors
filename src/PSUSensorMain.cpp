        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            if (devType == DevTypes::HWMON)
            {
                ...
                if (pos != std::string::npos)
                {
                    ...
                    std::string sensorPathStrPeak = sensorPathStr.substr(pos);
                    if (sensorPathStrPeak == "_input_highest" ||
                        sensorPathStrPeak == "_highest")
                    {
                        labelPath = boost::replace_all_copy(
                            sensorPathStr, sensorPathStrPeak.substr(1),
                            "label");
                        peakLabel = true;
                    }
                    else
