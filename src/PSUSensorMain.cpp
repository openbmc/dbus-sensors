...
static boost::container::flat_map<std::string, std::unique_ptr<PSUSensor>>
    sensors;
static boost::container::flat_map<std::string, std::unique_ptr<PSUEvent>>
    events;
...
boost::container::flat_map<std::string, std::vector<std::string>> eventMatch;
                   ...
                   std::shared_ptr<sdbusplus::asio::connection>& dbusConnection)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
                 ...
                 sensor : sensorConfigs)
        {
            ...
            {
                std::cerr
                    << "error finding necessary entry in configurationi\n";
                continue;
            }

            const uint64_t *confBus, *confAddr;
        ...
        for (const auto& pathList : eventPathList)
        {
            ...
            events[eventPSUName] = std::make_unique<PSUEvent>(
                pathList.second, objectServer, io, *psuName, eventName);
        }
    }
    return;
}
