...
static boost::container::flat_map<std::string, std::string> ledTable;
        ...
        limitEventMatch)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        for (iter = eventPathList.begin(); iter != eventPathList.end(); iter++)
        {
            ...
            std::string ledName = "NULL";

            auto findLED = ledTable.find(eventName);
            if (findLED != ledTable.end())
            {
                ledName = findLED->second;
            }

            events[eventPSUName] = std::make_unique<PSUEvent>(
                iter->second, objectServer, dbusConnection, io, psuName,
                eventName, ledName);
        }
    }
    return;
        ...
        limitEventMatch)
{
    ...
    ledTable = {{"Predictive", "status_degraded"},
                {"Failure", "status_degraded"},
                {"ACLost", "status_degraded"},
                {"FanFault", "status_non_critical"}};
}
