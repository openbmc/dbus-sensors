...
static
  ...
  "max20796", "max34451", "pmbus", "pxe1610","adm1278"};
...
void
checkEvent (const std::string & directory,
	    const boost::container::flat_map < std::string,
	    std::vector < std::string >> &eventMatch,
	    boost::container::flat_map < std::string,
	    std::vector < std::string >> &eventPathList)
{
for (const auto & match:eventMatch)
	...
	std::vector <
      std::string > &
	eventAttrs = match.second;
      const
	std::string &
	eventName = match.first;
    for (const auto & eventAttr:eventAttrs)
	{
	  auto
	    eventPath = directory + "/" + eventAttr;
...
// events.
void
checkGroupEvent (const std::string & directory,
		 const boost::container::flat_map <
		 std::string,
		 boost::container::flat_map < std::string,
		 std::vector < std::string >>> &groupEventMatch,
		 boost::container::flat_map < std::string,
		 boost::container::flat_map < std::string,
		 std::vector < std::string >>> &groupEventPathList)
{
for (const auto & match:groupEventMatch)
    {
      const
	std::string &
	groupEventName = match.first;
      const
	boost::container::flat_map <
	std::string,
	std::vector <
...
// to eventPathList.
void
checkEventLimits (const std::string & sensorPathStr,
		  const boost::container::flat_map < std::string,
		  std::vector < std::string >> &limitEventMatch,
		  boost::container::flat_map < std::string,
		  std::vector < std::string >> &eventPathList)
{
for (const auto & limitMatch:limitEventMatch)
	...
	std::vector <
      std::string > &
	limitEventAttrs = limitMatch.second;
      const
	std::string &
	eventName = limitMatch.first;
...
for (const auto & pwmName:pwmTable)
      ...
      if (pwmName.first != labelHead)
	{
      ...
      const
	std::string &
	sensorPathStr = sensorPath.string ();
      const
	std::string &
	pwmPathStr =
      ...
      if (!pwmFile.good ())
	{
      ...
      auto
	findPWMSensor = pwmSensors.find (psuName + labelHead);
      if (findPWMSensor != pwmSensors.end ())
	{
	  continue;
	}

      pwmSensors[psuName + labelHead] =
	std::make_unique < PwmSensor >
...
void
createSensors (boost::asio::io_service & io,
	       sdbusplus::asio::object_server & objectServer,
	       std::shared_ptr < sdbusplus::asio::connection >
	       &dbusConnection)
{
...
for (const auto & pmbusPath:pmbusPaths)
	...
	psuName;
      auto
	directory = pmbusPath.parent_path ();

      auto
	ret = directories.insert (directory.string ());
      if (!ret.second)
	{
    ...
    for (const std::pair < sdbusplus::message::object_path, SensorData > &sensor:sensorConfigs)
	{
	  sensorData = &(sensor.second);
	for (const char *type:sensorTypes)
	    {
	      auto
		sensorBase = sensorData->find (type);
	  ...
	  auto
	    configBus = baseConfig->second.find ("Bus");
	  auto
	    configAddress = baseConfig->second.find ("Address");
      ...
      if (interfacePath == nullptr)
	{
      ...
      auto
	findPSUName = baseConfig->second.find ("Name");
      ...
      if (!findFiles (directory, R "(\w\d+_input$)", sensorPaths, 0))
	{
      ...
      auto
	findLabelObj = baseConfig->second.find ("Labels");
    ...
    for (const auto & sensorPath:sensorPaths)
	{
	  ...
	  auto
	    psuProperty = psuProperties.rbegin ();
	  ...
	  auto
	    findCustomName = baseConfig->second.find (keyName);
	  ...
	  bool
	    customizedScale = false;
	  auto
	    findCustomScale = baseConfig->second.find (keyScale);
	  if (findCustomScale != baseConfig->second.end ())
	    {
	      try
	      {
		psuProperty->sensorScaleFactor =
	  ...
	  auto
	    findCustomMin = baseConfig->second.find (keyMin);
	  ...
	  auto
	    findCustomMax = baseConfig->second.find (keyMax);
	  ...
	  unsigned int
	    factor = psuProperty->sensorScaleFactor;
	  if (!customizedScale)
	    {
	      ...
	      char
		firstChar = sensorNameSubStr[0] - 0x20;
		...
		firstChar + sensorNameSubStr.substr (1) + "ScaleFactor";
	      ...
	      auto
		findScaleFactor = baseConfig->second.find (strScaleFactor);
	  ...
	  auto
	    findSensorType = sensorTable.find (sensorNameSubStr);
	  ...
	  sensors[sensorName] =
	    std::make_shared < PSUSensor > (sensorPathStr, sensorType,
					    objectServer, dbusConnection, io,
					    sensorName,
					    std::move (sensorThresholds),
					    *interfacePath,
					    findSensorType->second, factor,
					    psuProperty->maxReading,
					    psuProperty->minReading,
	...
	std::make_unique < PSUCombineEvent > (objectServer, dbusConnection,
					      io, *psuName, eventPathList,
					      groupEventPathList,
					      "OperationalStatus");
    }
...
propertyInitialize (void)
{
  ...
  {
    {
    "pin", PSUProperty ("Input Power", 3000, 0, 6)},
    {
  ...
  groupEventMatch =
  {
      ...
      {
	{
	  "fan1",
	  {
	"fan1_alarm", "fan1_fault"}},
	{
	  "fan2",
	  {
  "fan2_alarm", "fan2_fault"}}}}};
}
...
for (const char *type:sensorTypes)
    {
      auto
	match =
	std::make_unique < sdbusplus::bus::match::match > (static_cast <
							   sdbusplus::bus::
							   bus &
							   >(*systemBus),
