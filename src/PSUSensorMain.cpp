        ...
        sensors)
{
    ...
    for (auto& pmbusPath : pmbusPaths)
    {
        ...
        auto findSensorName = baseConfig->second.find("Name1");
        ...
        for (const auto& powerPath : powerPaths)
        {
            ...
            std::string sensorName =
                std::get<std::string>(findSensorName->second) + label;
...
int main(int argc, char** argv)
{
    ...
    io.post([&]() { createSensors(io, objectServer, systemBus, sensors); });
    ...
    std::function<void(sdbusplus::message::message&)> eventHandler =
        [&](sdbusplus::message::message& message) {
            ...
            filterTimer.async_wait([&](const boost::system::error_code& ec) {
                ...
                createSensors(io, objectServer, systemBus, sensors);
            });
        };
