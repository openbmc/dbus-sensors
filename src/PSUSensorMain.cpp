...
// Function CheckEvent will check the attribute from eventMatch table in the
// sysfs. If the attribute are exist in sysfs, then store the complete path
...
// Function checkLimitEvent will check all the psu related xxx_input attribute
// in sysfs to see if xxx_crit_alarm xxx_lcrit_alarm xxx_max_alarm
// xxx_min_alarm are exist, then store the exist path of the alarm attribute
// to eventPathList.
        ...
        limitEventMatch)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
                 ...
                 sensor : sensorConfigs)
        {
            ...
            const uint64_t* conf;
            if (!(conf = std::get_if<uint64_t>(&(configBus->second))) &&
                *conf != bus)
            {
                continue;
            }
            if (!(conf = std::get_if<uint64_t>(&(configAddress->second))) &&
                *conf != addr)
            {
                continue;
            }
        ...
        limitEventMatch)
{
    ...
    limitEventMatch = {{"Predictive", {"max_alarm", "min_alarm"}},
