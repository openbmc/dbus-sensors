...
static void timerCallback(std::weak_ptr<RedfishServer> that,
...
void RedfishServer::advanceDiscovery()
{
    ...
    if constexpr (debug)
    {
        double average = msDiscoveryTime / completionsDiscovery;
...
void RedfishServer::advanceReading()
{
    ...
    if constexpr (debug)
    {
        double average = msReadingTime / completionsReading;
        auto msAverage = static_cast<int>(average);
