...
static void
    createSensorsFromConfig(boost::asio::io_context& io,
...
boost::container::flat_map<std::string,
    ...
    assocInterfaces;
static std::unique_ptr<sdbusplus::bus::match_t> lanStatusMatch;
static std::unique_ptr<sdbusplus::bus::match_t> lanConfigMatch;
...
static void generateAssociations(sdbusplus::asio::object_server& objServer)
{
    ...
    systemBus->async_method_call(
        ...
        if (ec)
        {
            std::cerr << "Error calling entity manager. err: " << ec.message()
                      << "\n";
            return;
        }
        ...
        for (const auto& [path, interfaces] : managedObj)
        {
            ...
            if (assocInterfaces.find(parentPath.str) != assocInterfaces.end() &&
                assocInterfaces[parentPath.str] != nullptr)
            {
                continue;
            }

            for (const auto& [interface, propertyMap] : interfaces)
            {
                if (interface != cableStatusConfiguration)
                {
                    continue;
                }

                auto findConnectionType = propertyMap.find("ConnectionType");
                if (findConnectionType == propertyMap.end())
                {
                    std::cerr << interface << " missing ConnectionType\n";
                    break;
                }
                ...
                assocInterfaces[parentPath.str] =
                    objServer.add_interface(parentPath, association::interface);
...
int main()
{
    ...
    createSensorsFromConfig(io, objServer, intrusionSensor);
    ...
    auto cableMatch = std::make_unique<sdbusplus::bus::match_t>(
        static_cast<sdbusplus::bus_t&>(*systemBus),
        sdbusplus::bus::match::rules::propertiesChangedNamespace(
            std::string(inventoryPath), cableStatusConfiguration),
        [&objServer](sdbusplus::message_t& msg) {
    ...
    auto ncsiCableMatch = std::make_unique<sdbusplus::bus::match_t>(
        static_cast<sdbusplus::bus_t&>(*systemBus),
    ...
    if (initializeLanStatus())
    {
        ...
        lanStatusMatch = std::make_unique<sdbusplus::bus::match_t>(
            static_cast<sdbusplus::bus_t&>(*systemBus),
            "type='signal',"
        ...
        // change
        lanConfigMatch = std::make_unique<sdbusplus::bus::match_t>(
            static_cast<sdbusplus::bus_t&>(*systemBus),
            "type='signal', "
            "member='PropertiesChanged',interface='org.freedesktop.DBus."
            "Properties',path_namespace='" +
                std::string(inventoryPath) + "',arg0='" +
