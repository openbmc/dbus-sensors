...
{
    if (data.size() != sdr01_command::dataLength)
    {
                                      ...
                                      std::string tempName)
{
     ...
     */
    uint16_t mData =
        ((resp->mTolDataByte >> (sdr01_command::bitShiftMsb)) << 8) |
        resp->mDataByte;
     ...
     */
    uint16_t bData =
        ((resp->bAcuDataByte >> (sdr01_command::bitShiftMsb)) << 8) |
        resp->bDataByte;
