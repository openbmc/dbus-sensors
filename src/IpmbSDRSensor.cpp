                                 ...
                                 uint8_t dataLength) const
{
    ...
    {
        checkSDRThreshold(sdrDataBytes, hostIndex, tempName);
    }
}

/* This function will convert the raw value of threshold for each sensor */
void IpmbSDRDevice::checkSDRThreshold(std::vector<uint8_t>& sdrDataBytes,
                                      int busIndex, std::string tempName)
{
    uint8_t bitShiftMsb = 2;
    uint8_t sdrThresAccess = 0x0C;
 ...
 * margin */
double IpmbSDRDevice::dataConversion(double value, const int& busIndex,
                                     uint8_t sensorNum)
{
    if ((busIndex < 0) || (sensorNum == 0))
    {
        std::cerr << " Invalid busIndex or sensor Number \n";
    ...
    try
    {
        temp = sensorValRecord.at(busIndex).at(sensorNum);
    }
    catch (std::exception& e)
    {
        std::cerr << e.what() << "BusIndex or sensorNum is out of range \n";
        return std::numeric_limits<double>::quiet_NaN();
    }
    ...
    if (dataVal > sdrtype01::maxPosReadingMargin)
    {
        // Negative reading handle
        if (sdrtype01::twosCompVal == temp.negRead)
        {
            dataVal -= sdrtype01::thermalConst;
        }
    }
