                                  ...
                                  uint8_t resrvIDMSB)
{
    ...
    if ((validRecordCount <= recordCount) && (iCnt < sdr01Command::cntType))
    {
        iCnt += 1;
        getSDRSensorData(recordCount, resrvIDLSB, resrvIDMSB);
    }
    else if ((validRecordCount == recordCount) &&
             (iCnt == sdr01Command::cntType))
    {
        checkSDRData(sdrData);
        sdrData.clear();
        nextRecordIDLSB = 0;
        nextRecordIDMSB = 0;
        return;
...
{
    if (data.size() != sdr01Command::dataLength)
    {
                                      ...
                                      std::string tempName)
{
    ...
    int threshold = (resp->sdrSensCapability) & (sdr01Command::sdrThresAccess);
     ...
     */
    uint16_t mData =
        ((resp->mTolDataByte >> (sdr01Command::bitShiftMsb)) << 8) |
        resp->mDataByte;
     ...
     */
    uint16_t bData =
        ((resp->bAcuDataByte >> (sdr01Command::bitShiftMsb)) << 8) |
        resp->bDataByte;
