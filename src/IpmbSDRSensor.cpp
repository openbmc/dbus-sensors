...
void validateStatus(boost::system::error_code ec,
                    const IpmbMethodType& response, uint8_t cmdAddr)
{
...
/* This function will validate the sensor's threshold value */
double IpmbSDRDevice::sensorValidation(uint16_t mValue, double bValue,
...
void IpmbSDRDevice::checkSDRThreshold(std::vector<uint8_t> data,
                                      ...
                                      uint8_t unit, int neg, uint8_t sensorID)
{
    if (data.empty())
    {
        return;
    ...
    double thresUpCri =
        sensorValidation(mData, bDataVal, expVal,
                         data[static_cast<uint8_t>(SDR01Fields::sdrUpCriType)]);
    double thresLoCri =
        sensorValidation(mData, bDataVal, expVal,
                         data[static_cast<uint8_t>(SDR01Fields::sdrLoCriType)]);
    ...
    sensorValRecord[busIndex][sensorID] = val;
}
