    ...
    size_t retries = 0)
{
    auto getter = std::make_shared<GetSensorConfiguration>(
        ...
        [&io, &objectServer, &tachSensors, &pwmSensors, &presenceSensors,
         &dbusConnection, sensorsChanged](const ManagedObjectType& sensorConfigurations) {
        ...
        for (const auto& path : paths)
        {
            ...
            if (presenceConfig != sensorData->end())
            {
                ...
                else
                {
                    ...
                    if (pinName)
                    {
                        auto findPresenceSensor = presenceSensors.find(*pinName);
...
int main()
{
    ...
    boost::asio::post(io, [&]() {
        createSensors(io, objectServer, tachSensors, pwmSensors, presenceSensors,
                      systemBus, nullptr);
    });
