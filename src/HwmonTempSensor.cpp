...
HwmonTempSensor::HwmonTempSensor(
    ...
    const double scaleValue, const double minValue, const double maxValue,
    const std::string& units, const float pollRate,
    const std::string& sensorConfiguration, const PowerState powerState,
    const std::string& sensorType) :
    Sensor(boost::replace_all_copy(sensorName, " ", "_"),
           std::move(thresholdsIn), sensorConfiguration, objectType, false,
           false, maxValue, minValue, conn, powerState),
    std::enable_shared_from_this<HwmonTempSensor>(), objServer(objectServer),
    inputDev(io, open(path.c_str(), O_RDONLY)), waitTimer(io), path(path),
    offsetValue(offsetValue), scaleValue(scaleValue), minValue(minValue),
    maxValue(maxValue), units(units),
