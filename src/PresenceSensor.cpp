...
PresenceSensor::PresenceSensor(const std::string& iSensorType,
                               const std::string& iSensorName,
                               const std::string& iGpioName) :
    sensorType(iSensorType), sensorName(iSensorName), gpioName(iGpioName)
{
...
void PresenceSensor::updateAndTracePresence(int newValue)
{
    if (newValue != 0)
    {
...
EventPresenceSensor::EventPresenceSensor(
    ...
    PresenceSensor(iSensorType, iSensorName, iGpioName), gpioFd(io)
{
    try
    {
        gpioLine.request(
...
PollingPresenceSensor::PollingPresenceSensor(
    const std::string& iSensorType, const std::string& iSensorName,
    const std::string& iGpioName, bool inverted, boost::asio::io_context& io) :
    PresenceSensor(iSensorType, iSensorName, iGpioName), pollTimer(io)
{
    try
    {
        gpioLine.request(
    ...
    {
        std::cerr << "PollingPresenceSensor: Error reading gpio " << gpioName
                  << ": " << e.what() << std::endl;
        status = false;
