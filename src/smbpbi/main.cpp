                             ...
                             std::span<std::byte> data)
{
    ...
    ssize_t n = write(i2c.handle(), offsetBytes.data(), offsetBytes.size());
    if (n != ssize_t(offsetBytes.size()))
    {
        auto error = errno;
    ...
    n = read(i2c.handle(), data.data(), data.size());
...
// NOTE: clang-tidy tries to expand this function declaration to something
// nonsensical
// NOLINTNEXTLINE
