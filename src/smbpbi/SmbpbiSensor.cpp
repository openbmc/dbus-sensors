...
constexpr const char* configInterface =
    "xyz.openbmc_project.Configuration.SMBPBI_VEEPROM";
constexpr const char* sensorRootPath = "/xyz/openbmc_project/sensors/";
constexpr const char* objectType = "SMBPBI_VEEPROM";
...
SmbpbiSensor::SmbpbiSensor(
    ...
    std::string& valueType, size_t pollTime, double minVal, double maxVal, std::string &path) :
    ...
    sensorUnits(sensorUnits), valueType(valueType), objectServer(objectServer),
    inputDev(io, path, boost::asio::random_access_file::read_only),
    waitTimer(io), pollRate(pollTime)
{
    SmbpbiSensor::sensorType = sensor_paths::getPathForUnits(sensorUnits);
...
double SmbpbiSensor::reading2tempEp(const uint8_t* raw) const
{
...
double SmbpbiSensor::reading2power(const uint8_t* raw) const
{
...
int SmbpbiSensor::i2cCmdDouble(double& reading) 
{
    constexpr int length =
        I2C_READ_LEN_VALUES[static_cast<size_t>(I2C_READ_LEN_INDEX::DOUBLE64)];

    static_assert(length == sizeof(reading), "Unsupported arch");

    std::array<uint8_t, length> buf;
    int ret = i2cCmd(buf.data(), length);
    if (ret < 0)
    {
        return -1;
    }
    uint64_t& tempd = *reinterpret_cast<uint64_t*>(&reading);
...
int SmbpbiSensor::i2cCmdUI64(uint64_t& reading) 
{
    constexpr int length =
        I2C_READ_LEN_VALUES[static_cast<size_t>(I2C_READ_LEN_INDEX::UINT64)];
    std::array<uint8_t, length> buf;
    int ret = i2cCmd(buf.data(), length);
    if (ret < 0)
    {
...
int SmbpbiSensor::i2cCmd(uint8_t* reading,
           int length) 
{
    ...
    if (fd < 0)
    {
        lg2::error(" unable to open i2c device on bus {BUS} err={FD}",
                   "BUS", busId, "FD", fd);
        return -1;
    }
...
{
    uint64_t reading = 0;
    int ret = i2cCmdUI64(reading);
    if (ret >= 0)
    {
        if (debug)
        {
            std::cout << "offset: " << offset << std::hex
                      << " reading: " << reading << "\n";
        }
        if (sensorType == "temperature")
        {
...
int SmbpbiSensor::readDoubleEEPROMData(double& data)
{
    double reading = 0;
    int ret = i2cCmdDouble(reading);
    if (ret >= 0)
    {
        data = reading;
        return 0;
