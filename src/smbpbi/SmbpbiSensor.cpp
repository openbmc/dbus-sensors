...
int SmbpbiSensor::readRawEEPROMData(size_t off, uint8_t length,
                                    double* data) const
{
    uint64_t reading = 0;
...
int SmbpbiSensor::readDoubleEEPROMData(size_t off, uint8_t length,
                                       double* data) const
{
    double reading = 0;
...
void SmbpbiSensor::waitReadCallback(const boost::system::error_code& ec)
{
    ...
    {
        ret = readDoubleEEPROMData(offset, len, &temp);
    }
    else
    {
