...
void readAndProcessNVMeSensor(int index)
{
    ...
    responseTimer->async_wait([slaveSocket](
        ...
        else
        {
            if (DEBUG)
            {
                std::cerr << "Cancelling the request now, past MCTP timeout\n";
            }
            slaveSocket->cancel();
        }
    });
    ...
    if (DEBUG)
    {
        std::cerr << "Sending message to read data from Drive on bus: "
...
void NVMeContext::rxMessage(uint8_t eid, void* data, void* msg, size_t len)
{
    ...
    if (data == nullptr)
    {
        std::cerr << "Index not set for the NVMe drive to be updated \n";
        return;
    }
    ...
    if (DEBUG)
    {
        std::cerr << "Eid from the received messaged: " << eid << "\n";
    }
    ...
    if (*index >= 0 && *index < nvmeDeviceList.size())
    {
        ...
        if (DEBUG)
        {
            std::cerr << "Cancelling the timer now\n";
        }
