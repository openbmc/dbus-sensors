...
void readAndProcessNVMeSensor(int index)
{
    ...
    responseTimer->async_wait([&nvmeDevice](
...
static double getTemperatureReading(double reading)
{
    ...
    else if (reading == 0x80 || reading == 0x81)
    {
        ...
        if (DEBUG)
        {
            std::cout << "Temperature Reading: "
                      << getTemperatureReading(
                             static_cast<double>(messageData[5]))
                      << " Celsius for device at index: " << *index << "\n";
        }

        sensorInfo.value =
            getTemperatureReading(static_cast<double>(messageData[5]));
