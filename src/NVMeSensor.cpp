...
static size_t lastQueriedDeviceIndex = -1;

size_t getDeviceInfoToQuery()
{
                     ...
                     boost::asio::deadline_timer& mctpResponseTimer)
{
    ...
    scanTimer.async_wait([&io, &scanTimer, &mctpResponseTimer](
        ...
        {
            size_t index = getDeviceInfoToQuery();
            if (index >= 0)
            {
                readAndProcessNVMeSensor(io, mctpResponseTimer, index);
            }
        }
...
void readAndProcessNVMeSensor(boost::asio::io_service& io,
                              boost::asio::deadline_timer& mctpResponseTimer,
                              size_t index)
{
