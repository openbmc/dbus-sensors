...
{
    int8_t readingValue = static_cast<int8_t>(0xFF);

    if (temp_byte <= 0x7E || temp_byte > 0xC5)
    {
        readingValue = static_cast<int8_t>(temp_byte);
    }
    ...
    {
        readingValue = static_cast<int8_t>(0xFF);
    }
    ...
    bus(_bus), rootBus(_rootBus)
{
    ...
    int r = mctp_smbus_open_bus(smbus.get(), _bus, _rootBus);
    if (r < 0)
    {
