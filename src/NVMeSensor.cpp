...
void readResponse(struct NVMeContext& nvmeDevice,
                  boost::asio::deadline_timer& mtcpResponseTimer)
{
    nvmeDevice.nvmeSlaveSocket->async_wait(
        ...
        [&mtcpResponseTimer,
         &nvmeDevice](const boost::system::error_code& errorCode) {
            ...
            int rc = mctp_smbus_read(nvmeDevice.smbus);
            ...
            if (rc == -2)
            {
                ...
                readResponse(nvmeDevice, mtcpResponseTimer);
            }
                              ...
                              boost::asio::deadline_timer& mtcpResponseTimer)
{
    ...
    int mtcpResponseTimeout = 1;
    mtcpResponseTimer.expires_from_now(
        boost::posix_time::seconds(mtcpResponseTimeout));
    mtcpResponseTimer.async_wait(
        [&nvmeDevice](const boost::system::error_code& errorCode) {
    ...
    bus(_bus), rootBus(_rootBus)
{
    ...
    int r = mctp_smbus_open_bus(smbus, _bus, _rootBus);
    if (r < 0)
    {
