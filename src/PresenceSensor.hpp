...
class PresenceSensor : private boost::noncopyable
{
  ...
  protected:
    std::shared_ptr<gpiod::line> gpioLine;
    bool status = false;
    ...
    public std::enable_shared_from_this<PollingPresenceSensor>
{
  ...
  private:
    ...
    // Used to allow multiple objects to a single GPIO line
    static std::unordered_map<std::string, std::shared_ptr<gpiod::line>>
        staticGpioMap;
