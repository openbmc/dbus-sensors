                        ...
                        std::vector<uint8_t>& responseMsg)
{
    ...
    uint8_t *pldm_resp_msg = nullptr;
    size_t  resp_msg_len {};
    auto rc = pldm_send_recv(eid, fd, requestMsg.data(),
                requestMsg.size(), &pldm_resp_msg, &resp_msg_len);
    if (rc == PLDM_REQUESTER_SUCCESS) {
        responseMsg.resize(resp_msg_len);
        memcpy(responseMsg.data(), pldm_resp_msg, resp_msg_len);
        free(pldm_resp_msg);
    }
    close(fd);
    return rc;
    ...
    objectServer(objectServer), waitTimer(io)
{
    ...
    sensorInterface = objectServer.add_interface( sensorPath,
    ...
    association = objectServer.add_interface(sensorPath,
...
int PLDMSensor::getSensorReading(uint8_t sensorId, int32_t* reading)
{
    ...
    auto rc = encode_get_sensor_reading_req(instanceId, sensorId, 0xff,
                request);
    if (rc != 0)
        return -1;
    ...
    rc = decode_get_sensor_reading_resp(
            ...
            &completionCode, &dataSize, &operationalState,
            &eventMessageEnable, &presentState, &previousState,
            &eventState, reinterpret_cast<uint8_t*>(&presentReading));
