...
HSCNodePower::HSCNodePower(
    ...
    mPollTimer(io),
    mObjServer(objectServer),
    mType(HSCType::MP5920), mBusId(-1),
    mSlaveAddr(-1)
{
    itemIface = objectServer.add_interface(
        "/xyz/openbmc_project/HotSwapController/Node_Power",
        "xyz.openbmc_project.HSC.NodePower");

    itemIface->register_property("Value", mValue);
...
{
    mPollTimer.cancel();
    mObjServer.remove_interface(itemIface);
}
...
int HSCNodePower::getHSCRegsInfoBytes(uint8_t regs, uint8_t length, uint8_t *pu8data)
{
    std::string i2cBus = "/dev/i2c-" + std::to_string(mBusId);
    int fd = open(i2cBus.c_str(), O_RDWR);
...
int HSCNodePower::setHSCRegsInfoByte(uint8_t regs, uint8_t length, uint8_t pu8data)
{
    std::string i2cBus = "/dev/i2c-" + std::to_string(mBusId);
    int fd = open(i2cBus.c_str(), O_RDWR);
    size_t i = 0;
...
int HSCNodePower::setHSCRegsInfoBytes(uint8_t regs, uint8_t length, uint8_t *pu8data)
{
    std::string i2cBus = "/dev/i2c-" + std::to_string(mBusId);
    int fd = open(i2cBus.c_str(), O_RDWR);
    size_t i = 0;
...
void HSCNodePower::initMP5920()
{
    ...
    iStatus = setHSCRegsInfoBytes(MP5920_PMBUS_PWD, sizeof(writeBuff), &writeBuff[0]);
    if (iStatus != 0)
    {
        return ;
    }
    ...
    iStatus = setHSCRegsInfoBytes(MP5920_BLACKBOX_PWD, sizeof(writeBuff), &writeBuff[0]);
    if (iStatus != 0)
    {
        return ;
    }
    ...
    writeBuff[1] = BLACKBOX_PWD_HIGH;
    iStatus = setHSCRegsInfoBytes(MP5920_NVM_PWD, sizeof(writeBuff), &writeBuff[0]);
    if (iStatus != 0)
    {
        return ;
    }
...
{
    itemIface->set_property("Value", newValue);
    mValue = newValue;
}
...
int HSCNodePower::get_average_power(SHSCDevInfo *psHSCInfo)
{
    ...
    psHSCInfo->u64CurrentSampleCount = (u8ReadBuf[6] << 16) + (u8ReadBuf[5] << 8) + u8ReadBuf[4];
    ...
    if(psHSCInfo->u64CurrentSampleCount > psHSCInfo->u64PrevSampleCount
        ...
        u16Raw = (psHSCInfo->u64CurrentEnergyCount - psHSCInfo->u64PrevEnergyCount)/(psHSCInfo->u64CurrentSampleCount - psHSCInfo->u64PrevSampleCount);
        dReading = ( static_cast<double>(1) / static_cast<double>(mHSCCoefficient.power_m)) * ((static_cast<double>(u16Raw) * pow(10, mHSCCoefficient.power_r)) - static_cast<double>(mHSCCoefficient.power_b));

        powerVal = dReading;
        //PRINT(PRINT_CMC, PRINT_INFO, "hsc %d reading is %lf. \n", psHSCInfo->u8HSCNum, dReading);
        if (static_cast<uint64_t>(powerVal) <= 0x7FFF) {
            // Truncate the power value. The ME currently does not have a way to accept a non-whole-integer power value.
            psHSCInfo->u64AveragePower = (static_cast<uint16_t>(powerVal) & 0x7FFF);
        } else {
            // Anything greater than 0x7FFF should be treated as an error.
            // Return a reading of -1 to indicate to the ME that there was a problem.
