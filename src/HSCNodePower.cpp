...
int HSCNodePowerSensor::getHSCRegsInfoByte(uint8_t regs, uint8_t* pu8data)
{
...
int HSCNodePowerSensor::setHSCRegsInfoByte(uint8_t regs, uint8_t length,
                                           uint8_t pu8data)
{
    std::string i2cBus = "/dev/i2c-" + std::to_string(mBusId);
    int fd = open(i2cBus.c_str(), O_RDWR);
...
void HSCNodePowerSensor::pollHSCChipRegs()
{
    ...
    mPollTimer.async_wait([&](const boost::system::error_code& ec) {
        ...
        if (!ec)
        {
            if (!isPowerOn())
            {
                int statusValue = getAveragePower(&mHSCDevInfo);
