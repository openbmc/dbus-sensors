...
int HSCNodePowerSensor::initMP5920()
{
    uint8_t writeBuff[2];
    int iStatus;
...
int HSCNodePowerSensor::configHSCDevice()
{
    if (mType == HSCType::MP5920)
    {
        return initMP5920();
    }
    // Currently only support MP5920
    return -1;
}
...
void HSCNodePowerSensor::start(HSCType type, int busId, int slaveAddr)
{
    ...
    if (configHSCDevice() != 0)
    {
        std::cerr << "configHSCDevice Failed..."
                  << "\n";
        return;
    }
