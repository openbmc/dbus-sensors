...
static constexpr string nodePwrInf =
...
HSCNodePowerSensor::HSCNodePowerSensor(
    ...
    : Sensor(boost::replace_all_copy(sensorName, " ", "_"),
             ...
             "xyz.openbmc_project.Configuration.MP5920", hscMaxReading,
             hscMinReading),
...
void HSCNodePowerSensor::initMP5920() {
  ...
  iStatus =
      setHSCRegsInfoBytes(mp5920BlackboxPwd, sizeof(writeBuff), &writeBuff[0]);
  if (iStatus != 0) {
    return;
  }
  ...
  writeBuff[1] = blackboxPwdHigh;
  iStatus = setHSCRegsInfoBytes(mp5920NvmPwd, sizeof(writeBuff), &writeBuff[0]);
  if (iStatus != 0) {
    return;
  }
...
int HSCNodePowerSensor::getAveragePower(SHSCDevInfo *psHSCInfo) {
  ...
  getHSCRegsInfoBytes(pmbCmdReadEin, 7, u8ReadBuf);
  ...
  if (psHSCInfo->u64CurrentSampleCount > psHSCInfo->u64PrevSampleCount &&
    ...
    dReading =
        (static_cast<double>(1) / static_cast<double>(mHSCCoefficient.powerM)) *
    ...
    if (static_cast<uint64_t>(powerVal) <= 0x7FFF) {
      ...
      psHSCInfo->u64AveragePower = (static_cast<uint64_t>(powerVal) & 0x7FFF);
    } else {
