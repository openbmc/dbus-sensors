...
constexpr const uint8_t local_eid_default = 8;
        ...
        [&io, &objectServer, &dbusConnection, &sensors, instanceId](
            ...
            for (const auto& pathPair : resp) {
                for (const auto& entry : pathPair.second) {
                    ...
                    uint16_t sensorId = loadVariant<uint16_t>(entry.second,
                                            "SensorId");
                    std::string sensorDataSize = loadVariant<std::string>(
                                                    entry.second,
                                                    "SensorDataSize");

                    std::string sensorType = loadVariant<std::string>(
                                                entry.second, "SensorType");
                    std::string sensorUnit = loadVariant<std::string>(
                                                entry.second, "SensorUnit");
                    ...
                    sensor = std::make_unique<PLDMSensor>(
                        ...
                        sensorType, sensorUnit, sensorDataSize, eid,
                        sensorId, instanceId);
                    sensor->init();
                }
            }
...
void usage(void)
{
    ...
    std::cerr << "Options:\n";
    std::cerr
        << "  --eid=<8-255> - Local EID, default=8\n"
        << "  --help - Print this help message and exit\n";
}

int main(int argc, char * const *argv)
{
    uint8_t local_eid = local_eid_default;
    static const struct option options[] = {
	    { "eid", optional_argument, 0, 'e' },
    ...
    std::cout << "eid = " << local_eid << std::endl;
    ...
    try
    {
        sdbusplus::message::message getInstanceId =
            systemBus->new_method_call(
                "xyz.openbmc_project.PLDM", "/xyz/openbmc_project/pldm",
                "xyz.openbmc_project.PLDM.Requester", "GetInstanceId");

        getInstanceId.append(local_eid);
        sdbusplus::message::message reply = systemBus->call(getInstanceId);
        reply.read(instanceId);
        std::cout << "GetInstanceId = " << (int) instanceId << std::endl;
    }
    catch (const std::exception& e)
    {
        std::cerr << "GetInstanceId D-Bus call failed, error = "
                  << e.what() << "\n";
        return -1;
    }
