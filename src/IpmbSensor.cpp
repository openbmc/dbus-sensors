...
IpmbSensor::IpmbSensor(std::shared_ptr<sdbusplus::asio::connection>& conn,
    ...
    sensorPollSec(pollRate), objectServer(objectServer), waitTimer(io)
{
        ...
        [&](boost::system::error_code ec, const ManagedObjectType& resp) {
            ...
            for (const auto& pathPair : resp)
            {
                for (const auto& entry : pathPair.second)
                {
                    ...
                    int sensorPollSec = pollRateDefault;
                    auto findPollRate = entry.second.find("PollRate");
                    if (findPollRate != entry.second.end())
                    {
                        sensorPollSec = std::visit(VariantToIntVisitor(),
                                                   findPollRate->second);
                    }
                    ...
                    sensor = std::make_unique<IpmbSensor>(
                        ...
                        hostSMbusIndex, sensorPollSec, sensorTypeName);
