...
std::vector<uint8_t> IpmbSensor::getRawPmbusCommand(
    ipmi::sensor::SmbusMessageType messageType,
    const std::vector<uint8_t>& pmbusCommand, uint8_t readLength,
    ipmi::sensor::AddressMode isExtendedDeviceAddress = true,
    bool doEnablePec = false)
{
     ...
     */
    uint8_t byte4 = 0x00;
    if (ipmi::sensor::AddressMode::elevenBit == isExtendedDeviceAddress)
    {
        byte4 |= (1 << 4);
    }
     ...
     */
    if (std::numeric_limits<uint8_t>::max() < commandBytes.size())
    {
