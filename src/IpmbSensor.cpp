...
boost::container::flat_map<std::string, std::unique_ptr<IpmbFWVersion>>
    versions;
        ...
        [&](boost::system::error_code ec, const ManagedObjectType& resp) {
            ...
            for (const auto& pathPair : resp)
            {
                for (const auto& entry : pathPair.second)
                {
                    ...
                    auto findVersionType = entry.second.find("Name1");
                    if (findVersionType != entry.second.end())
                    {
                        versionTypeName = std::visit(VariantToStringVisitor(),
                                                     findVersionType->second);

                        versions.emplace(name,
                                         std::make_unique<IpmbFWVersion>(
                                             dbusConnection, io, pollRate,
                                             objectServer, name, ipmbBusIndex,
                                             deviceAddress, versionTypeName,
                                             sensorClass));
                    }
                    else
                    {
