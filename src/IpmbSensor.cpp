...
void IpmbSensor::getRawData(uint8_t registerToRead,
                            ...
                            std::vector<uint8_t>& result)
{
    ...
    if (input.size() < static_cast<std::size_t>(offset + 1))
    {
        return;
    }
...
void IpmbSensor::loadDefaults()
{
    ...
    else if (type == IpmbType::PXE1410CVR)
    {
        ...
        {
            command = ipmi::me_bridge::getSensorReading;
        ...
        dbusConnection->async_method_call(
            [this](boost::system::error_code ec,
                ...
                else
                {
                    ...
                    getRawData(registerToRead, std::get<5>(response), data);
                }
                if constexpr (debug)
                {
                    std::cout << name << ": ";
...
void IpmbSensor::setReadMethod(const SensorBaseConfigMap& sensorBaseConfig)
{
    ...
    if (std::visit(VariantToStringVisitor(), readMethod->second) != "IPMI")
    {
     ...
     * address.
     */
    auto sensorMeAddressConfig = sensorBaseConfig.find("SensorMeAddress");
    ...
    sensorMeAddress = std::visit(VariantToUnsignedIntVisitor(),
