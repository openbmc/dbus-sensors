                      ...
                      std::string& sensorTypeName)
{
    ...
    name = std::to_string(index) + "-" + sensorInfo.sensorReadName;

    if (sensorInfo.sensCap != sdr01_command::sdrSensNoThres)
    {
    ...
    std::shared_ptr<sdbusplus::asio::connection>& dbusConnection)
{
    ...
    dbusConnection->async_method_call(
        ...
        for (const auto& pathPair : resp)
        {
            ...
            {
                if ((!ipmbSDREnable) && (entry.first != configInterface))
                {
                    continue;
                }

                if ((ipmbSDREnable) && (entry.first != sdrInterface))
                {
                    continue;
                }
                ...
                {
                    uint8_t index = ipmbBusIndex + 1;

                    for (auto& sensorInfo : sensorRecord[index])
                    {
                        ...
                        configSDRSensors(sensorInfo, entry.first, name,
                                         deviceAddress, index, sensorThreshold,
                                         sensorTypeName);

                        auto& sensor = sensors[name];
                        sensor = std::make_unique<IpmbSensor>(
                            dbusConnection, io, name, pathPair.first,
                            objectServer, std::move(sensorThreshold),
...
void sdrHandler(sdbusplus::message::message& message,
                std::shared_ptr<sdbusplus::asio::connection>& dbusConnection,
                boost::asio::io_service& io,
                sdbusplus::asio::object_server& objectServer)
{
    ...
    std::unique_ptr<sdbusplus::bus::match_t> matchSignal =
        std::make_unique<sdbusplus::bus::match_t>(
            ...
            "type='signal',member='PropertiesChanged',path_namespace='" +
                ...
                sdrInterface + "'",
            [&systemBus, &io, &objectServer](sdbusplus::message::message& msg) {
        sdrHandler(msg, systemBus, io, objectServer);
            });
