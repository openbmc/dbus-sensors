    ...
    std::shared_ptr<sdbusplus::asio::connection>& dbusConnection)
{
    ...
    dbusConnection->async_method_call(
        ...
        for (const auto& [path, interfaces] : resp)
        {
            ...
            {
                if (intf != configInterface && intf != hostControllerInterface)
                {
                    continue;
                }
                ...
                if (intf == hostControllerInterface)
                {
                    std::vector<std::string> ipmbStateNames;
                    auto findIpmbStateNames = cfg.find("ChannelNames");
                    if (findIpmbStateNames == cfg.end())
                    {
