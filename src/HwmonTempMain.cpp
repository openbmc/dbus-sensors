...
/* open a file and try to read it.
 * return nullopt if file cannot be opened.
 */
...
std::optional<std::string> getFullHwmonFilePath(const std::string& directory, const std::string& hwmonBaseName, const std::set<std::string>& permitSet)
{
    ...
    auto getter = std::make_shared<GetSensorConfiguration>(
        ...
        std::move([&io, &objectServer, &sensors, &dbusConnection,
            ...
            for (auto& path : paths)
            {
                ...
                sensor = nullptr;
                auto hwmonFile = getFullHwmonFilePath(directory.string(), "temp1", permitSet);
                if (hwmonFile)
                {
                    sensor = std::make_shared<HwmonTempSensor>(
                        *hwmonFile, sensorType,
                ...
                while (true)
                {
                    ...
                    hwmonFile = getFullHwmonFilePath(directory.string(), "temp" + std::to_string(i+1), permitSet);
                    if(hwmonFile)
                    {
                        auto& sensor = sensors[sensorName];
                        sensor = nullptr;
