        ...
        sensorsChanged)
{
    auto getter = std::make_shared<GetSensorConfiguration>(
        ...
        std::move([&io, &objectServer, &sensors, &dbusConnection,
            ...
            if (paths.empty())
            {
                std::cerr << "No temperature or pressure sensors in system\n";
                return;
            }
            ...
            for (auto& path : paths)
            {
                ...
                // offsetVlaue and scaleVale from the driver
                ...
                // Temperature has "Name", pressure has "PressureSensorName"
                auto findSensorName = baseConfigMap->find("Name");
                if (sensorTypeName == "pressure")
                {
                    findSensorName = baseConfigMap->find("PressureSensorName");
                }
