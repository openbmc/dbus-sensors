    ...
    std::string_view service, std::string_view path)
{
    ...
    {
        std::cerr << "Failed to get property. err: " << e.what() << "\n";
        return;
    }
                  ...
                  const std::vector<std::string>& interfaces)
{
    ...
    static auto handler = [&systemBus,
                           &interfaces](sdbusplus::message::message&) {
    ...
    {
        std::cerr << "Failed to get subtree. err: " << e.what() << "\n";
        return;
    }
...
// valid `sensorName`.
std::optional<string> findInventoryPath(const std::string& sensorName)
{
    auto path = std::find_if(inventoryMap.begin(), inventoryMap.end(),
            ...
            if (hwmonFile)
            {
                ...
                else
                {
                    ...
                    if (foundIfaces != sensorTargetMap.end())
                    {
                        ...
                        inventoryPath =
                            findInventoryPath(sensorName).value_or("");
                    }
