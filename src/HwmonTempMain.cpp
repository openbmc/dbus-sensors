...
static double readRawSensorCompValue(const std::string& path)
{
    ...
    // read a line from input_file
    // if read fails close input_file and return
    if (!std::getline(input_file, sensorCompValueRead))
    {
        input_file.close();
        return SNAN;
    }
    ...
    return std::stod(sensorCompValueRead, nullptr);
}
