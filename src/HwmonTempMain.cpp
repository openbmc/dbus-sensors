...
static std::optional<double> readRawSensorCompValue(const std::string& path)
{
        ...
        std::move([&io, &objectServer, &sensors, &dbusConnection,
            ...
            for (auto& path : paths)
            {
                ...
                if (pathStr.ends_with("_raw"))
                {
                    ...
                    std::string pathOffsetStr = pathStr;
                    std::size_t rawFound =
                        pathOffsetStr.length() - rawStr.length();
                    pathOffsetStr.replace(rawFound, rawStr.length(), offsetStr);
                    std::optional<double> tmpOffsetValue =
                        readRawSensorCompValue(pathOffsetStr);
                    // In case there is nothing to read skip this device
                    if (!tmpOffsetValue.has_value())
                    {
                        continue;
                    }
                    offsetValue = tmpOffsetValue.value();
                    std::string scaleStr = "scale";
                    std::string pathScaleStr = pathStr;
                    pathScaleStr.replace(rawFound, rawStr.length(), scaleStr);

                    std::optional<double> tmpScaleValue =
                        readRawSensorCompValue(pathScaleStr);
                    // In case there is nothing to read skip this device
                    if (!tmpScaleValue.has_value())
                    {
                        continue;
                    }
                    scaleValue = tmpScaleValue.value();
                }
