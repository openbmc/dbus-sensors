    ...
    dbusConnection(conn), ioTimer(io)
{
    ...
    try
    {
        for (const auto& pathNames : ipmbStateNames)
        {
            ...
            dbusPath /= hostName;
            std::string ipmbPowerState = escapeName(pathNames);
            dbusPath /= ipmbPowerState;
...
void IpmbPowerMonitor::setIpmbPowerState(std::vector<uint8_t> ipmbResp)
{
    ...
    for (const auto& pathNames : ipmbStateNames)
    {
        std::string ipmbStateName = escapeName(pathNames);
        ...
        bool ipmbState = ((ipmbResp[stateByteMask]) >> stateBitMask) & 0x01;
