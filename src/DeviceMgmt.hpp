...
static constexpr const char* muxDir = "/dev/i2c-mux/";
...
class I2CMux
{
  public:
    ...
    static std::optional<I2CMux> findMux(const std::string& baseIntf,
                                         const SensorData& cfgData,
                                         ...
                                         std::string& channelName)
    {
        ...
        try
        {
            ...
            {
                throw std::logic_error("No Bus or MuxChannel config for " +
                                       path);
            }
            ...
            {
                throw std::logic_error("Can't find Mux or Channel name for " +
                                       path);
            }
            if (std::get_if<std::string>(&findMuxName->second) == nullptr ||
                std::get_if<std::string>(&findChName->second) == nullptr)
            {
                throw std::logic_error("Mux or Channel name invalid for " +
                                       path);
            }
