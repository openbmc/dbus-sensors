    ...
    name(sensor_paths::escapePathForDbus(pwmname))
{
    ...
    controlInterface->register_property(
        ...
        [this](uint64_t& curVal) {
            ...
            if (curVal != value)
            {
                auto scaledTargetValue = static_cast<double>(curVal) / targetIfaceMax;
                auto roundTargetValue = std::round(scaledTargetValue * pwmMax);
                setValue(static_cast<uint32_t>(roundTargetValue));
                sensorInterface->signal_property("Value");
            }
            return curVal;
        });
