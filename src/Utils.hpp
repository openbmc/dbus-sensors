...
bool findFiles(const std::filesystem::path& dirPath,
               ...
               int symlinkDepth = 1);
bool isPowerOn(size_t hostId = 0);
bool hasBiosPost(size_t hostId = 0);
bool isChassisOn(size_t hostId = 0);
bool isChassisReqTransOff(size_t hostId = 0);
...
bool readingStateGood(const PowerState& powerState, size_t hostId = 0);
...
inline size_t getHostId(const SensorBaseConfigMap& cfg)
{
    size_t hostId = 0; // If there's no HostId defined, default to host0
    auto findHostId = cfg.find("HostId");
    if (findHostId != cfg.end())
    {
        std::string readHostId =
            std::visit(VariantToStringVisitor(), findHostId->second);
        hostId = std::stod(readHostId);
    }
    return hostId;
}
