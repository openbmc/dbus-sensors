...
bool isChassisReqTransOff(size_t slotId = 0);
...
{
    size_t slotId = 0; // If there's no SlotId defined, default to slot 0
    auto findSlotId = cfg.find("SlotId");
    if (findSlotId != cfg.end())
    {
        std::string readSlotId =
            std::visit(VariantToStringVisitor(), findSlotId->second);
        slotId = std::stod(readSlotId);
    }
    return slotId;
}
