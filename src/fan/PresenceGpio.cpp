    ...
    PresenceGpio(deviceType, deviceName, gpioName), gpioFd(io)
{
    ...
    {
        lg2::error("Error reading gpio {NAME}: {ERR}", "NAME", gpioName, "ERR",
                   e);
...
void EventPresenceGpio::monitorPresence()
{
    ...
    gpioFd.async_wait(
        ...
        [weakRef](const boost::system::error_code& ec) {
            ...
            if (!self)
            {
                lg2::error("Failed to get lock for eventPresenceGpio: {ERR}",
                           "ERR", ec.message());
                return;
            }
            if (ec)
            {
                ...
                {
                    lg2::error("Error on event presence device {NAME}: {ERR}",
                               "NAME", self->deviceName, "ERR", ec.message());
                }
                return;
            }
            self->read();
    ...
    const boost::system::error_code& ec)
{
    ...
    if (!self)
    {
        lg2::error("Failed to get lock for pollingPresenceGpio: {ERR}", "ERR",
                   ec.message());
        return;
    }
    if (ec)
    {
        if (ec != boost::system::errc::bad_file_descriptor)
        {
            lg2::error("GPIO polling timer failed for {NAME}: {ERR}", "NAME",
                       self->gpioName, "ERR", ec.message());
        }
        return;
    }
    self->monitorPresence();
