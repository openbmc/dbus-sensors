...
bool isTachless(const std::filesystem::path& pwmPath)
{
    ...
    // if no fanN_tach nodes exist in the parent directory,
    // this is a pwm path for a tachless fan
    return !findFiles(directory, R"(fan\d+_input)", tachPaths);
}
...
void createSensors(
    ...
    size_t retries = 0)
{
<<<<<<< PATCH SET (3b671d fan status updates via gpio reads)
...
>>>>>>> BASE      (bfc8bb fanpath disambiguation w/ tachless fan support)
            {
