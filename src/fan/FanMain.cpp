...
// pass in a pwm path, and it'll tell if you in fanN_tach files exist in the
// same directory
bool isTachless(const std::filesystem::path& pwmPath)
{
    ...
    if (!findFiles(directory, R"(fan\d+_tach)", tachPaths))
    {
        lg2::error("No fan tach sensors were found in '{PATH}'", "PATH",
                   directory.string());
        return true;
    }
                    ...
                    std::filesystem::path& pwmPath)
{
    ...
    if (!findFiles(std::filesystem::path("/sys/class/hwmon"), R"(pwm\d+)",
                   pwmfanPaths))
    {
        lg2::error("No fans controls or sensors were found!");
        return false;
    }
...
// The argument to this function should be the fanN_tach or pwmN file
    ...
    size_t retries = 0)
{
    auto getter = std::make_shared<
        ...
        if (!findFiles(std::filesystem::path("/sys/class/hwmon"),
                       R"(pwm\d+|fan\d+_tach)", fanNodePaths))
        {
            lg2::error("No fan pwm controls or sensors in system");
            return;
        }
        ...
        for (const auto& fanNodePath : fanNodePaths)
        {
            ...
            if (fanNodePath.filename().string().starts_with("pwm"))
            {
                ...
                // for tachless fans that have no hwmon/fan_tach
            }
