    ...
    auto getter = std::make_shared<
        ...
        for (const auto& fanOutputPath : fanOutputPaths)
        {
            ...
            for (const auto& [intfPath, cfgData] : sensorConfigurations)
            {
                ...
                {     
                    // sensorConfigurations contains no i2cfan records,
                    // so this lets us skip scanning for i2cfan in the 
                    // sensorTypes loop
                    ...
                    {
                        // indicates the current sensorConfiguration includes no interfaces 
                        // matching xyz.openbmc_project.Configuration.[AspeedFan/NuvotonFan/HPEFan/GenericFan]
                        //
                        // after the surrounding loop exits, baseType being left 'empty' 
                        // will cause the parent loop to 'continue' onto the next sensorConfiguration record.
                        continue;
                    }
