...
class KernelSocketInterface
{
  public:
     ...
     */
    std::weak_ptr<int> init() noexcept
    {
        const int fd = socket(AF_MCTP, SOCK_DGRAM, 0);

        sockfd = fd < 0 ? nullptr : std::make_shared<int>(fd);
     ...
     */
    std::pair<eid_t, std::unique_ptr<std::vector<uint8_t>>> recv()
        const noexcept
    {
        struct sockaddr_mctp addr{};
        ...
        return {eid, std::make_unique<std::vector<uint8_t>>(std::move(resp))};
    }
