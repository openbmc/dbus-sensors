...
static auto asyncDbusMethodCall(
...
static boost::asio::awaitable<int> getGPUEIDs(
    std::shared_ptr<sdbusplus::asio::connection> dbusConnection,
    mctp::MctpRequester& mctpRequester, std::vector<GpuInfo>& eids)
{
    std::string searchPath{"/au/com/codeconstruct/mctp/"};
    ...
    for (const auto ocpamiEndpoint : ocpamiEndpoints)
    {
        ...
        auto rc = gpu_encode_query_device_identification_req(
            OCP_AMI_INSTANCE_MIN, msg);
        if (rc != 0)
        {
            lg2::error(
                "getGPUEIDs(): gpu_encode_query_device_identification_req failed, rc={RC}",
                "RC", rc);
            continue;
        }
        ...
        rc = gpu_decode_query_device_identification_resp(
            new (respMsg.data()) ocp_ami_msg, respMsg.size(), &cc, &reasonCode,
            &deviceIdentification, &instanceId);
        if (rc != 0)
        {
            lg2::error(
                "getGPUEIDs(): gpu_decode_query_device_identification_resp() failed, rc={RC} cc={CC} reasonCode={RESC}\n",
                "RC", rc, "CC", cc, "RESC", reasonCode);
            continue;
        }

        if (deviceIdentification == GPU_DEV_ID_GPU)
        {
            lg2::info(
    ...
    std::shared_ptr<sdbusplus::asio::connection> dbusConnection, bool verbose)
{
    ...
    while (true)
    {
        if (verbose)
        {
            lg2::info("gpuSensorTask(): updating the sensors.");
        }
