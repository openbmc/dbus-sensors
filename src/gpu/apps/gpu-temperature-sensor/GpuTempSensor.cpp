...
boost::asio::awaitable<void> GpuTempSensor::update(
    mctp::MctpRequester& mctpRequester)
{
    std::vector<uint8_t> reqMsg(sizeof(struct ocp_ami_binding_pci_vid) +
                                sizeof(gpu_get_temperature_reading_req));

    auto* msg = new (reqMsg.data()) ocp_ami_msg;

    auto rc = gpu_encode_get_temperature_reading_req(OCP_AMI_INSTANCE_MIN,
                                                     sensorId, msg);
    ...
    rc = gpu_decode_get_temperature_reading_resp(
