...
class RequesterTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        requester = std::make_unique<Requester<MockSocket>>(*ctx);
    }
...
TEST_F(RequesterTest, BasicRequestResponse)
{
    ...
    requester->sendRecvMsg(
        ...
        [this, testId, callbackId](ErrorCode ec, const Response&) {
            EXPECT_EQ(ec, 0);
    ...
    for (int idx = 0; idx < numRequests; ++idx)
    {
        requester->sendRecvMsg(
            ...
            [this, testId, callbackId = callbackIds[idx], &done,
             &completedCallbackIds](ErrorCode ec, const Response&) {
                EXPECT_EQ(ec, 0);
                state[testId].insert(callbackId);
...
TEST_F(RequesterTest, MultipleEndpoints)
{
    ...
    requester->sendRecvMsg(
        ...
        [testId, &flag1, &stop](ErrorCode ec, const Response&) {
            EXPECT_EQ(ec, 0);
            flag1 = true;
    ...
    requester->sendRecvMsg(
        ...
        [testId, &flag2, &stop](ErrorCode ec, const Response&) {
            EXPECT_EQ(ec, 0);
            flag2 = true;
        ...
        for (size_t idx = 0; idx < callbacks.size(); ++idx)
        {
            ...
            requester->sendRecvMsg(
                ...
                [this, testId, callbackId, &completed, &completedCallbacks,
                 eid](ErrorCode ec, [[maybe_unused]] const Response& resp) {
                    EXPECT_EQ(ec, 0);
                    state[testId].insert(callbackId);
