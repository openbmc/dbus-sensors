...
template <SocketInterface SockIntf,
          typename SocketType = boost::asio::local::datagram_protocol::socket>
class Requester
{
  public:
    Requester() = delete;
     ...
     */
    explicit Requester(boost::asio::io_context& ctx) : ctx(ctx), mctpsocket(ctx)
    {
        ...
        mctpsocket.assign(boost::asio::local::datagram_protocol{}, *fd, ec);

        if (ec)
        {
            lg2::error(
                "MctpRequesterT failed to connect to the MCTP socket - ErrorCode={EC}, Error={ER}.",
    ...
    {
        auto req = std::make_unique<OcpVdmRequest>(std::move(reqMsg),
                                                   std::move(callback));
  ...
  private:
    ...
    void awaitResponse()
    {
        mctpsocket.async_wait(
            ...
            [this](const boost::system::error_code& err) {
                ...
                if (!resp)
                {
                    lg2::error("MctpRequesterT: Failed to receive message");
                    return;
                }
                ...
                {
                    lg2::error(
                        "MctpRequesterT: Received Message of Invalid length - {LEN}",
                        "LEN", resp->size());
                    return;
                }
     ...
     *            - A vector containing the response message bytes
     */
    bool enqueueRequest(const eid_t eid, std::unique_ptr<OcpVdmRequest> req)
    {
        if (req->vec.size() <
            sizeof(ocp::accelerator_management::BindingPciVid))
        {
            lg2::error(
                "MctpRequesterT: Message too small - expected={ES}, received={RS}",
                "ES", sizeof(ocp::accelerator_management::BindingPciVid), "RS",
                req->vec.size());
            req->callback(-EBADMSG, Response{});
            return false;
        }
    ...
    void processPendingRequests(const eid_t eid)
    {
        ...
        {
            lg2::error("MctpRequesterT failed send data to the MCTP Socket");
    ...
    void handleResponse(const eid_t eid, std::unique_ptr<Response> response)
    {
        ...
        {
            lg2::error("MctpRequesterT: No active request found for EID={EID}, "
        ...
        {
            lg2::error(
                "MctpRequesterT: Instance ID mismatch for EID={EID} - expected={IID}, received={RIID}",
    ...
    /** @brief Local socket */
    SocketType mctpsocket;
};

using MctpRequester = Requester<KernelSocketInterface>;
} // namespace mctp
