...
{
    ocp::ami::BindingPciVidInfo hdr{};
    hdr.ocp_ami_msg_type = static_cast<uint8_t>(ocp::ami::MessageType::REQUEST);
    hdr.instance_id = 0;
    hdr.msg_type = 0x04;
    ...
    auto rc = ocp::ami::packHeader(pciVendorId, hdr, msg);
    EXPECT_EQ(rc, ocp::ami::CompletionCode::SUCCESS);

    EXPECT_EQ(msg.pci_vendor_id, htobe16(pciVendorId));
    EXPECT_EQ(msg.reserved, 0);
    EXPECT_EQ(msg.datagram, 0);
    EXPECT_EQ(msg.request, 1);
    EXPECT_EQ(msg.ocp_type, ocp::ami::type);
    EXPECT_EQ(msg.ocp_version, ocp::ami::version);
...
TEST(PackMessage, badPathTest)
{
    ...
    hdr.instance_id = 32;
    auto rc = ocp::ami::packHeader(pciVendorId, hdr, msg);
    EXPECT_EQ(rc, ocp::ami::CompletionCode::ERR_INVALID_DATA);
}
...
TEST(encodeReasonCode, testGoodEncodeReasonCode)
{
    std::vector<uint8_t> responseMsg(
        ...
        sizeof(ocp::ami::CommonNonSuccessResponse));
    auto* response = new (responseMsg.data()) ocp::ami::Message;

    uint8_t cc = static_cast<uint8_t>(ocp::ami::CompletionCode::ERROR);
    uint16_t reasonCode =
        static_cast<uint16_t>(ocp::ami::ReasonCode::REASON_NONE);

    auto rc = ocp::ami::encodeReasonCode(cc, reasonCode, 0x00, *response);

    ocp::ami::CommonNonSuccessResponse resp{};
    std::memcpy(&resp, &response->data, sizeof(resp));

    EXPECT_EQ(rc, ocp::ami::CompletionCode::SUCCESS);
    EXPECT_EQ(static_cast<uint8_t>(ocp::ami::CompletionCode::ERROR),
              resp.completion_code);
    EXPECT_EQ(0x00, resp.command);
    EXPECT_EQ(static_cast<uint16_t>(ocp::ami::ReasonCode::REASON_NONE),
...
TEST(decodeReasonCodeCC, testGoodDecodeReasonCode)
{
    std::vector<uint8_t> responseMsg{
        ...
        0x00, // MSG_TYPE
        0x09, // command
        0x01, // completion code != ocp::ami::CompletionCode::SUCCESS
        0x00, // reason code
        0x00};
    ...
    auto rc =
        ocp::ami::decodeReasonCodeAndCC(*response, msgLen, cc, reasonCode);

    EXPECT_EQ(rc, ocp::ami::CompletionCode::SUCCESS);
    EXPECT_EQ(cc, static_cast<uint8_t>(ocp::ami::CompletionCode::ERROR));
...
TEST(decodeReasonCodeCC, testGoodDecodeCompletionCode)
{
    std::vector<uint8_t> responseMsg{
        ...
        0x00, // MSG_TYPE
        0x09, // command
        0x00, // completion code = ocp::ami::CompletionCode::SUCCESS
        0x00, // reason code
        0x02};
    ...
    auto rc =
        ocp::ami::decodeReasonCodeAndCC(*response, msgLen, cc, reasonCode);
    EXPECT_EQ(rc, ocp::ami::CompletionCode::SUCCESS);
    EXPECT_EQ(cc, static_cast<uint8_t>(ocp::ami::CompletionCode::SUCCESS));
...
TEST(decodeReasonCode, testBadDecodeReasonCode)
{
    ...
    uint8_t cc = static_cast<uint8_t>(ocp::ami::CompletionCode::SUCCESS);
    uint16_t reasonCode =
        static_cast<uint16_t>(ocp::ami::ReasonCode::REASON_NONE);

    // Null pointer tests are no longer applicable with references

    auto rc = ocp::ami::decodeReasonCodeAndCC(
        *response, msgLen - 2, cc,
        reasonCode); // sending msg len less then expected
    EXPECT_EQ(rc, ocp::ami::CompletionCode::ERR_INVALID_DATA_LENGTH);
}
...
class GpuCommonTest : public ::testing::Test
{
  protected:
    ...
    void SetUp() override
    {
        buf.resize(1024, 0);
        msg_len = buf.size();
        msg = new (buf.data()) ocp::ami::Message;
    }

    void setOcpVersionAndVendorId()
    {
        msg->hdr.ocp_type = ocp::ami::type;
        msg->hdr.ocp_version = ocp::ami::version;
...
TEST_F(GpuSensorsTest, QueryDeviceIdentificationRequestEncode)
{
    ...
    EXPECT_EQ(msg->hdr.ocp_type, ocp::ami::type);
    EXPECT_EQ(msg->hdr.ocp_version, ocp::ami::version);
...
TEST_F(GpuSensorsTest, QueryDeviceIdentificationResponseEncode)
{
    ...
    uint8_t errorCc = static_cast<uint8_t>(ocp::ami::CompletionCode::ERROR);
