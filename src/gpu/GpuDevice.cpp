...
void GpuDevice::processGpuEndpoint(uint8_t eid)
{
    ...
    {
        lg2::error(
            "Error processing GPU endpoints: gpuEncodeQueryDeviceIdentificationRequest failed, rc={RC}",
    ...
    mctpRequester.sendRecvMsg(
        ...
        [this, eid](int sendRecvMsgResult, std::vector<uint8_t> respMsg) {
            ...
            {
                lg2::error(
                    "Error processing GPU endpoints: MctpRequester::sendRecvMsg() failed, rc={RC}",
            ...
            {
                lg2::error(
                    "Error processing GPU endpoints: MctpRequester::sendRecvMsg() failed, respMsgLen=0");
                return;
            }
            ...
            {
                lg2::error(
                    "Error processing GPU endpoints: gpuDecodeQueryDeviceIdentificationResponse() failed, rc={RC} cc={CC} reasonCode={RESC}",
