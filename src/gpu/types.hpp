...
concept SocketInterface =
    requires(T socket, eid_t eid, const Request& request) {
        { socket.init() } -> std::same_as<std::weak_ptr<int>>;
        { socket.send(eid, request) } -> std::same_as<bool>;
        {
            socket.recv()
        } -> std::same_as<
            std::pair<eid_t, std::unique_ptr<std::vector<uint8_t>>>>;
    };
 ...
 */
using MctpSocketCallback =
    std::function<void(const boost::system::error_code&, const eid_t&,
