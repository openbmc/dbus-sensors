...
GpuTempSensor::GpuTempSensor(
    ...
    sensorId{gpuTempSensorId}, sensorPollMs(pollRate), deviceInfo(deviceInfo),
...
{
    std::vector<uint8_t> reqMsg(sizeof(ocp::ami::BindingPciVid) +
                                sizeof(gpu::GetTemperatureReadingRequest));

    auto* msg = new (reqMsg.data()) ocp::ami::Message;

    auto rc = gpu::encodeGetTemperatureReadingRequest(deviceInfo.instanceId,
                                                      sensorId, *msg);
    ...
    mctpRequester.sendRecvMsg(
        ...
        [this](int sendRecvMsgResult, std::vector<uint8_t> respMsg) {
            ...
            auto rc = gpu::decodeGetTemperatureReadingResponse(
                *new (respMsg.data()) ocp::ami::Message, respMsg.size(), cc,
