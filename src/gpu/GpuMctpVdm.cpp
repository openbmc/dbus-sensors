...
ocp::accelerator_management::CompletionCode
    encodeQueryDeviceIdentificationRequest(uint8_t instanceId,
                                           std::vector<uint8_t>& buf)
{
    ...
    std::memcpy(buf.data() - 1 + sizeof(msg), &request, sizeof(request));
...
ocp::accelerator_management::CompletionCode
    decodeQueryDeviceIdentificationResponse(
        const std::vector<uint8_t>& buf, uint8_t& cc, uint16_t& reasonCode,
        uint8_t& deviceIdentification, uint8_t& deviceInstance)
{
    ...
    std::memcpy(&nonSuccessResponse, buf.data() + sizeof(msg) - 1,
                sizeof(nonSuccessResponse));

    cc = nonSuccessResponse.completion_code;
    if (cc != static_cast<uint8_t>(
                  ocp::accelerator_management::CompletionCode::SUCCESS))
    {
        reasonCode = le16toh(nonSuccessResponse.reason_code);
        return ocp::accelerator_management::CompletionCode::SUCCESS;
    }
    ...
    QueryDeviceIdentificationResponse response{};
    std::memcpy(&response, buf.data() + sizeof(msg) - 1, sizeof(response));
