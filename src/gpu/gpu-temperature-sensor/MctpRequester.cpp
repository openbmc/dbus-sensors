    ...
    const std::function<void(int, std::vector<uint8_t>)>& callback)
{
    ...
    if (reqMsg.size() < sizeof(OcpAmiBindingPciVid))
    {
    ...
    // Send message
    ssize_t rc =
        sendto(sockfd, reqMsg.data(), reqMsg.size(), 0,
        ...
        [this, eid, reqMsg, callback,
         timer](const boost::system::error_code& ec, std::size_t peekedLength) {
            ...
            receivedLength = recvfrom(
                sockfd, fullRespMsg.data(), peekedLength, MSG_TRUNC,
                // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
                reinterpret_cast<struct sockaddr*>(&respAddr), &addrlen);
            ...
            if (receivedLength > sizeof(OcpAmiBindingPciVid))
            {
                OcpAmiBindingPciVid reqHdr{};
                std::memcpy(&reqHdr, reqMsg.data(),
                            sizeof(OcpAmiBindingPciVid));

                OcpAmiBindingPciVid respHdr{};
