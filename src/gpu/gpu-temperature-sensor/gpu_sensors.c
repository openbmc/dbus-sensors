					       ...
					       struct ocp_ami_msg *msg)
{
	...
	header.msg_type = GPU_TYPE_DEVICE_CAPABILITY_DISCOVERY;

	rc = gpu_pack_header(&header, &msg->hdr);
	if (rc != OCP_AMI_SUCCESS) {
		return rc;
	}

	struct gpu_query_device_identification_req *request =
		(struct gpu_query_device_identification_req *)msg->payload;
...
int gpu_encode_query_device_identification_resp(
	...
	struct ocp_ami_msg *msg)
{
	...
	if (cc != OCP_AMI_SUCCESS) {
		return gpu_encode_reason_code(
			cc, reason_code, GPU_QUERY_DEVICE_IDENTIFICATION, msg);
	}

	struct gpu_query_device_identification_resp *response =
		(struct gpu_query_device_identification_resp *)msg->payload;
						...
						uint8_t *device_instance)
{
	...
	if (msg_len <
	    sizeof(struct ocp_ami_binding_pci_vid) +
		    sizeof(struct gpu_query_device_identification_resp)) {
...
int gpu_encode_get_temperature_reading_req(uint8_t instance_id,
					   uint8_t sensor_id,
					   struct ocp_ami_msg *msg)
{
	int rc = 0;
	...
	header.ocp_ami_msg_type = OCP_AMI_REQUEST;
	header.instance_id = instance_id & OCP_AMI_INSTANCEID_MASK;
	header.msg_type = GPU_MSG_TYPE_PLATFORM_ENVIRONMENTAL;
	...
	gpu_get_temperature_reading_req *request =
		(gpu_get_temperature_reading_req *)msg->payload;
					    ...
					    struct ocp_ami_msg *msg)
{
	...
	if (cc != OCP_AMI_SUCCESS) {
		return gpu_encode_reason_code(cc, reason_code,
					      GPU_GET_TEMPERATURE_READING, msg);
	}

	struct gpu_get_temperature_reading_resp *response =
		(struct gpu_get_temperature_reading_resp *)msg->payload;
					    ...
					    double *temperature_reading)
{
	...
	if (msg_len < sizeof(struct ocp_ami_binding_pci_vid) +
			      sizeof(struct gpu_get_temperature_reading_resp)) {
		return OCP_AMI_ERR_INVALID_DATA_LENGTH;
	}
