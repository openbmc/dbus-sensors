...
TEST(queryDeviceIdentification, testGoodEncodeRequest)
{
	std::vector<uint8_t> requestMsg(
		...
		sizeof(gpu_query_device_identification_req));

	auto request = new (requestMsg.data()) ocp_ami_msg;
	uint8_t instance_id = 0x12;
	auto rc = gpu_encode_query_device_identification_req(instance_id,
							     request);
	...
	EXPECT_EQ(GPU_TYPE_DEVICE_CAPABILITY_DISCOVERY,
		  request->hdr.ocp_ami_msg_type);

	EXPECT_EQ(GPU_QUERY_DEVICE_IDENTIFICATION, req->command);
...
TEST(queryDeviceIdentification, testGoodEncodeResponse)
{
	std::vector<uint8_t> responseMsg(
		sizeof(ocp_ami_binding_pci_vid) +
		sizeof(gpu_query_device_identification_resp));
	auto response = new (responseMsg.data()) ocp_ami_msg;

	uint8_t instance_id = 0x12;
	uint8_t cc = OCP_AMI_SUCCESS;
	uint16_t reason_code = NONE;
	uint8_t device_identification = GPU_DEV_ID_GPU;
	uint8_t device_instance = 1;

	auto rc = gpu_encode_query_device_identification_resp(
		instance_id, cc, reason_code, device_identification,
		device_instance, response);

	struct gpu_query_device_identification_resp *resp = new (
		response->payload) struct gpu_query_device_identification_resp;
	...
	EXPECT_EQ(GPU_QUERY_DEVICE_IDENTIFICATION, resp->hdr.command);
...
TEST(queryDeviceIdentification, testGoodDecodeResponse)
{
	std::vector<uint8_t> responseMsg{
		...
		GPU_TYPE_DEVICE_CAPABILITY_DISCOVERY, // GPU_MSG_TYPE
		GPU_QUERY_DEVICE_IDENTIFICATION,      // command
		0,				      // completion code
		0,
		0,
		2,
		0,		// data size
		GPU_DEV_ID_GPU, // device _identification
		1		// instance_id
	};
	...
	auto rc = gpu_decode_query_device_identification_resp(
		response, msg_len, &cc, &reason_code, &device_identification,
		&device_instance);

	EXPECT_EQ(rc, OCP_AMI_SUCCESS);
	EXPECT_EQ(cc, OCP_AMI_SUCCESS);
	EXPECT_EQ(GPU_DEV_ID_GPU, device_identification);
...
TEST(getTemperature, testGoodEncodeRequest)
{
	std::vector<uint8_t> requestMsg(
		sizeof(ocp_ami_binding_pci_vid) +
		sizeof(gpu_get_temperature_reading_req));

	auto request = new (requestMsg.data()) ocp_ami_msg;
	uint8_t sensor_id = 0;

	auto rc = gpu_encode_get_temperature_reading_req(0, sensor_id, request);

	gpu_get_temperature_reading_req *req = new (request->payload)
	...
	EXPECT_EQ(1, request->hdr.request);
	EXPECT_EQ(0, request->hdr.datagram);
	EXPECT_EQ(GPU_MSG_TYPE_PLATFORM_ENVIRONMENTAL,
		  request->hdr.ocp_ami_msg_type);

	EXPECT_EQ(GPU_GET_TEMPERATURE_READING, req->hdr.command);
...
TEST(getTemperature, testGoodDecodeRequest)
{
	std::vector<uint8_t> requestMsg{
		...
		0x89, // OCP_TYPE=8, OCP_VER=9
		GPU_MSG_TYPE_PLATFORM_ENVIRONMENTAL, // GPU_MSG_TYPE
		GPU_GET_TEMPERATURE_READING,	     // command
		1,				     // data size
		1				     // sensor_id
	};
	...
	uint8_t sensor_id = 0;
	auto rc = gpu_decode_get_temperature_reading_req(request, msg_len,
							 &sensor_id);
...
TEST(getTemperature, testGoodEncodeResponse)
{
	std::vector<uint8_t> responseMsg(
		sizeof(ocp_ami_binding_pci_vid) +
		sizeof(gpu_get_temperature_reading_resp));
	auto response = new (responseMsg.data()) ocp_ami_msg;

	double temperature_reading = 12.34;
	uint16_t reasonCode = 0;

	auto rc = gpu_encode_get_temperature_reading_resp(
		0, OCP_AMI_SUCCESS, reasonCode, temperature_reading, response);

	struct gpu_get_temperature_reading_resp *resp =
		new (response->payload) struct gpu_get_temperature_reading_resp;
	...
	EXPECT_EQ(GPU_GET_TEMPERATURE_READING, resp->hdr.command);
...
TEST(getTemperature, testGoodDecodeResponse)
{
	...
	auto rc = gpu_decode_get_temperature_reading_resp(
