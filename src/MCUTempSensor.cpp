...
//constexpr int mcuBusId = 10; // For other smbus topo on MM[11]
...
MCUTempSensor::MCUTempSensor(std::shared_ptr<sdbusplus::asio::connection>& conn,
    ...
    Sensor(boost::replace_all_copy(sensorName, " ", "_"),
           ...
           mcuTempMaxReading, mcuTempMinReading),
...
int MCUTempSensor::getMCURegsInfoWord(uint8_t regs, int16_t *pu16data)
{
    ...
    *pu16data = i2c_smbus_read_word_data(fd, regs);
    if( *pu16data < 0) {
        std::cerr << " read word data failed at " << (int)regs << "\n";
        return -1;
    }
...
void MCUTempSensor::read(void)
{
    ...
    waitTimer.async_wait([this](const boost::system::error_code& ec) {
        ...
        if (ret >= 0) {
            double v = (double)temp / 1000;
            if constexpr (debug) {
                std::cerr << "Value update to " << (double)v
                          << "raw reading " << (int)temp << "\n";
            }
            updateValue(v);
        } else {
        ...
        [&](boost::system::error_code ec, const ManagedObjectType& resp) {
            ...
            for (const auto& pathPair : resp)
            {
                for (const auto& entry : pathPair.second)
                {
                    ...
                    uint8_t tempReg =
                        loadVariant<uint8_t>(entry.second, "Reg");
...
int main(int argc, char** argv)
{
    ...
    sdbusplus::bus::match::match configMatch(
        ...
        "path_namespace='" + std::string(inventoryPath) + "',"
       	"arg0namespace='" + configInterface + "'",
        eventHandler);
