...
MCUTempSensor::MCUTempSensor(std::shared_ptr<sdbusplus::asio::connection>& conn,
                             ...
                             const std::string& modeStr, uint8_t length,
                             double scale) :
    ...
    length(length), scale(scale), objectServer(objectServer), waitTimer(io)
{
...
int MCUTempSensor::getMCURegsInfoWord(uint8_t regs, int16_t* pu16data)
{
    ...
    else if (modeStr == "Block")
    {
        ...
        std::vector<uint8_t> val(length);
        int result = i2c_smbus_read_i2c_block_data(fd, regs, length, val.data());
