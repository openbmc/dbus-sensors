...
MCUTempSensor::MCUTempSensor(std::shared_ptr<sdbusplus::asio::connection>& conn,
                             ...
                             const std::string& modeStr, double scale) :
...
int MCUTempSensor::getMCURegsInfoWord(uint8_t regs, int16_t* pu16data)
{
    ...
    if (modeStr == "Word")
    {
        ...
        *pu16data = static_cast<int16_t>(i2c_smbus_read_word_data(fd, regs));
    }
    else if (modeStr == "Byte")
    {
        ...
        *pu16data = static_cast<int16_t>(i2c_smbus_read_byte_data(fd, regs));
    }
    else
    {
