...
sdbusplus::async::task<config::DetectorConfig>
...
// NOLINTEND(readability-static-accessed-through-instance)
{
    ...
    config.type =
        config::validDetectorTypes.find(co_await client.sub_type())->second;
    config.pinName = co_await client.pin_name();
    config.polarity =
        config::validPinPolarity.find(co_await client.polarity())->second;
    config.level =
        config::validDetectorLevel.find(co_await client.level())->second;
