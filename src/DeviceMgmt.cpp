...
// paths for devices added to a bus from outside userspace,
// but then had driver bindings set from userspace by this
// daemon
std::vector<std::filesystem::path> deviceDriverBindings;
...
int I2CDevice::create() const
{
    ...
    // hasn't happened yet
    else if (params.devicePresent()) // if device is 'present' (handle exists on
                                ...
                                // expected hwmon node)
    {
        ...
        // keep track of drivers we needed to handle bindings for
        deviceDriverBindings.push_back(
            i2cBusPath(params.bus) / deviceDirName(params.bus, params.address));
        std::cerr << "CTS: set binding for "
                  << i2cBusPath(params.bus) /
                         deviceDirName(params.bus, params.address)
                  << "\n";
    }
...
int I2CDevice::destroy() const
{
    ...
    if (!dtor.good())
    {
        // Writing to delete_device will fail for devices added to bus from
        // outside userspace In that case, try to unbind the driver if - but
        // only if  we set the binding for it in the first place

        fs::path basePath =
            i2cBusPath(params.bus) / deviceDirName(params.bus, params.address);

        auto bindIt = std::find(deviceDriverBindings.begin(),
                                deviceDriverBindings.end(), basePath);

        if (bindIt != deviceDriverBindings.end())
        {
            fs::path unbindPath = basePath / "driver/unbind";
            std::ofstream unbind(unbindPath);
            if (!unbind.good())
            {
                // indicates there is no bound driver to unbind
                // something else must have unbound it, so
                // take it off the list of managed paths and return
                deviceDriverBindings.erase(bindIt);
