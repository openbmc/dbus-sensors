    ...
    std::function<void(PowerState type, bool state)>&& hostStatusCallback)
{
    ...
    // Query the initial states before the callback is moved into the
    // chassisMatch lambda below. The queries are async, so ordering here
    // only affects when the requests are sent, not when replies arrive.
