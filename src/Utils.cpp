    ...
    conn->async_method_call(
        [conn, hostStatusCallback,
         retries](boost::system::error_code ec,
            ...
            bool updatedPowerStatus = std::get<std::string>(state).ends_with(".Running");
    ...
    std::function<void(PowerState type, bool state)>&& callback)
{
    ...
    // Move the rvalue reference into a local lvalue so it can be safely shared
    // across multiple asynchronous handlers.
