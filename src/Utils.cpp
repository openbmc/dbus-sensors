    ...
    const std::string& path)
{
    ...
    conn->async_method_call(
        [&association, &path](const boost::system::error_code ec,
            ...
            {
                if (object.first.find("Baseboard"))
                {
                    boardPath = object.first;
                }
                for (const auto& data : object.second)
                {
                    for (const auto& type : data.second)
                    {
                        ...
                        {
                            chassisPath = object.first;
                            setAssociation(association, path, chassisPath);
                            return;
                        }
                    }
                }
            }
            // If no chassis, use baseboard
            setAssociation(association, path, boardPath);
        },
        mapper::busName, mapper::path, mapper::interface, mapper::subtree,
        "/xyz/openbmc_project/inventory/system", 2,
        std::array<std::string, 2>{"xyz.openbmc_project.Inventory.Item.Chassis",
