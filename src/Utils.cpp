    ...
    const size_t hostId = 0, const size_t retries = 2)
{
    ...
    std::string path =
        std::format("{}{}", power::path, std::to_string(hostId));
    ...
    const size_t hostId = 0, const size_t retries = 2)
{
    ...
    std::string path =
        std::format("{}{}", post::path, std::to_string(hostId));
        ...
        hostStatusCallback)
{
    ...
    for (const auto& path : hostSubTreePaths)
    {
        ...
        size_t hostId = 0;
        auto [ptr, ec] = std::from_chars(numberStr.data(),
        ...
        lg2::debug("Setting up Host {HOSTID} signal match", "HOSTID",
                   hostId);
    ...
    for (const auto& path : chassisSubTreePaths)
    {
        ...
        if (pos >= path.size())
        {
            lg2::error("No host id after '/chassis' in path: '{PATH}'",
                       "PATH", path);
            return;
        }
