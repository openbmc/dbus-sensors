                              ...
                              std::function<void(bool post)>&& postCallback)
{
    ...
    powerMatch = std::make_unique<sdbusplus::bus::match_t>(
        ...
        [powerCallback](sdbusplus::message_t& message) {
        ...
        if (findState != values.end())
        {
            ...
            if (!on)
            {
                timer.cancel();
                powerStatusOn = false;
                powerCallback(powerStatusOn);
                return;
            }
            // on comes too quickly
            timer.expires_after(std::chrono::seconds(10));
            timer.async_wait([powerCallback](boost::system::error_code ec) {
    ...
    postMatch = std::make_unique<sdbusplus::bus::match_t>(
        ...
        [postCallback](sdbusplus::message_t& message) {
        std::string objectName;
        boost::container::flat_map<std::string, std::variant<std::string>>
            values;
        message.read(objectName, values);
...
{
    setupPowerMatchCallbacks(conn,
                             [](bool power) { (void)power; },
                             [](bool post) { (void)post; });
}
...
void getMgmtParams(const SensorBaseConfigMap& cfg, std::optional<MgmtParams>& params)
{
    params = std::nullopt;
    ...
    if (clientManaged == cfg.end() || ctorPath == cfg.end() ||
        ctorData == cfg.end() || dtorPath == cfg.end() || dtorData == cfg.end() ||
        presentPath == cfg.end())
    {
        return;
    }
    ...
    try
    {
        if (std::get<bool>(clientManaged->second))
        {
            ...
            MgmtParams::MgmtOp destroy{
                std::get<std::string>(dtorPath->second),
