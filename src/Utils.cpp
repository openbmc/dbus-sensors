...
static std::unordered_map<std::string, std::unique_ptr<sdbusplus::bus::match_t>> powerMatchMap;
static std::unordered_map<std::string, std::unique_ptr<sdbusplus::bus::match_t>> postMatchMap;
static std::unordered_map<std::string, std::unique_ptr<sdbusplus::bus::match_t>> chassisMatchMap;
...
bool hasBiosPost(const size_t& slotId)
{
    ...
    auto match_it = postMatchMap.find(path);
...
bool isChassisOn(const size_t& slotId)
{
    ...
    auto match_it = chassisMatchMap.find(path);
        ...
        hostStatusCallback)
{
    ...
    for (const auto& path : chassisSubTreePaths)
    {
        ...
        chassisMatchMap.emplace(std::string(path),
