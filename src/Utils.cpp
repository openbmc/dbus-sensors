...
static std::unordered_map<size_t, bool> powerStatusOn;
static std::unordered_map<size_t, bool> biosHasPost;
static std::unordered_map<size_t, bool> chassisStatusOn;
static std::unordered_map<size_t, bool> chassisReqPowerTransOff;

static std::unordered_map<size_t, std::unique_ptr<sdbusplus::bus::match_t>>
    powerMatchMap;
static std::unordered_map<size_t, std::unique_ptr<sdbusplus::bus::match_t>>
    postMatchMap;
static std::unordered_map<size_t, std::unique_ptr<sdbusplus::bus::match_t>>
    chassisMatchMap;
static std::unordered_map<size_t, std::unique_ptr<sdbusplus::bus::match_t>>
    chassisReqPowerTransOffMatchMap;
...
bool isPowerOn(const size_t hostId)
{
    auto matchIt = powerMatchMap.find(hostId);
    if (matchIt == powerMatchMap.end())
    {
        throw std::runtime_error(
            "Host " + std::to_string(hostId) + " Power Match Not Created");
    }
    auto it = powerStatusOn.find(hostId);
...
bool hasBiosPost(const size_t hostId)
{
    auto matchIt = postMatchMap.find(hostId);
    if (matchIt == postMatchMap.end())
    {
        throw std::runtime_error(
            "Host " + std::to_string(hostId) + "Post Match Not Created");
    }
    auto it = biosHasPost.find(hostId);
...
bool isChassisOn(const size_t hostId)
{
    auto matchIt = chassisMatchMap.find(hostId);
    if (matchIt == chassisMatchMap.end())
    {
        throw std::runtime_error(
            "Host " + std::to_string(hostId) + " Chassis On Match Not Created");
    }
    auto it = chassisStatusOn.find(hostId);
...
bool isChassisReqTransOff(const size_t hostId)
{
    auto matchIt = chassisReqPowerTransOffMatchMap.find(hostId);
    if (matchIt == chassisReqPowerTransOffMatchMap.end())
    {
        throw std::runtime_error(
            "Host " + std::to_string(hostId) +
            " Chassis Requested Transitioning to Off Match Not Created");
    }
    auto it = chassisReqPowerTransOff.find(hostId);
    if (it == chassisReqPowerTransOff.end())
    {
        return false;
    }
    return it->second;
...
bool readingStateGood(const PowerState& powerState, const size_t hostId)
{
    if (powerState == PowerState::on && !isPowerOn(hostId))
    {
        return false;
    }
    if (powerState == PowerState::biosPost &&
        (!hasBiosPost(hostId) || !isPowerOn(hostId)))
    {
        return false;
    }
    if (powerState == PowerState::chassisOn && !isChassisOn(hostId))
    {
        return false;
    }
    if (powerState == PowerState::chassisOn && isChassisReqTransOff(hostId))
    {
        return false;
    }
...
static void getPowerStatus(
    ...
    const size_t hostId = 0, const size_t retries = 2)
{
    std::string busname =
        std::format("{}{}", power::busname, std::to_string(hostId));
    std::string path = std::format("{}{}", power::path, std::to_string(hostId));

    conn->async_method_call(
        [conn, retries, hostId, path](boost::system::error_code ec,
            ...
            if (ec)
            {
                if (retries != 0U)
                {
                    ...
                    timer->async_wait([timer, conn, retries,
                                       hostId](boost::system::error_code) {
                        getPowerStatus(conn, hostId, retries - 1);
                    });
                    return;
                }
...
{
    std::string busname =
        std::format("{}{}", post::busname, std::to_string(hostId));
    std::string path = std::format("{}{}", post::path, std::to_string(hostId));
        ...
        [conn, retries, hostId, path](boost::system::error_code ec,
            ...
            if (ec)
            {
                if (retries != 0U)
                {
                    ...
                    timer->async_wait([timer, conn, retries,
                                       hostId](boost::system::error_code) {
                        getPostStatus(conn, hostId, retries - 1);
                    });
                    return;
                }
                // we commonly come up before power control, we'll capture the
...
static void getChassisStatus(
    const std::shared_ptr<sdbusplus::asio::connection>& conn,
    const size_t hostId = 0, size_t retries = 2)
{
    std::string busname =
        std::format("{}{}", chassis::busname, std::to_string(hostId));
    std::string path =
        std::format("{}{}", chassis::path, std::to_string(hostId));
        ...
        [conn, retries, hostId, path](boost::system::error_code ec,
            ...
            if (ec)
            {
                if (retries != 0U)
                {
                    ...
                    timer->async_wait([timer, conn, retries,
                                       hostId](boost::system::error_code) {
                        getChassisStatus(conn, hostId, retries - 1);
                    });
                    return;
                }
...
void setupPowerMatchCallback(
    const std::shared_ptr<sdbusplus::asio::connection>& conn,
    const std::function<void(PowerState type, bool state, size_t hostId)>&
        hostStatusCallback)
{
    /* Get host paths */
    ...
    {
        lg2::error("Error getting host subtree paths: '{ERROR_MESSAGE}'",
    ...
    {
        auto pos = path.rfind("/host");
        if (pos == std::string::npos)
        {
            lg2::error("Missing '/host' in path: '{PATH}'", "PATH", path);
            return;
        }
        pos += std::string("/host").size();
        if (pos >= path.size())
        {
            lg2::error("No host id after '/host' in path: '{PATH}'", "PATH",
                       path);
            return;
        }
        std::string_view numberStr(path.data() + pos, path.size() - pos);
        size_t hostId = 0;
        auto [ptr, ec] = std::from_chars(
            numberStr.data(), numberStr.data() + numberStr.size(), hostId, 10);
        if (ec != std::errc{} || ptr != numberStr.data() + numberStr.size())
        {
            lg2::error("Error parsing host id from path: '{PATH}'", "PATH",
                       path);
            return;
        }
        ...
        lg2::debug("Setting up Host {HOSTID} signal match", "HOSTID", hostId);

        powerStatusOn.emplace(hostId, false);
        biosHasPost.emplace(hostId, false);
        ...
        auto powerMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, hostId, timer](sdbusplus::message_t& message) {
                ...
                lg2::debug("Received Host {HOSTID} host power change signal",
                           "HOSTID", hostId);
                ...
                if (findState != values.end())
                {
                    ...
                    if (it != powerStatusOn.end())
                    {
                        ...
                        timer->async_wait([hostStatusCallback, it, hostId](
        ...
        powerMatchMap.emplace(hostId, std::move(powerMatch));
        ...
        auto postMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, hostId](sdbusplus::message_t& message) {
                std::string objectName;
                boost::container::flat_map<std::string,
                                           std::variant<std::string>>
                    values;
        ...
        postMatchMap.emplace(hostId, std::move(postMatch));
        getPowerStatus(conn, hostId);
        getPostStatus(conn, hostId);
    }
    ...
    {
        lg2::error("Error getting chassis subtree paths: '{ERROR_MESSAGE}'",
                   "ERROR_MESSAGE", e.what());
    ...
    for (const auto& path : chassisSubTreePaths)
    {
        ...
        if (pos == std::string::npos)
        {
            lg2::error("Missing '/chassis' in path: '{PATH}'", "PATH", path);
            return;
        }

        pos += std::string("/chassis").size();
        if (pos >= path.size())
        {
            lg2::error("No host id after '/chassis' in path: '{PATH}'", "PATH",
                       path);
            return;
        }
        ...
        chassisStatusOn.emplace(hostId, false);
        chassisReqPowerTransOff.emplace(hostId, false);
        ...
        lg2::debug("Setting up Chassis {HOSTID} signal match", "HOSTID",
                   hostId);
        ...
        auto chassisMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, timerChassisOn,
             hostId](sdbusplus::message_t& message) {
                std::string objectName;
                boost::container::flat_map<std::string,
                                           std::variant<std::string>>
                    values;
                ...
                lg2::debug(
                    "Received Chassis {HOSTID} chassis power change signal",
                    "HOSTID", hostId);
                ...
                if (findState != values.end())
                {
                    ...
                    auto it = chassisStatusOn.find(hostId);
                    auto itReq = chassisReqPowerTransOff.find(hostId);
                    if (it != chassisStatusOn.end())
                    {
                        if (!on)
                        {
                            ...
                            hostStatusCallback(PowerState::chassisOn,
                                               it->second, hostId);
                            if (itReq != chassisReqPowerTransOff.end())
                            {
                                itReq->second = true;
                            }
                            return;
                        ...
                        timerChassisOn->async_wait(
                            [hostStatusCallback, it,
                             hostId](boost::system::error_code ec) {
                                if (ec == boost::asio::error::operation_aborted)
                                {
                                    return;
                                }
                                ...
                                hostStatusCallback(PowerState::chassisOn,
                                                   it->second, hostId);
                                auto itReq =
                                    chassisReqPowerTransOff.find(hostId);
                                if (itReq != chassisReqPowerTransOff.end())
                                {
                                    itReq->second = false;
                                }
        ...
        chassisMatchMap.emplace(hostId, std::move(chassisMatch));
        getChassisStatus(conn, hostId);

        auto chassisReqPowerTransOffMatch = std::make_unique<
            ...
            [hostStatusCallback, hostId](sdbusplus::message_t& message) {
                ...
                lg2::debug(
                    "Received Chassis {HOSTID} chassis power request signal",
                    "HOSTID", hostId);

                auto findRequested =
                    values.find(chassis::propRequestedPowerTransition);
                if (findRequested != values.end())
                {
                    bool rOff = std::get<std::string>(findRequested->second)
                                    .ends_with(chassis::rOff);
                    auto itReq = chassisReqPowerTransOff.find(hostId);
                    if (itReq != chassisReqPowerTransOff.end())
                    {
                        itReq->second = rOff;
        ...
        chassisReqPowerTransOffMatchMap.emplace(
            hostId, std::move(chassisReqPowerTransOffMatch));
    }
}
