    ...
    const size_t slotId = 0, const size_t retries = 2)
{
    ...
    std::string path =
        std::format("{}{}", power::path, std::to_string(slotId));
    ...
    const size_t slotId = 0, const size_t retries = 2)
{
    ...
    std::string path =
        std::format("{}{}", post::path, std::to_string(slotId));
        ...
        hostStatusCallback)
{
    ...
    for (const auto& path : hostSubTreePaths)
    {
        ...
        lg2::debug("Setting up Slot {SLOTID} signal match", "SLOTID",
                   slotId);
        ...
        auto powerMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, slotId,
             timer](sdbusplus::message_t& message) {
                ...
                lg2::debug("Received Slot {SLOTID} host power change signal",
                           "SLOTID", slotId);
    ...
    for (const auto& path : chassisSubTreePaths)
    {
        ...
        if (pos >= path.size())
        {
            lg2::error("No slot id after '/chassis' in path: '{PATH}'",
                       "PATH", path);
            return;
        }
