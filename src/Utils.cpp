...
bool isChassisReqTransOff(const size_t slotId)
{
    ...
    auto match_it = chassisReqPowerTransOffMatchMap.find(slotId);
    if (match_it == chassisReqPowerTransOffMatchMap.end())
    {
        throw std::runtime_error(
            "Slot " + std::to_string(slotId) + " Chassis Requested Transitioning to Off Match Not Created");
    }
        ...
        hostStatusCallback)
{
    ...
    for (const auto& path : hostSubTreePaths)
    {
        ...
        if constexpr (debug)
        {
            lg2::info("Setting up Host {SLOTID} signal match", "SLOTID",
                      slotNumber);
        }
        ...
        auto powerMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, slotNumber,
             timer](sdbusplus::message_t& message) {
                ...
                if constexpr (debug)
                {
                    lg2::info("Received Host {SLOTID} host power change signal",
                              "SLOTID", slotNumber);
                }
    ...
    for (const auto& path : chassisSubTreePaths)
    {
        ...
        if constexpr (debug)
        {
            lg2::info("Setting up Chassis {SLOTID} signal match", "SLOTID",
                      slotNumber);
        }
        ...
        auto chassisReqPowerTransOffMatch = std::make_unique<sdbusplus::bus::match_t>(
            static_cast<sdbusplus::bus_t&>(*conn),
            "type='signal',interface='" + std::string(properties::interface) +
                "',path='" + std::string(path) + "',arg0='" +
                std::string(chassis::interface) + "'",
            [hostStatusCallback, slotNumber](sdbusplus::message_t& message) {
                ...
                auto findRequested = values.find(chassis::propRequestedPowerTransition);
        ...
        chassisReqPowerTransOffMatchMap.emplace(slotNumber, std::move(chassisReqPowerTransOffMatch));
    }
}
