...
{
    auto matchIt = powerMatchMap.find(slotId);
    if (matchIt == powerMatchMap.end())
    {
        throw std::runtime_error(
            "Slot " + std::to_string(slotId) + " Power Match Not Created");
    }
    auto it = powerStatusOn.find(slotId);
...
{
    auto matchIt = postMatchMap.find(slotId);
    if (matchIt == postMatchMap.end())
    {
        throw std::runtime_error(
            "Slot " + std::to_string(slotId) + "Post Match Not Created");
    }
    auto it = biosHasPost.find(slotId);
...
{
    auto matchIt = chassisMatchMap.find(slotId);
    if (matchIt == chassisMatchMap.end())
    {
        throw std::runtime_error(
            "Slot " + std::to_string(slotId) + " Chassis On Match Not Created");
    }
    auto it = chassisStatusOn.find(slotId);
...
bool isChassisReqTransOff(const size_t slotId)
{
    auto matchIt = chassisReqPowerTransOffMatchMap.find(slotId);
    ...
    auto it = chassisReqPowerTransOff.find(slotId);
...
bool readingStateGood(const PowerState& powerState, const size_t slotId)
{
    ...
    if (powerState == PowerState::chassisOn && isChassisReqTransOff(slotId))
    {
        return false;
    }
...
static void getPowerStatus(
    ...
    const size_t slotId = 0, const size_t retries = 2)
{
    std::string busname =
        std::format("{}{}", power::busname, std::to_string(slotId));
    std::string path = std::format("{}{}", power::path, std::to_string(slotId));

    conn->async_method_call(
        [conn, retries, slotId, path](boost::system::error_code ec,
            ...
            if (ec)
            {
                if (retries != 0U)
                {
                    ...
                    timer->async_wait([timer, conn, retries,
                                       slotId](boost::system::error_code) {
                        getPowerStatus(conn, slotId, retries - 1);
                    });
                    return;
                }
...
{
    std::string busname =
        std::format("{}{}", post::busname, std::to_string(slotId));
        ...
        [conn, retries, slotId, path](boost::system::error_code ec,
            ...
            if (ec)
            {
                if (retries != 0U)
                {
                    ...
                    timer->async_wait([timer, conn, retries,
                                       slotId](boost::system::error_code) {
                        getPostStatus(conn, slotId, retries - 1);
                    });
                    return;
                }
                // we commonly come up before power control, we'll capture the
...
static void getChassisStatus(
    const std::shared_ptr<sdbusplus::asio::connection>& conn,
    const size_t slotId = 0, size_t retries = 2)
{
    std::string busname =
        std::format("{}{}", chassis::busname, std::to_string(slotId));
    std::string path =
        std::format("{}{}", chassis::path, std::to_string(slotId));
        ...
        [conn, retries, slotId, path](boost::system::error_code ec,
            ...
            if (ec)
            {
                if (retries != 0U)
                {
                    ...
                    timer->async_wait([timer, conn, retries,
                                       slotId](boost::system::error_code) {
                        getChassisStatus(conn, slotId, retries - 1);
                    });
                    return;
                }
        ...
        hostStatusCallback)
{
    ...
    for (const auto& path : hostSubTreePaths)
    {
        ...
        if (pos >= path.size())
        {
            lg2::error("No slot id after '/host' in path: '{PATH}'", "PATH",
                       path);
            return;
        }
        ...
        auto [ptr, ec] = std::from_chars(
            numberStr.data(), numberStr.data() + numberStr.size(), slotId, 10);
        if (ec != std::errc{} || ptr != numberStr.data() + numberStr.size())
        {
            lg2::error("Error parsing slot id from path: '{PATH}'", "PATH",
                       path);
            return;
        }
        ...
        lg2::debug("Setting up Host {SLOTID} signal match", "SLOTID", slotId);

        powerStatusOn.emplace(slotId, false);
        biosHasPost.emplace(slotId, false);
        ...
        auto powerMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, slotId, timer](sdbusplus::message_t& message) {
                ...
                lg2::debug("Received Host {SLOTID} chassis power change signal",
                           "SLOTID", slotId);
                ...
                if (findState != values.end())
                {
                    ...
                    if (it != powerStatusOn.end())
                    {
                        ...
                        timer->async_wait([hostStatusCallback, it, slotId](
        ...
        powerMatchMap.emplace(slotId, std::move(powerMatch));
        ...
        auto postMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, slotId](sdbusplus::message_t& message) {
                std::string objectName;
                boost::container::flat_map<std::string,
                                           std::variant<std::string>>
                    values;
        ...
        postMatchMap.emplace(slotId, std::move(postMatch));
        getPowerStatus(conn, slotId);
        getPostStatus(conn, slotId);
    }
    ...
    for (const auto& path : chassisSubTreePaths)
    {
        ...
        if (pos >= path.size())
        {
            lg2::error("No slot id after '/chassis' in path: '{PATH}'", "PATH",
                       path);
            return;
        }
        ...
        chassisStatusOn.emplace(slotId, false);
        chassisReqPowerTransOff.emplace(slotId, false);
        ...
        lg2::debug("Setting up Chassis {SLOTID} signal match", "SLOTID",
                   slotId);
        ...
        auto chassisMatch = std::make_unique<sdbusplus::bus::match_t>(
            ...
            [hostStatusCallback, timerChassisOn,
             slotId](sdbusplus::message_t& message) {
                std::string objectName;
                boost::container::flat_map<std::string,
                                           std::variant<std::string>>
                    values;
                ...
                lg2::debug(
                    "Received Chassis {SLOTID} chassis power change signal",
                    "SLOTID", slotId);
                ...
                if (findState != values.end())
                {
                    ...
                    auto it = chassisStatusOn.find(slotId);
                    auto itReq = chassisReqPowerTransOff.find(slotId);
                    if (it != chassisStatusOn.end())
                    {
                        if (!on)
                        {
                        ...
                        timerChassisOn->async_wait(
                            [hostStatusCallback, it,
                             slotId](boost::system::error_code ec) {
                                if (ec == boost::asio::error::operation_aborted)
                                {
                                    return;
                                }
                                ...
                                hostStatusCallback(PowerState::chassisOn,
                                                   it->second, slotId);
                                auto itReq =
                                    chassisReqPowerTransOff.find(slotId);
                                if (itReq != chassisReqPowerTransOff.end())
                                {
                                    itReq->second = false;
                                }
        ...
        chassisMatchMap.emplace(slotId, std::move(chassisMatch));
        getChassisStatus(conn, slotId);
        ...
        auto chassisReqPowerTransOffMatch = std::make_unique<
            ...
            [hostStatusCallback, slotId](sdbusplus::message_t& message) {
                ...
                lg2::debug(
                    "Received Chassis {SLOTID} chassis power request signal",
                    "SLOTID", slotId);
        ...
        chassisReqPowerTransOffMatchMap.emplace(
            slotId, std::move(chassisReqPowerTransOffMatch));
    }
}
