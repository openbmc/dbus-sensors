...
bool isSpecialMode()
{
    ...
    auto mapperCall = bus.new_method_call("xyz.openbmc_project.ObjectMapper", "/xyz/openbmc_project/object_mapper",
                                          "xyz.openbmc_project.ObjectMapper", "GetSubTree");
    mapperCall.append("/");
    mapperCall.append(5);
    mapperCall.append(std::vector<std::string>({"xyz.openbmc_project.Security.SpecialMode"}));

    auto mapperReply = bus.call(mapperCall);

    std::vector<
    std::pair<std::string,
              std::vector<std::pair<std::string, std::vector<std::string>>>>> getSubTreeType;
    ...
    auto methodCall = bus.new_method_call(serviceName.c_str(), path.c_str(),
                                            "org.freedesktop.DBus.Properties", "Get");
    methodCall.append("xyz.openbmc_project.Security.SpecialMode", "SpecialMode");

    auto mapperResponseMsg2 = bus.call(methodCall);

    std::variant<std::string> getManufactMode;
    mapperResponseMsg2.read(getManufactMode);
    std::string manufacturingModeStatus = std::get<std::string>(getManufactMode);
