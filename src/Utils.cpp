...
{
    return fs::path("/sys/bus/i2c/devices/i2c-" + std::to_string(bus));
}
...
{
    fs::path busDir = i2cBusPath(bus);
    fs::path presentPath = busDir / deviceDirName(bus, address);
    ...
    if (pathExists(presentPath))
    {
        return 0;
    }

    // Try to create it: 'echo $devtype $addr > .../i2c-$bus/new_device'
    fs::path ctorPath = busDir / "new_device";
