...
static void handlePowerSignal(const HostStatusCallback& hostStatusCallback,
...
static void handlePostSignal(const HostStatusCallback& hostStatusCallback,
                             sdbusplus::message_t& message)
{
    std::string objectName;
    boost::container::flat_map<std::string, std::variant<std::string>> values;
    ...
    if (findState != values.end())
    {
        const auto& value = std::get<std::string>(findState->second);
        biosHasPost = (value != "Inactive") &&
                      (value != "xyz.openbmc_project.State.OperatingSystem."
                                "Status.OSStatus.Inactive");
...
static void handleChassisSignal(const HostStatusCallback& hostStatusCallback,
                                boost::asio::steady_timer& timerChassisOn,
                                sdbusplus::message_t& message)
{
    std::string objectName;
    boost::container::flat_map<std::string, std::variant<std::string>> values;
...
void setupPowerMatchCallback(
    const std::shared_ptr<sdbusplus::asio::connection>& conn,
    HostStatusCallback&& hostStatusCallback)
{
