...
static constexpr const char* hostMiscManagerService = "xyz.openbmc_project.Host.Misc.Manager";
static constexpr const char* platformStatePath = "/xyz/openbmc_project/misc/platform_state";
static constexpr const char* hostMiscIfce = "xyz.openbmc_project.State.Host.Misc";
...
int main()
{
    ...
    std::function<void(const boost::system::error_code& err)>
        scheduleCpuDetection = [&](const boost::system::error_code& ec) {
        if (ec == boost::asio::error::operation_aborted)
        {
            return; // we're being canceled
        }
        ...
        if (err)
        {
            ...
            cbdTimer.async_wait(
                [&](boost::system::error_code ec) {
                    ...
                    sdbusplus::asio::getProperty<bool>(*systemBus,
                                                       hostMiscManagerService,
                                                       platformStatePath,
                                                       hostMiscIfce,
                                                       biosDoneProperty,
                                                       getPropertyHandler);
