...
static ssize_t processBasicQueryStream(FileHandle& in, FileHandle& out)
{
    ...
    while (true)
    {
        ...
        {
            lg2::error("Failed to read request from in descriptor '{ERRNO}'",
                       "ERRNO", errno);
            if (rc != 0)
            {
                return -errno;
        ...
        while (cursor != resp.end())
        {
            ...
            {
                lg2::error(
                    "Failed to write block data of length '{LEN}' to out pipe: '{ERRNO}'",
                    "LEN", lenRemaining, "ERRNO", strerror(errno));
                if (rc != 0)
                {
                    return -errno;
                }
