...
static boost::container::flat_map<std::string, CPUProperty> propMatch;
static boost::container::flat_map<std::string, int> numCPUSensors;

static std::vector<CPUProperty> socProperties;
...
void propertyInitialize(void)
{
    ...
    propMatch = {{"power", CPUProperty("Power Property", 30000, 0, 1)},
                 {"curr", CPUProperty("Curr property", 30000, 0, 1)},
                 {"temp", CPUProperty("Temp property", 255, 0, 1)},
                 {"in", CPUProperty("Voltage property", 30000, 0, 1)}};
}
