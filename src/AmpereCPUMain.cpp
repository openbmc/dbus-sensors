...
static bool getDeviceInfo(const std::string& devPath, size_t* bus, size_t* addr)
{
    ...
    if (match.empty() || (match.size() != DEV_MAX_IDX))
    {
        std::cerr << "Found bad device path " << devPath << "\n";
        return false;
    }

    std::string busStr = match[DEV_BUS_IDX];
    std::string addrStr = match[DEV_ADDR_IDX];
    ...
    const std::vector<std::string>& findLabels, PowerState readState)
{
    ...
    if (std::regex_search(sensorNameStr, matches, sensorNameRegEx))
    {
        ...
        try
        {
            sensorNameSubStr = matches[DEV_BUS_IDX];
        }
...
int main()
{
    ...
    auto matchPropChanged = std::make_unique<sdbusplus::bus::match_t>(
    ...
    std::function<void(sdbusplus::message_t&)> hostStateHandler =
        [&io, &objectServer, &systemBus](sdbusplus::message_t& message) {
        if (message.is_method_error())
        {
            std::cerr << "callback method error\n";
            return;
    ...
    auto matchHostState = std::make_unique<sdbusplus::bus::match_t>(
        static_cast<sdbusplus::bus_t&>(*systemBus),
