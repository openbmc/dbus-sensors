...
bool findMatchConfig(
    ...
    size_t* thresholdConfSize, size_t bus, size_t addr, std::string deviceName,
    std::string* socName)
{
...
static bool matchSensor(
    ...
    std::vector<std::string> socNames, std::vector<std::string> findLabels,
        ...
        sensorsChanged)
{
    ...
    for (const auto& busPath : busPaths)
    {
        ...
        bool retVal = findMatchConfig(
            sensorConfigs, baseConfig, sensorData, interfacePath, &sensorType,
            &thresholdConfSize, bus, addr, devPath, &socName);
        if (!retVal)
        {
        ...
        {
            if (baseConfig == nullptr ||
                !rescanSensors(sensorsChanged, socName))
            {
                continue;
            }
        }
