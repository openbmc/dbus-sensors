...
auto LeakDetector::readState() -> bool
{
    ...
    info("Read state {STATE} for {PINNAME}", "STATE", lineValue, "PINNAME", config.pinName);
...
auto LeakDetector::setupAsyncEvent() -> bool
{
  ...
  info("setupAsyncEvent: Event fd {LINEFD} for {PINNAME}", "LINEFD", lineFd, "PINNAME", config.pinName);
...
auto LeakDetector::readStateAsync() -> sdbusplus::async::task<>
{
    while (!ctx.stop_requested())
    {
        ...
        info("Received event for {PINNAME}, {LINEFD}", "PINNAME", config.pinName, "LINEFD", lineFd);
