    ...
    std::enable_shared_from_this<ExternalSensor>(), objServer(objectServer)
{
    ...
    if (!sensors::InAllowedMeasures(sensorMeasure))
    {
        throw std::runtime_error("Measure not in AllowedMeasures allowlist");
    }
    std::string objectPath = sensors::objectPath;
