...
{
    const uint8_t instanceId = 3;
    std::vector<uint8_t> buf(gpu::resetNetworkDeviceRequestSize);
    ...
    EXPECT_EQ(request.data_size, 1);

    const uint8_t mode = buf[sizeof(request)];
...
TEST_F(GpuMctpVdmTests, DecodeResetNetworkDeviceResponseSuccess)
{
    ...
    headerInfo.ocp_accelerator_management_msg_type = static_cast<uint8_t>(
        ocp::accelerator_management::MessageType::RESPONSE);
    headerInfo.instance_id = 0;
    headerInfo.msg_type = static_cast<uint8_t>(gpu::MessageType::DIAGNOSTICS);
