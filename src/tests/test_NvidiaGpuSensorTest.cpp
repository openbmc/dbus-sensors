...
TEST_F(GpuMctpVdmTests, DecodeQueryScalarGroupTelemetryV2ResponseSuccess)
{
    ...
    uint32_t* telemetryData = std::bit_cast<uint32_t*>(
        buf.data() + sizeof(ocp::accelerator_management::CommonResponse));
    telemetryData[0] = htole32(100);
    telemetryData[1] = htole32(200);
    telemetryData[2] = htole32(300);
    telemetryData[3] = htole32(400);
...
TEST_F(GpuMctpVdmTests, DecodeListPciePortsResponseSuccess)
{
    ...
    uint16_t* upstreamCount = std::bit_cast<uint16_t*>(
        buf.data() + sizeof(ocp::accelerator_management::CommonResponse));
    *upstreamCount = htole16(totalUpstreamPorts);
