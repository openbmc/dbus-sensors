...
TEST_F(PersistThresholdTest, UpdatesOnlyMatchingIndexedThreshold)
{
    ...
    interface1->register_property(
        "Value", 70.0, [&updated1](const double& request, double& oldValue) {
            oldValue = request;
            updated1 = true;
            return 1;
        });
    ...
    interface2->register_property(
        "Value", 80.0, [&updated2](const double& request, double& oldValue) {
            oldValue = request;
            updated2 = true;
            return 1;
        });
    ...
    thresholds::persistThreshold(thresholdPath1, baseInterface, threshold,
                                 bus(), 2, "Cpu Temp", 1);

    ASSERT_TRUE(
        pumpIoUntil([&updated1] { return updated1; }, std::chrono::seconds{5}));
    EXPECT_TRUE(updated1);
    EXPECT_FALSE(updated2);
    EXPECT_DOUBLE_EQ(
        getProperty<double>(thresholdPath1, thresholdInterface0, "Value"),
        55.0);
    EXPECT_DOUBLE_EQ(
        getProperty<double>(thresholdPath1, thresholdInterface1, "Value"),
        80.0);
}
