...
// Mirrors how HwmonTempMain.cpp calls in today: only the index is known, so
// a threshold naming its channel with Label cannot be placed at all. None of
// them name temp1, so temp1 must come away with nothing rather than with
// temp2's and temp3's thresholds stacked on top of its own.
