...
std::vector<uint8_t> buildErrorResponse()
{
    ...
    pack.pack(static_cast<uint16_t>(0x1234));
    EXPECT_EQ(pack.getError(), 0);
    return buf;
}
...
class NvidiaGpuControlTestBase : public DbusMockTestBase
{
  protected:
    ...
    static std::shared_ptr<NvidiaGpuControl> createControl(
        const std::string& name = "GPU_CTRL", uint8_t eid = defaultEid)
    {
...
{
    {
        auto ctrl = createControl("ctrl_dtor");
        ASSERT_NE(ctrl, nullptr);
    }
    // ctrl destroyed here — should not crash
    SUCCEED();
}
