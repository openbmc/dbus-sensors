...
class DbusMockTestBase : public ::testing::Test
{
  protected:
    ...
    // Intentionally never destroyed: handlers registered into process-global
    // registries (e.g. NvidiaEventHandler) can own a shared_ptr to `conn` and
    // outlive this fixture's static teardown. Keeping the io_context alive for
    // the whole process means ~connection() can never touch a destroyed
    // io_uring service during static destruction at exit.
    ...
    {
        // GTest calls this inherited hook for every derived fixture. Keep the
        // shared connection alive until process exit so queued asio handlers
        // never reference a destroyed sdbusplus connection between suites.
    }
