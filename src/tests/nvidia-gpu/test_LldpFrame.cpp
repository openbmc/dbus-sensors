...
{
    LldpFrame frame;
    ASSERT_EQ(parseLldpFrame(minimalUnit(), frame), 0);

    EXPECT_EQ(frame.chassisIdSubtype, LldpIdSubtype::MacAddr);
    EXPECT_EQ(frame.chassisId, "00:11:22:33:44:55");
    EXPECT_EQ(frame.portIdSubtype, LldpIdSubtype::IfName);
...
TEST(LldpFrame, ClearsWhatAnEarlierUnitLeftBehind)
{
    ...
    appendEnd(named);
    ASSERT_EQ(parseLldpFrame(named, frame), 0);
...
TEST(LldpFrame, AcceptsZeroPaddingAfterTheEndMarker)
{
    ...
    LldpFrame frame;
    ASSERT_EQ(parseLldpFrame(pdu, frame), 0);
    EXPECT_EQ(frame.portId, "eth0");
}
...
TEST(LldpFrame, RejectsNonZeroBytesAfterTheEndMarker)
{
    ...
    LldpFrame frame;
    EXPECT_NE(parseLldpFrame(pdu, frame), 0);
}

TEST(LldpFrame, RejectsAnEmptyUnit)
{
    LldpFrame frame;
    EXPECT_NE(parseLldpFrame({}, frame), 0);
}

TEST(LldpFrame, RejectsAUnitWithNoEndMarker)
{
    LldpFrame frame;
    EXPECT_NE(parseLldpFrame(mandatoryFields(), frame), 0);
}
...
TEST(LldpFrame, ReadsChassisAndPortSubtypesFromSeparateTables)
{
    ...
    LldpFrame frame;
    ASSERT_EQ(parseLldpFrame(pdu, frame), 0);
    EXPECT_EQ(frame.chassisIdSubtype, LldpIdSubtype::PortComp);
    EXPECT_EQ(frame.portIdSubtype, LldpIdSubtype::MacAddr);
...
TEST(LldpFrame, LeavesOutAnIdentifierOfAnUnnamedSubtype)
{
    ...
    LldpFrame frame;
    ASSERT_EQ(parseLldpFrame(pdu, frame), 0);
    EXPECT_EQ(frame.chassisIdSubtype, LldpIdSubtype::NotTransmitted);
...
TEST(LldpFrame, DecodesTheCapabilitiesTheDeviceClaims)
{
    ...
    EXPECT_EQ(frame.systemCapabilities,
              (std::vector<LldpSystemCapability>{
                  LldpSystemCapability::Bridge, LldpSystemCapability::Router}));
}
...
TEST(LldpFrame, TellsClaimingNothingApartFromSayingNothing)
{
    ...
    ASSERT_EQ(parseLldpFrame(pdu, frame), 0);
    EXPECT_EQ(frame.systemCapabilities,
              (std::vector<LldpSystemCapability>{LldpSystemCapability::None}));
