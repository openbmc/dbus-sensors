...
std::vector<uint8_t> buildLeakDetectionInfoResponse(uint16_t adcReadingMv)
{
    ...
    //            numSensors * (sensorId(1) + leakState(1) + thresholds(2*numThresholdLevels) + adcReadingMv(2))
    ...
    pack.pack(static_cast<uint8_t>(gpu::PlatformEnvironmentalCommands::GET_LEAK_DETECTION_INFO));
    pack.pack(static_cast<uint8_t>(ocp::accelerator_management::CompletionCode::SUCCESS));
...
std::vector<uint8_t> buildLeakErrorResponse(uint8_t cc, uint16_t reasonCode)
{
    return test_utils::buildPlatformEnvErrorResponse(
        gpu::PlatformEnvironmentalCommands::GET_LEAK_DETECTION_INFO, cc, reasonCode);
}
...
class NvidiaSmaLeakSensorTest : public MctpMockTestBase
{
  protected:
    ...
    {
        // sensor path prefix in NvidiaSmaLeakSensor.cpp is "/xyz/openbmc_project/sensors/"
        // and it appends "voltage/" + escapeName(name)
...
TEST_F(NvidiaSmaLeakSensorTest, UpdateSuccessValidReading)
{
    EXPECT_CALL(mctpMock, sendRecvMsg)
        .WillOnce(mock_mctp::respondWith({}, buildLeakDetectionInfoResponse(1500)));
...
{
    EXPECT_CALL(mctpMock, sendRecvMsg)
        .WillOnce(mock_mctp::respondWith({}, buildLeakDetectionInfoResponse(0)));
...
{
    std::shared_ptr<NvidiaSmaLeakSensor> sensor =
        createSensor("leak_dtor");
