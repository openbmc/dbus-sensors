...
constexpr uint8_t portStatePhysicalUp = 10;
...
TEST_F(NvidiaNVLinkPortStatusTest, UpdateMapsPollingPortStateToNoLink)
{
    EXPECT_CALL(mctpMock, sendRecvMsg)
        .Times(testing::AtLeast(1))
        .WillRepeatedly(mock_mctp::respondWith(
...
TEST_F(NvidiaNVLinkPortStatusTest, UpdateMapsPhysicalUpPortStateToStarting)
{
    EXPECT_CALL(mctpMock, sendRecvMsg)
        .Times(testing::AtLeast(1))
        .WillRepeatedly(mock_mctp::respondWith(
            {},
            buildPortStatusResponse(portStatePhysicalUp, portStatusEnabled)));

    const std::string gpuName = "nvlink_status_physical_up";
    const std::shared_ptr<NvidiaNVLinkPortStatus> status =
        createStatus(gpuName);
    status->update();
...
TEST_F(NvidiaNVLinkPortStatusTest, UpdateMapsReservedPortStateToUnknown)
{
    EXPECT_CALL(mctpMock, sendRecvMsg)
        .Times(testing::AtLeast(1))
        .WillRepeatedly(mock_mctp::respondWith(
    ...
    const std::string gpuName = "nvlink_status_reserved";
    const std::shared_ptr<NvidiaNVLinkPortStatus> status =
        createStatus(gpuName);
    status->update();
    ...
    EXPECT_EQ(
        getProperty<std::string>(portPath(gpuName), portInterfaceName,
                                 "LinkStatus"),
        "xyz.openbmc_project.Inventory.Connector.Port.LinkStatus.Unknown");
}
