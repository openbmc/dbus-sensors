...
TEST_F(NvidiaGpuEnergySensorTest, ConstructorUpdatedTimeIsNaN)
{
    ...
    EXPECT_TRUE(std::isnan(getProperty<double>(
        energyPath(name), "xyz.openbmc_project.Sensor.Value", "UpdatedTime")));
}
...
TEST_F(NvidiaGpuEnergySensorTest, UpdateMctpTransportErrorLeavesUpdatedTimeNaN)
{
    EXPECT_CALL(mctpMock, sendRecvMsg)
        .WillOnce(mock_mctp::respondWith(
            std::make_error_code(std::errc::timed_out), {}));
...
TEST_F(NvidiaGpuEnergySensorTest, UpdateBadCompletionCodeLeavesUpdatedTimeNaN)
{
    EXPECT_CALL(mctpMock, sendRecvMsg)
        .WillOnce(mock_mctp::respondWith(
            {}, buildEnergyErrorResponse(
