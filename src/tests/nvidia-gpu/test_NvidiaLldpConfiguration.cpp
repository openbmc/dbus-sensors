...
class NvidiaLldpConfigurationTest : public MctpMockTestBase
{
  protected:
    ...
    std::vector<uint8_t> buildReply(std::span<const uint8_t> request)
    {
        ...
        return buildGetModeResponse(deviceMode);
    }
    ...
    // The device is asked for its mode before the object exists, so the
    // fixture hands the object what that answer held, as PcieDevice does.
    std::shared_ptr<NvidiaLldpConfiguration> makeConfiguration() const
    {
        return std::make_shared<NvidiaLldpConfiguration>(
