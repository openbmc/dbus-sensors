...
class NvidiaSmaDeviceTestBase : public DbusMockTestBase
{
  protected:
...
TEST_F(NvidiaSmaDeviceTestBase, ConstructorSetsPath)
{
    const std::shared_ptr<SmaDevice> device = createDevice();
    EXPECT_NE(device, nullptr);
}

TEST_F(NvidiaSmaDeviceTestBase, InitSendsAtLeastOneRequest)
{
    const std::shared_ptr<SmaDevice> device = createDevice();
    device->init();
...
TEST_F(NvidiaSmaDeviceTestBase, ReadLoopStopsOnTimerCancel)
{
    {
        const std::shared_ptr<SmaDevice> device = createDevice();
        device->init();
