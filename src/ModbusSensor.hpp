...
class ModbusSensor : public Sensor
{
  public:
    ModbusSensor(std::shared_ptr<sdbusplus::asio::connection>& conn,
                 ...
                 std::string& device, uint32_t baud, std::string& parity,
                 uint8_t dataBit, uint8_t stopBit, uint16_t requestFunctionCode,
                 uint16_t requestAddress, uint16_t requestQuantity,
                 uint16_t requestByteCount, uint16_t requestValue,
