    ...
    for (const auto& [path, cfgData] : sensorConfigs)
    {
        for (const auto& [intf, cfg] : cfgData)
        {
            ...
            if (nameCfg != cfg.end())
            {
                ...
                const auto namesCfg = cfg.find("Names");
                if (namesCfg != cfg.end() &&
                    std::holds_alternative<std::vector<std::string>>(
                        namesCfg->second))
                {
                    const auto names =
                        std::get<std::vector<std::string>>(namesCfg->second);
                    hwmonNames.append_range(names);
                }
         ...
         activateOnly](const ManagedObjectType& sensorConfigurations) {
            ...
            for (auto& path : paths)
            {
                ...
                while (true)
                {
                    ...
                    auto findNames = baseConfigMap.find("Names");
                    ...
                    if (findNames != baseConfigMap.end())
                    {
                        ...
                        // "Name1", ... was 1-based, "Names" is 0-based
