...
void PresenceSensor::initGpio(const std::string& gpioName, bool inverted)
{
    ...
    std::string gpioPresLabel =
        gpioLine.get_chip().name() + std::to_string(gpioLine.offset());

    staticGpioMap.emplace(gpioName, gpioLine);
    gpioPresValueMap.emplace(gpioPresLabel, gpioLine.get_value());
}
...
void PresenceSensor::monitorPresence(void)
{
    ...
    if (gpioPresValueMap.at(gpioPresLabel) != statusTry)
    {
        if (++confirmTimes >= 3)
        {
            gpioPresValueMap.at(gpioPresLabel) = statusTry;
            status = statusTry;
            if (statusTry)
            {
                logFanInserted(name);
...
bool PresenceSensor::getValue()
{
    ...
    std::string gpioPresLabel =
        gpioLine.get_chip().name() + std::to_string(gpioLine.offset());
    return gpioPresValueMap.at(gpioPresLabel);
}
