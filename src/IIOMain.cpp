        ...
        std::move([&io, &objectServer, &sensors, &dbusConnection,
            ...
            for (auto& path : paths)
            {
                ...
                if (pathStr.find("_raw") != std::string::npos)
                {
                    ...
                    // TODO: get the values from the drive via the
                    // /sys/bus/iio/devices/iio:deviceX/in_XXX_offset
                    // /sys/bus/iio/devices/iio:deviceX/in_XXX_scale
                    offsetValue = -4368.0;
                    scaleValue = 10.725097656;
                }
