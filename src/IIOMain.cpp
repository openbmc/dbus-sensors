    ...
    auto getter = std::make_shared<GetSensorConfiguration>(
        ...
        std::move([&io, &objectServer, &sensors, &dbusConnection,
            ...
            for (auto& path : paths)
            {
                ...
                {
                    std::cerr << "failed to find the sensor type for "
                              << deviceName << "\n";
                    continue;
                }

                std::string configSensorTypeName =
                    std::visit(VariantToStringVisitor(), findType->second);
                if (sensorTypeName.compare(configSensorTypeName))
                {
                    std::cerr << "config sensor type " << configSensorTypeName
                              << " doesn't match sensor type " << sensorTypeName
                              << "\n";
                }
