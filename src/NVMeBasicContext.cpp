...
static ssize_t execBasicQuery(uint32_t bus, uint8_t addr, uint8_t cmd,
...
static ssize_t processBasicQueryStream(int in, int out)
{
    ...
    while (true)
    {
        ...
        /* Deserialise request */
        memcpy(&bus, req.data(), sizeof(bus));
        bus = le32toh(bus);
        uint8_t addr = req[sizeof(bus) + 0];
        uint8_t cmd = req[sizeof(bus) + 1];

        /* Execute the query */
        rc = execBasicQuery(bus, addr, cmd, resp);
...
void NVMeBasicContext::readAndProcessNVMeSensor()
{
    uint32_t bus;
    /* bus + address + command */
    auto command = std::make_shared<std::array<uint8_t, sizeof(bus) + 1 + 1>>();
    ...
    /* Serialise the query parameters for the IO thread */
    bus = htole32(bus);
    memcpy(command->data(), &bus, sizeof(bus)); /* I2C bus for the device */
    (*command)[sizeof(bus) + 0] = 0x6a;         /* I2C address of the device */
    (*command)[sizeof(bus) + 1] =
        0x00; /* Offset for the start of the block read */
