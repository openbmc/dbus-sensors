...
PSUSensor::PSUSensor(
    ...
    i2cDevice(i2cDevice), bridgeGpio(std::move(bridgeGpio)), objServer(objectServer),
...
void PSUSensor::setupRead()
{
    ...
    if (bridgeGpio.has_value())
    {
        ...
        waitTimer.expires_after(std::chrono::milliseconds(bridgeGpio->setupTimeMs));
        waitTimer.async_wait(
            [weak, buffer{buffer}](const boost::system::error_code& ec) {
                ...
                self->inputDev.async_read_some_at(
                    ...
                    [weak, buffer](const boost::system::error_code& ec, size_t bytesRead) {
                        std::shared_ptr<PSUSensor> self = weak.lock();
                        if (!self)
                        {
                            return;
