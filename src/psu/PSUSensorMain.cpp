    ...
    const std::string* label /* = nullptr */)
{
    for (const auto& [key, cfgMap] : sensorData)
    {
        ...
        std::string gpioName = std::visit(VariantToStringVisitor(), findName->second);
        ...
        {
            setupTime = std::visit(VariantToFloatVisitor(), findSetupTime->second);
        }
        ...
        if constexpr (debug)
        {
            lg2::info("Found BridgeGpio config: Label={LABEL}, Name={NAME}, Polarity={POL}, SetupTime={TIME}",
                      "LABEL", labelVal,
                      "NAME", gpioName,
                      "POL", (polarity == gpiod::line::ACTIVE_LOW ? "Low" : "High"),
                      "TIME", setupTime);
        }
    ...
    bool activateOnly)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            auto bridgeGpio = parseBridgeGpioFromConfig(*sensorData, &labelHead);
