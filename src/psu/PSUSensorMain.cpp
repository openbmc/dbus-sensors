    ...
    bool activateOnly)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        if (!nameFile.good())
        {
            lg2::error("Failure finding {PATH}", "PATH", pmbusPath.string());
            continue;
        }
        ...
        if (!ret.second)
        {
            lg2::error("Duplicate path: {PATH}", "PATH", directory.string());
        ...
        if (directory.parent_path() == "/sys/class/hwmon")
        {
            ...
            if (match.empty())
            {
                lg2::error("Found bad device path: {PATH}", "PATH", devicePath);
                continue;
            }
            // Extract <bus>-<address>
        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            if (findProperty == labelMatch.end())
            {
                ...
                {
                    lg2::error(
                        "Could not find matching default property for {NAME}",
                        "NAME", sensorNameSubStr);
                }
                continue;
            }
            ...
            if (findCustomOffset != baseConfig->end())
            {
                ...
                {
                    lg2::error("Unable to parse {OFFSSET}", "OFFSET",
                               keyOffset);
                    continue;
                }
            }
            ...
            if (!customizedName)
            {
                ...
                if constexpr (debug)
                {
                    lg2::error("{LABEL} paired with {NAME} at index {INDEX}",
            ...
            {
                lg2::error(
                    "Sensor properties - Name: {NAME}, Scale: {SCALE}, Min: {MIN}, Max: {MAX}, Offset: {OFFSET}",
