    ...
    bool activateOnly)
{
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        size_t readSlotId = getSlotId(*baseConfig);
...
{
    lg2::debug(
        "PowerStateChanged: Host {SLOTID}, newState: {STATE}, PowerState_type: {TYPE}",
        "SLOTID", slotId, "STATE",
    ...
    else
    {
        ...
        {
            lg2::debug(
                "Checking host {SLOTID}, sensor {SENSOR}  PowerState_type: {TYPE}",
                "SENSOR", path, "SLOTID", sensor->slotId, "TYPE",
                toString(type));
            ...
            if (sensor != nullptr && sensor->readState == type)
            {
                ...
                {
                    lg2::debug(
                        "Deactivating host {SLOTID}, sensor {SENSOR}, due to PowerState::chassisOn",
                        "SENSOR", path, "SLOTID", slotId);
                    sensor->deactivate();
                }
            }
        }
