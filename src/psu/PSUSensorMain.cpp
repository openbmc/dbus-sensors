...
static boost::container::flat_map<std::string, FwUpdateGroup>
...
static void handleFirmwareUpdate(const std::string& path, bool activating)
{
    ...
    auto& association = firmwareAssociations[fwName];
    association.activating = activating;

    for (const auto& sensorName : association.sensorNames)
    {
    ...
    if (!association.psuName.empty())
    {
        auto eventIt =
            combineEvents.find(association.psuName + "OperationalStatus");
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        if (findFwName != baseConfig->end())
        {
            ...
            if (fwNamePtr != nullptr && !fwNamePtr->empty())
            {
                fwName = fwNamePtr;
                auto& association = firmwareAssociations[*fwName];
                association.psuName = *psuName;
                if (association.activating)
                {
                    skipReading = true;
                }
            }
        ...
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            else
            {
                ...
                if (fwName != nullptr)
                {
                    firmwareAssociations[*fwName].sensorNames.push_back(
                        sensorName);
                    if (skipReading)
                    {
