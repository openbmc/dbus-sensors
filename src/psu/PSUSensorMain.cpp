...
static constexpr auto firmwareTypes = std::to_array<std::string_view>({
    "MP2X6XXFirmware",
    "MP292XFirmware",
    "MP297XFirmware",
    "MP5998Firmware",
...
static void handleFirmwareUpdate(std::string_view path, bool activating)
{
    std::string_view fwPath = path;
    ...
    if (pos != std::string_view::npos)
    {
        ...
        {
            fwPath = path.substr(0, pos);
        }
    }
    std::string fwName = std::filesystem::path(fwPath).filename().string();
...
static void processFwActivation(sdbusplus::message_t& message)
{
    ...
    if (findActivation != values.end())
    {
        ...
        {
            handleFirmwareUpdate(message.get_path(),
