...
constexpr auto methodGetAll = "GetAll";
        ...
        std::function<void(sdbusplus::message_t& msg)>&& statusCallback)
    {
        return std::make_shared<sdbusplus::bus::match_t>(
            ...
            "type='signal',interface='" +
                ...
                std::string(std::move(path)) + "',arg0='" +
                std::string(std::move(interface)) + "'",
            std::move(statusCallback));
    }

    static uint8_t getHostStatus(
                               ...
                               const std::string& sensorObjectPath)
{
    ...
    try
    {
        service = getService(criticalInterface, objectPath);
