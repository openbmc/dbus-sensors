...
Config::Config(const sdbusplus::message::object_path& obj, const SensorData& item) : detectedIface(std::nullopt)
{
    (void)obj;
    ...
    for(unsigned int i=0; true; i++){
       ...
       if (found == item.end())
       {
           ...
           lg2::error("could not find gpio interface for device {NAME}", "NAME",
                   name);
        ...
        auto gpioName = loadVariant<std::string>(gpioProperties, properties::propertyName);
        auto polarity =
           loadVariant<std::string>(gpioProperties, properties::propertyPolarity);
