...
void GPIOPresence::removeObj(const std::string& objPath)
{
    ...
    lg2::debug("erasing configuration for object path {OBJPATH}", "OBJPATH", objPath);
    ...
    event->async_wait(
        ...
        [ref{this}, &name, eventHandler,
         this](const boost::system::error_code ec) {
            ...
            const bool gpioState = (lineEvent.event_type == gpiod::line_event::RISING_EDGE);
                                           ...
                                           bool state, Config& config)
{
    ...
    lg2::debug("gpioLine {GPIO_NAME} went {GPIO_LEVEL}, meaning {CONNECT_STATUS}", "GPIO_NAME",
              gc.gpioLine, "GPIO_LEVEL", (state)?"high":"low", "CONNECT_STATUS", (present) ? "connected" : "disconnected");
...
{
    lg2::debug("initially read of all configured gpios to check device presence");
