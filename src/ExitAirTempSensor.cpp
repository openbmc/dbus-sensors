...
CFMSensor::CFMSensor(std::shared_ptr<sdbusplus::asio::connection>& conn,
    ...
    std::enable_shared_from_this<CFMSensor>(),
    ...
    parent(parent), objServer(objectServer)
{
    sensorCommon.sensorInterface = objectServer.add_interface(
        "/xyz/openbmc_project/sensors/cfm/" + sensorCommon.name,
...
void CFMSensor::setupMatches()
{
    ...
    setupSensorMatch(
        matches, *sensorCommon.dbusConnection, "fan_tach",
...
void CFMSensor::updateReading(void)
{
    ...
    if (calculate(val))
    {
        if (sensorCommon.value != val && parent)
        {
            parent->updateReading();
        }
        sensorCommon.updateValue(val);
    }
    else
    {
...
{
    sensorCommon.sensorInterface = objectServer.add_interface(
        "/xyz/openbmc_project/sensors/temperature/" + sensorCommon.name,
        "xyz.openbmc_project.Sensor.Value");
...
void ExitAirTempSensor::setupMatches(void)
{
    ...
    {
        setupSensorMatch(matches, *sensorCommon.dbusConnection, type,
        ...
        [self](boost::system::error_code ec, const GetSubTreeType& subtree) {
            ...
            for (const auto& item : subtree)
            {
                    ...
                    boost::ends_with(sensorName, "Input_Power"))
                {
                    ...
                    self->sensorCommon.dbusConnection->async_method_call(
