...
bool ExitAirTempSensor::calculate(double& val)
{
    ...
    // cfm is not expected to be less than qMin normally
    // prevent an extreme exit air temp value due to
    // an invalid cfm that might be from incorrect fan sensor reading
    if (cfm < qMin)
    {
        reading /= qMin;
    }
