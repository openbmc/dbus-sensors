...
void ModbusSensor::setSensorValue()
{
    switch (config.requestFunctionCode)
    {
        ...
        default:
            if constexpr (debug)
            {
                std::cerr << "No matching function code found for reading."
                          << std::endl;
            }
            break;
    }
...
int ModbusSensor::request()
{
    switch (config.requestFunctionCode)
    {
        ...
        default:
            if constexpr (debug)
            {
                std::cerr << "No matching function code found for requesting"
                          << std::endl;
                return -1;
            }
            break;
    ...
    waitTimer.async_wait([this](const boost::system::error_code& ec) {
        ...
        switch (config.requestFunctionCode)
        {
            ...
            default:
                if constexpr (debug)
                {
                    std::cerr << "No matching function code found for routine."
                              << std::endl;
                }
                break;
        }
...
int ModbusSensor::setupModbusConnection()
{
    ...
    {
        std::cerr << "No matching protocol." << std::endl;
        return -1;
    }

    if (modbus_set_slave(mb, config.deviceId) == -1)
    {
        std::cerr << "Failed to set device ID." << std::endl;
        return -1;
    }
...
void ModbusSensor::init()
{
    ...
    switch (config.requestFunctionCode)
    {
        ...
        default:
            if constexpr (debug)
                std::cerr
                    << "No matching function code found for initialization."
                    << std::endl;
            break;
    }
